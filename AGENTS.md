...
- **IA (roteamento):** consultas leves → **Gemini** (`GOOGLE_GENAI_API_KEY`); relatórios e tarefas pesadas → **DeepSeek** (`DEEPSEEK_API_KEY`). Ver `docs/IA-ROTEAMENTO.md`. Sem chaves, fluxos de IA falham; o restante do app funciona.
...
- **Pasta local de referências para IA (import / RAG):** opcional em `.env.local` — `ENABLE_AI_LOCAL_IMPORT=true` para ativar `POST /api/ai-lab/import-reference-files` (omissão: 503); `NEXT_PUBLIC_AI_REFERENCE_FILES_PATH` (sugestão na UI) e `AI_REFERENCE_FILES_PATH` no servidor. Se ambos vazios, a API usa a pasta `termos de referencia/` na raiz. Produção (App Hosting): `docs/APP-HOSTING-VARIAVEIS.md`. Ver também `docs/AUDITORIA-CODIGO-MORTO-E-API.md`.
