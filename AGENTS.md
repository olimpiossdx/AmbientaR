...
- **Pasta local de referências para IA (import / RAG):** opcional em `.env.local` — `NEXT_PUBLIC_AI_REFERENCE_FILES_PATH` (sugestão na UI) e, no servidor, `AI_REFERENCE_FILES_PATH` (prioritário em `POST /api/ai-lab/import-reference-files`). Se ambos vazios, a API usa a pasta `termos de referencia/` na raiz do projeto. Ver `docs/AUDITORIA-CODIGO-MORTO-E-API.md`.
