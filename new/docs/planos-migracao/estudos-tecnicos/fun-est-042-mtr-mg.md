...
Migrar a funcionalidade **MTR-MG** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: integração isolada e idempotente; AC-INT, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **MTR-MG**.
- Rotas alvo declaradas: /app/studies/mtr.
- Claim do filho: proposta: recurso.estudo-mtr-mg=*; validar no refinamento.
...
- `web/src/app/(app)/studies/mtr/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-mtr-mg=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /mtr-studies; sync/export jobs.
...
- [ ] Regra específica validada: integração isolada e idempotente; AC-INT, AC-JOB.
