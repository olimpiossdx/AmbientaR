# FUN-EST-013 — Inventário Florestal / Lista
...
Migrar a funcionalidade **Inventário Florestal / Lista** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: unidade amostral, método e versão; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário Florestal / Lista**.
- Rotas alvo declaradas: /app/studies/inventario, /$id.
- Claim do filho: proposta: recurso.estudo-inventario-florestal-lista=*; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/page.tsx`
- `web/src/app/(app)/studies/inventario/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-florestal-lista=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /forest-inventories.
...
- [ ] Regra específica validada: unidade amostral, método e versão; AC-CRUD.
