# FUN-EST-034 — Barragens / Piscinão
...
Migrar a funcionalidade **Barragens / Piscinão** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: balanço hídrico e geometria; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Barragens / Piscinão**.
- Rotas alvo declaradas: /app/studies/piscinao-off-stream, /new, /$id/edit.
- Claim do filho: proposta: recurso.estudo-barragens-piscinao=*; validar no refinamento.
...
- `web/src/app/(app)/studies/piscinao-off-stream/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/piscinao-off-stream/new/page.tsx`
- `web/src/app/(app)/studies/piscinao-off-stream/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-barragens-piscinao=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /offstream-reservoir-projects; calculation/export jobs.
...
- [ ] Regra específica validada: balanço hídrico e geometria; AC-JOB.
