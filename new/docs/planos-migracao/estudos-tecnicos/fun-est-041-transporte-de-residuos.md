# FUN-EST-041 — Transporte de resíduos
...
Migrar a funcionalidade **Transporte de resíduos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: transportador, resíduo e destino; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Transporte de resíduos**.
- Rotas alvo declaradas: /app/studies/relatorios-diversos/transporte-residuos.
- Claim do filho: proposta: recurso.estudo-transporte-de-residuos=*; validar no refinamento.
...
- `web/src/app/(app)/studies/relatorios-diversos/transporte-residuos/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-transporte-de-residuos=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /misc-reports/waste-transport; exports.
...
- [ ] Regra específica validada: transportador, resíduo e destino; AC-CRUD, AC-JOB.
