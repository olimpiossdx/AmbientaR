# FUN-EST-002 — Programa de Ação Emergencial
...
Migrar a funcionalidade **Programa de Ação Emergencial** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cenários, responsáveis e revisão; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Programa de Ação Emergencial**.
- Rotas alvo declaradas: /app/studies/acao-emergencial.
- Claim do filho: proposta: recurso.estudo-programa-de-acao-emergencial=*; validar no refinamento.
...
- `web/src/app/(app)/studies/acao-emergencial/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-programa-de-acao-emergencial=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /studies/emergency-action-programs.
...
- [ ] Regra específica validada: cenários, responsáveis e revisão; AC-CRUD, AC-JOB.
