# FUN-EST-015 — Inventário / Parcelas
...
Migrar a funcionalidade **Inventário / Parcelas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: área e geometria; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário / Parcelas**.
- Rotas alvo declaradas: /app/studies/inventario/$id/parcelas.
- Claim do filho: proposta: recurso.estudo-inventario-parcelas=*; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/parcelas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-parcelas=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /forest-inventories/{id}/plots.
...
- [ ] Regra específica validada: área e geometria; AC-CRUD.
