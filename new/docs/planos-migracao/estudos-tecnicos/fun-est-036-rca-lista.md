...
Migrar a funcionalidade **RCA / Lista** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: listagem A–H; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **RCA / Lista**.
- Rotas alvo declaradas: /app/studies/rca, /$id/edit.
- Claim do filho: proposta: recurso.estudo-rca-lista=*; validar no refinamento.
...
- `web/src/app/(app)/studies/rca/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/studies/rca/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/rca/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-rca-lista=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /studies/rca.
...
- [ ] Regra específica validada: listagem A–H; AC-CRUD.
