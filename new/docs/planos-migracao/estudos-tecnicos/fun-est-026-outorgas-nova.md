...
Migrar a funcionalidade **Outorgas / Nova** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: pré-requisitos e escopo; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Outorgas / Nova**.
- Rotas alvo declaradas: /app/studies/outorgas/new.
- Claim do filho: proposta: recurso.estudo-outorgas-nova=*; validar no refinamento.
...
- `web/src/app/(app)/studies/outorgas/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-outorgas-nova=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /water-grant-processes.
...
- [ ] Regra específica validada: pré-requisitos e escopo; AC-WORKFLOW.
