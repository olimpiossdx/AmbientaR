# FUN-EST-017 — Inventário / Fórmulas
...
Migrar a funcionalidade **Inventário / Fórmulas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: expressão validada e versionada; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário / Fórmulas**.
- Rotas alvo declaradas: /app/studies/inventario/$id/formulas.
- Claim do filho: proposta: recurso.estudo-inventario-formulas=*; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/formulas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-formulas=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /forest-inventories/{id}/formulas.
...
- [ ] Regra específica validada: expressão validada e versionada; AC-CRUD.
