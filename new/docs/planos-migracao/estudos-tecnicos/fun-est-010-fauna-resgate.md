...
Migrar a funcionalidade **Fauna / Resgate** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: indivíduo, destino e cadeia de custódia; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Fauna / Resgate**.
- Rotas alvo declaradas: /app/studies/fauna/resgate, /$id.
- Claim do filho: proposta: recurso.estudo-fauna-resgate=*; validar no refinamento.
...
- `web/src/app/(app)/studies/fauna/resgate/[id]/page.tsx`
- `web/src/app/(app)/studies/fauna/resgate/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-fauna-resgate=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fauna-studies/rescues.
...
- [ ] Regra específica validada: indivíduo, destino e cadeia de custódia; AC-CRUD.
