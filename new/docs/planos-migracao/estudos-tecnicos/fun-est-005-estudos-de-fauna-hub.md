# FUN-EST-005 — Estudos de Fauna / Hub
...
Migrar a funcionalidade **Estudos de Fauna / Hub** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cards somente com claims filhas; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Estudos de Fauna / Hub**.
- Rotas alvo declaradas: /app/studies/fauna.
- Claim do filho: proposta: recurso.estudo-estudos-de-fauna-hub=visualizar; validar no refinamento.
...
- `web/src/app/(app)/studies/fauna/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-estudos-de-fauna-hub=visualizar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /fauna-studies/dashboard.
...
- [ ] Regra específica validada: cards somente com claims filhas; AC-READ.
