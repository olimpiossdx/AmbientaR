# FUN-EST-046 — Compensação / Mata Atlântica
...
Migrar a funcionalidade **Compensação / Mata Atlântica** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: área e proporção; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Compensação / Mata Atlântica**.
- Rotas alvo declaradas: /app/studies/compensacao-ambiental/mata-atlantica.
- Claim do filho: proposta: recurso.estudo-compensacao-mata-atlantica=*; validar no refinamento.
...
- `web/src/app/(app)/studies/compensacao-ambiental/[tipo]/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-compensacao-mata-atlantica=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /environmental-compensations/atlantic-forest.
...
- [ ] Regra específica validada: área e proporção; AC-CRUD.
