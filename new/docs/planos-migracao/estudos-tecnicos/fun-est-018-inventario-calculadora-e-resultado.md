# FUN-EST-018 — Inventário / Calculadora e Resultado
...
Migrar a funcionalidade **Inventário / Calculadora e Resultado** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fórmula/entrada congeladas e reprodutíveis; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário / Calculadora e Resultado**.
- Rotas alvo declaradas: /app/studies/inventario/$id/calculadora, /resultado/$runId.
- Claim do filho: proposta: recurso.estudo-inventario-calculadora-e-resultado=executar; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/calculadora/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-calculadora-e-resultado=executar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /forest-inventories/{id}/calculation-jobs; resultado.
...
- [ ] Regra específica validada: fórmula/entrada congeladas e reprodutíveis; AC-JOB.
