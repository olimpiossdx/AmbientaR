# FUN-EST-014 — Inventário / Árvores
...
Migrar a funcionalidade **Inventário / Árvores** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: DAP, altura, espécie e parcela; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário / Árvores**.
- Rotas alvo declaradas: /app/studies/inventario/$id/arvores.
- Claim do filho: proposta: recurso.estudo-inventario-arvores=*; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/arvores/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-arvores=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /forest-inventories/{id}/trees.
...
- [ ] Regra específica validada: DAP, altura, espécie e parcela; AC-CRUD.
