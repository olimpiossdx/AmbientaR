...
Migrar a funcionalidade **Coleta de Campo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: offline apenas se aprovado; conflito explícito; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Coleta de Campo**.
- Rotas alvo declaradas: /app/coleta-campo, /nova, /$id, /$id/parcelas/$parcelaId.
- Claim do filho: proposta: recurso.estudo-coleta-de-campo=*; validar no refinamento.
...
- `web/src/app/(app)/coleta-campo/[id]/page.tsx`
- `web/src/app/(app)/coleta-campo/[id]/parcelas/[parcelaId]/page.tsx`
- `web/src/app/(app)/coleta-campo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-coleta-de-campo=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /field-collections; sub-recurso plots.
...
- [ ] Regra específica validada: offline apenas se aprovado; conflito explícito; AC-CRUD.
