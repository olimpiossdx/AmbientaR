# FUN-EST-025 — Outorgas / Processos
...
Migrar a funcionalidade **Outorgas / Processos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fases, protocolo e documentos; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Outorgas / Processos**.
- Rotas alvo declaradas: /app/studies/outorgas, /processo/$id, /$id/edit.
- Claim do filho: proposta: recurso.estudo-outorgas-processos=*; validar no refinamento.
...
- `web/src/app/(app)/studies/outorgas/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/outorgas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-outorgas-processos=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /water-grant-processes.
...
- [ ] Regra específica validada: fases, protocolo e documentos; AC-WORKFLOW.
