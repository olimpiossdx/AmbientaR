# FUN-EST-007 — Fauna / Relatório de Inventário
...
Migrar a funcionalidade **Fauna / Relatório de Inventário** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: documento visual e dados congelados por versão; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Fauna / Relatório de Inventário**.
- Rotas alvo declaradas: /app/studies/fauna/inventario-relatorio, /$id.
- Claim do filho: proposta: recurso.estudo-fauna-relatorio-de-inventario=executar; validar no refinamento.
...
- `web/src/app/(app)/studies/fauna/inventario-relatorio/[id]/page.tsx`
- `web/src/app/(app)/studies/fauna/inventario-relatorio/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-fauna-relatorio-de-inventario=executar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /fauna-studies/inventories/{id}/report-jobs.
...
- [ ] Regra específica validada: documento visual e dados congelados por versão; AC-JOB.
