...
Migrar a funcionalidade **Compensação / APP** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: intervenção e área; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Compensação / APP**.
- Rotas alvo declaradas: /app/studies/compensacao-ambiental/app.
- Claim do filho: proposta: recurso.estudo-compensacao-app=*; validar no refinamento.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-compensacao-app=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /environmental-compensations/app.
...
- [ ] Regra específica validada: intervenção e área; AC-CRUD.
