# FUN-EST-043 — Compensação / Visão geral
...
Migrar a funcionalidade **Compensação / Visão geral** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.
...
- Filho/tela: **Compensação / Visão geral**.
- Rotas alvo declaradas: /app/studies/compensacao-ambiental.
- Claim do filho: proposta: recurso.estudo-compensacao-visao-geral=visualizar; validar no refinamento.
...
- `web/src/app/(app)/studies/compensacao-ambiental/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-compensacao-visao-geral=visualizar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /environmental-compensations/dashboard.
