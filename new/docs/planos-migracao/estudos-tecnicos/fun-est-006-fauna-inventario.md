...
Migrar a funcionalidade **Fauna / Inventário** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: campanha, grupo, ponto e espécie; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Fauna / Inventário**.
- Rotas alvo declaradas: /app/studies/fauna/inventario, /$id.
- Claim do filho: proposta: recurso.estudo-fauna-inventario=*; validar no refinamento.
...
- `web/src/app/(app)/studies/fauna/inventario/[id]/page.tsx`
- `web/src/app/(app)/studies/fauna/inventario/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-fauna-inventario=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fauna-studies/inventories.
...
- [ ] Regra específica validada: campanha, grupo, ponto e espécie; AC-CRUD.
