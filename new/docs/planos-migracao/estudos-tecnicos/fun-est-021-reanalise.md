...
Migrar a funcionalidade **Reanálise** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: referência ao estudo anterior e motivo; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Reanálise**.
- Rotas alvo declaradas: /app/studies/reanalise, /new, /$id/edit.
- Claim do filho: proposta: recurso.estudo-reanalise=*; validar no refinamento.
...
- `web/src/app/(app)/studies/reanalise/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/reanalise/new/page.tsx`
- `web/src/app/(app)/studies/reanalise/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-reanalise=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /studies/reanalysis; exports.
...
- [ ] Regra específica validada: referência ao estudo anterior e motivo; AC-CRUD, AC-JOB.
