# FUN-EST-016 — Inventário / Espécies
...
Migrar a funcionalidade **Inventário / Espécies** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: taxonomia e proteção; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Inventário / Espécies**.
- Rotas alvo declaradas: /app/studies/inventario/$id/especies.
- Claim do filho: proposta: recurso.estudo-inventario-especies=*; validar no refinamento.
...
- `web/src/app/(app)/studies/inventario/[id]/especies/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-inventario-especies=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /forest-inventories/{id}/species.
...
- [ ] Regra específica validada: taxonomia e proteção; AC-CRUD.
