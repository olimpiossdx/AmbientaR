# FUN-EST-008 — Fauna / Monitoramento
...
Migrar a funcionalidade **Fauna / Monitoramento** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: campanhas comparáveis; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Fauna / Monitoramento**.
- Rotas alvo declaradas: /app/studies/fauna/monitoramento, /$id.
- Claim do filho: proposta: recurso.estudo-fauna-monitoramento=*; validar no refinamento.
...
- `web/src/app/(app)/studies/fauna/monitoramento/[id]/page.tsx`
- `web/src/app/(app)/studies/fauna/monitoramento/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-fauna-monitoramento=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fauna-studies/monitoring.
...
- [ ] Regra específica validada: campanhas comparáveis; AC-CRUD.
