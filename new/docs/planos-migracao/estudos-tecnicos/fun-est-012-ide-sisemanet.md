...
Migrar a funcionalidade **IDE-SisemaNet** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.ide-sisema=consultar; AC-INT, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **IDE-SisemaNet**.
- Rotas alvo declaradas: /app/studies/ide-sisemanet.
- Claim do filho: recurso.ide-sisema=consultar; AC-INT, AC-JOB.
...
- `web/src/app/(app)/studies/ide-sisemanet/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.ide-sisema=consultar; AC-INT, AC-JOB; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /integrations/ide-sisema/catalog, analysis jobs.
...
- [ ] Regra específica validada: recurso.ide-sisema=consultar; AC-INT, AC-JOB.
