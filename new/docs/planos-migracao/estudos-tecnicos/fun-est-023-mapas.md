...
Migrar a funcionalidade **Mapas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: CRS, camadas, escala e layout; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Mapas**.
- Rotas alvo declaradas: /app/studies/mapas.
- Claim do filho: proposta: recurso.estudo-mapas=*; validar no refinamento.
...
- `web/src/app/(app)/studies/mapas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-mapas=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /study-maps; render jobs.
...
- [ ] Regra específica validada: CRS, camadas, escala e layout; AC-CRUD, AC-JOB.
