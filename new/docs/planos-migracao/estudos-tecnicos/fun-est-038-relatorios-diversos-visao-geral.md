# FUN-EST-038 — Relatórios Diversos / Visão geral
...
Migrar a funcionalidade **Relatórios Diversos / Visão geral** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.relatorio-diverso=visualizar; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Relatórios Diversos / Visão geral**.
- Rotas alvo declaradas: /app/studies/relatorios-diversos.
- Claim do filho: recurso.relatorio-diverso=visualizar; AC-READ.
...
- `web/src/app/(app)/studies/relatorios-diversos/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.relatorio-diverso=visualizar; AC-READ; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /misc-reports/catalog.
...
- [ ] Regra específica validada: recurso.relatorio-diverso=visualizar; AC-READ.
