...
Migrar a funcionalidade **Procuração** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: outorgante/outorgado, poderes e validade; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Procuração**.
- Rotas alvo declaradas: /app/studies/procuracao, /new, /$id/edit.
- Claim do filho: proposta: recurso.estudo-procuracao=*; validar no refinamento.
...
- `web/src/app/(app)/studies/procuracao/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/procuracao/new/page.tsx`
- `web/src/app/(app)/studies/procuracao/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-procuracao=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /powers-of-attorney; exports.
...
- [ ] Regra específica validada: outorgante/outorgado, poderes e validade; AC-CRUD, AC-JOB.
