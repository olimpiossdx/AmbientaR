...
Migrar a funcionalidade **Compensação / SNUC** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cálculo e base legal; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Compensação / SNUC**.
- Rotas alvo declaradas: /app/studies/compensacao-ambiental/snuc.
- Claim do filho: proposta: recurso.estudo-compensacao-snuc=*; validar no refinamento.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-compensacao-snuc=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /environmental-compensations/snuc.
...
- [ ] Regra específica validada: cálculo e base legal; AC-CRUD.
