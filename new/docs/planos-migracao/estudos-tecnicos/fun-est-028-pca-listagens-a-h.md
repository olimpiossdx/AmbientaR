...
Migrar a funcionalidade **PCA / Listagens A–H** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cada schema versionado; export preserva versão; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **PCA / Listagens A–H**.
- Rotas alvo declaradas: /app/studies/pca/new?listagem=A..H.
- Claim do filho: proposta: recurso.estudo-pca-listagens-a-h=*; validar no refinamento.
...
- `web/src/app/(app)/studies/pca/(.)new/page.tsx`
- `web/src/app/(app)/studies/pca/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-pca-listagens-a-h=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /studies/pca; schemas GET /study-schemas/pca/{A..H}.
...
- [ ] Regra específica validada: cada schema versionado; export preserva versão; AC-CRUD, AC-JOB.
