...
Migrar a funcionalidade **RCA / Listagens A–H** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: schema/export versionados; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **RCA / Listagens A–H**.
- Rotas alvo declaradas: /app/studies/rca/new?listagem=A..H.
- Claim do filho: proposta: recurso.estudo-rca-listagens-a-h=*; validar no refinamento.
...
- `web/src/app/(app)/studies/rca/(.)new/page.tsx`
- `web/src/app/(app)/studies/rca/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-rca-listagens-a-h=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /studies/rca; schemas GET /study-schemas/rca/{A..H}.
...
- [ ] Regra específica validada: schema/export versionados; AC-CRUD, AC-JOB.
