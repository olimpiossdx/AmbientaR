# FUN-EST-044 — Compensação / Espécies
...
Migrar a funcionalidade **Compensação / Espécies** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: base normativa versionada; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Compensação / Espécies**.
- Rotas alvo declaradas: /app/studies/compensacao-ambiental/especies.
- Claim do filho: proposta: recurso.estudo-compensacao-especies=*; validar no refinamento.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-compensacao-especies=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /environmental-compensations/protected-species.
...
- [ ] Regra específica validada: base normativa versionada; AC-CRUD.
