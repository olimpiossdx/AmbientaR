...
Migrar a funcionalidade **PCA / Lista** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: listagem A–H é enum validado; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **PCA / Lista**.
- Rotas alvo declaradas: /app/studies/pca, /$id/edit.
- Claim do filho: proposta: recurso.estudo-pca-lista=*; validar no refinamento.
...
- `web/src/app/(app)/studies/pca/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/studies/pca/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/pca/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-pca-lista=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /studies/pca.
...
- [ ] Regra específica validada: listagem A–H é enum validado; AC-CRUD.
