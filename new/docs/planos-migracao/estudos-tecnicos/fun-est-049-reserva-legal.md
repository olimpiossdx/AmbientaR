...
Migrar a funcionalidade **Reserva Legal** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: imóvel, percentual, geometria e compensação; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Reserva Legal**.
- Rotas alvo declaradas: /app/studies/reserva-legal.
- Claim do filho: proposta: recurso.estudo-reserva-legal=*; validar no refinamento.
...
- `web/src/app/(app)/studies/reserva-legal/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-reserva-legal=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /legal-reserves; analysis/export jobs.
...
- [ ] Regra específica validada: imóvel, percentual, geometria e compensação; AC-JOB.
