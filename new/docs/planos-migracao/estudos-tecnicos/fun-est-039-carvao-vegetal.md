...
Migrar a funcionalidade **Carvão vegetal** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **Carvão vegetal**.
- Rotas alvo declaradas: /app/studies/relatorios-diversos/carvao-vegetal.
- Claim do filho: proposta: recurso.estudo-carvao-vegetal=*; validar no refinamento.
...
- `web/src/app/(app)/studies/relatorios-diversos/carvao-vegetal/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-carvao-vegetal=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /misc-reports/charcoal; exports.
...
- [ ] Regra específica validada: AC-CRUD, AC-JOB.
