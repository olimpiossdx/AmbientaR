# FUN-EST-031 — Barragens / Visão geral
...
Migrar a funcionalidade **Barragens / Visão geral** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.
...
- Filho/tela: **Barragens / Visão geral**.
- Rotas alvo declaradas: /app/studies/barragens.
- Claim do filho: proposta: recurso.estudo-barragens-visao-geral=visualizar; validar no refinamento.
...
- `web/src/app/(app)/studies/barragens/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-barragens-visao-geral=visualizar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /dam-studies/dashboard.
