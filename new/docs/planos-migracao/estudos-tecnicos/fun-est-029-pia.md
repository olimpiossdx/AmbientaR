...
Migrar a funcionalidade **PIA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: inventário vinculado e cálculo consistente; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos**.
- Filho/tela: **PIA**.
- Rotas alvo declaradas: /app/studies/pia, /new, /$id/edit.
- Claim do filho: proposta: recurso.estudo-pia=*; validar no refinamento.
...
- `web/src/app/(app)/studies/pia/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/studies/pia/(.)new/page.tsx`
- `web/src/app/(app)/studies/pia/[id]/edit/page.tsx`
- `web/src/app/(app)/studies/pia/new/page.tsx`
- `web/src/app/(app)/studies/pia/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.estudo-pia=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /studies/pia; inventory/export jobs.
...
- [ ] Regra específica validada: inventário vinculado e cálculo consistente; AC-CRUD, AC-JOB.
