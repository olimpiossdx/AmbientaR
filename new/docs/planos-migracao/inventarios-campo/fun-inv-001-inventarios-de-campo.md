# FUN-INV-001 — Inventários de campo

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Estudos Técnicos / Inventários de campo**  
Origem funcional: seção “Ofícios, governo e ferramentas do sistema” do catálogo  
Dependência de acesso do pai: `modulo.estudos-tecnicos=acessar + recurso.inventario-campo=*`

## Objetivo e escopo

Migrar a funcionalidade **Inventários de campo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.inventario-campo=*; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Estudos Técnicos / Inventários de campo**.
- Filho/tela: **Inventários de campo**.
- Rotas alvo declaradas: /app/inventarios, /new, /$id, /$id/parcelas/$parcelaId.
- Claim do filho: recurso.inventario-campo=*; AC-CRUD.
...
- `web/src/app/(app)/inventarios/[id]/page.tsx`
- `web/src/app/(app)/inventarios/[id]/parcelas/[parcelaId]/page.tsx`
- `web/src/app/(app)/inventarios/new/page.tsx`
- `web/src/app/(app)/inventarios/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Estudos Técnicos / Inventários de campo** como pai.
...
- [ ] Implementar todas as variações de tela declaradas (4), preservando busca, filtros, paginação, seleção, ações, anexos e exportações existentes que forem confirmados no refinamento.
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.inventario-campo=*; AC-CRUD; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /field-inventories; plots.
...
- [ ] Regra específica validada: recurso.inventario-campo=*; AC-CRUD.
