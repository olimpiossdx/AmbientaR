...
Migrar a funcionalidade **FAD / Início** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Início**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/dashboard.
- Claim do filho: recurso.fad-dashboard=visualizar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/dashboard/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-dashboard=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /fad/dashboard.
...
- [ ] Regra específica validada: AC-READ.
