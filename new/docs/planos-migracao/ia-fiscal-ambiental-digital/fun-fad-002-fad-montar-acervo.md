...
Migrar a funcionalidade **FAD / Montar acervo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fonte, perímetro e período; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Montar acervo**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/montar-acervo.
- Claim do filho: recurso.fad-acervo=criar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/montar-acervo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-acervo=criar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /fad/collection-jobs.
...
- [ ] Regra específica validada: fonte, perímetro e período; AC-JOB.
