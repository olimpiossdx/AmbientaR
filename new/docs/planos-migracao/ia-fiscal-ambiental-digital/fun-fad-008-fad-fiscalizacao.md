...
Migrar a funcionalidade **FAD / Fiscalização** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: checks determinísticos identificados; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Fiscalização**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/fiscalizacao.
- Claim do filho: recurso.fad-fiscalizacao=executar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/fiscalizacao/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-fiscalizacao=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /fad/inspection-check-jobs.
...
- [ ] Regra específica validada: checks determinísticos identificados; AC-JOB.
