...
Migrar a funcionalidade **FAD / Workspace** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: membro/ownership e sub-recursos escopados; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Workspace**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/workspace/$workspaceId.
- Claim do filho: recurso.fad-workspace=*.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-workspace=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fad/workspaces.
...
- [ ] Regra específica validada: membro/ownership e sub-recursos escopados; AC-CRUD.
