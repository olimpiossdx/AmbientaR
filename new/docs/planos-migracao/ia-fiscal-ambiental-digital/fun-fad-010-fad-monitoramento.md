...
Migrar a funcionalidade **FAD / Monitoramento** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: regra, agenda e deduplicação; AC-WORKFLOW, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Monitoramento**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/monitoramento.
- Claim do filho: recurso.fad-monitoramento=*.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/monitoramento/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-monitoramento=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fad/monitoring-rules; POST /runs.
...
- [ ] Regra específica validada: regra, agenda e deduplicação; AC-WORKFLOW, AC-JOB.
