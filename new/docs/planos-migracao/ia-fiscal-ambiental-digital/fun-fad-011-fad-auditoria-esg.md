...
Migrar a funcionalidade **FAD / Auditoria ESG** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: metodologia e snapshot imutável; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Auditoria ESG**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/esg.
- Claim do filho: recurso.fad-esg com visualizar/executar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/esg/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-esg com visualizar/executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /fad/esg, POST /fad/esg/snapshots.
...
- [ ] Regra específica validada: metodologia e snapshot imutável; AC-READ.
