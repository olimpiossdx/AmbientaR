...
Migrar a funcionalidade **FAD / Comparar** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: duas fontes compatíveis e resultado versionado; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Comparar**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/comparador.
- Claim do filho: recurso.fad-comparador=executar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/comparador/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-comparador=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /fad/comparison-jobs.
...
- [ ] Regra específica validada: duas fontes compatíveis e resultado versionado; AC-JOB.
