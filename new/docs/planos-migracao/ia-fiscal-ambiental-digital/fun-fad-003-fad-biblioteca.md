...
Migrar a funcionalidade **FAD / Biblioteca** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: ownership e download; AC-READ, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Biblioteca**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/biblioteca.
- Claim do filho: recurso.fad-biblioteca=visualizar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/biblioteca/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-biblioteca=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /fad/assets, API-FILE.
...
- [ ] Regra específica validada: ownership e download; AC-READ, AC-FILE.
