# FUN-FAD-012 — FAD / Configurações

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **IA / Fiscal Ambiental Digital**  
Origem funcional: seção “IA e Fiscal Ambiental Digital” do catálogo  
Dependência de acesso do pai: `modulo.ia=acessar + modulo.fiscal-ambiental=acessar`

## Objetivo e escopo

Migrar a funcionalidade **FAD / Configurações** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: segredo não retorna; versão e auditoria; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **IA / Fiscal Ambiental Digital**.
- Filho/tela: **FAD / Configurações**.
- Rotas alvo declaradas: /app/ia/fiscal-ambiental-digital/configuracoes.
- Claim do filho: recurso.fad-configuracao=configurar.
...
- `web/src/app/(app)/ia/fiscal-ambiental-digital/configuracoes/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **IA / Fiscal Ambiental Digital** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fad-configuracao=configurar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /fad/settings.
...
- [ ] Regra específica validada: segredo não retorna; versão e auditoria; AC-CRUD.
