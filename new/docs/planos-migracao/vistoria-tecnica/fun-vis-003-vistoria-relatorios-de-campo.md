# FUN-VIS-003 — Vistoria / Relatórios de Campo
...
Migrar a funcionalidade **Vistoria / Relatórios de Campo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fotos, coordenadas, assinatura e documento visual validado; AC-JOB, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Vistoria Técnica**.
- Filho/tela: **Vistoria / Relatórios de Campo**.
- Rotas alvo declaradas: /app/inspections/reports.
- Claim do filho: recurso.vistoria-relatorio=exportar.
...
- `web/src/app/(app)/inspections/reports/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.vistoria-relatorio=exportar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /inspections/{id}/report-jobs.
...
- [ ] Regra específica validada: fotos, coordenadas, assinatura e documento visual validado; AC-JOB, AC-FILE.
