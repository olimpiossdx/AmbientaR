...
Migrar a funcionalidade **Vistoria / Nova** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: checklist coerente com tipo; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Vistoria Técnica**.
- Filho/tela: **Vistoria / Nova**.
- Rotas alvo declaradas: /app/inspections/new.
- Claim do filho: recurso.vistoria=criar.
...
- `web/src/app/(app)/inspections/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.vistoria=criar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /inspections.
...
- [ ] Regra específica validada: checklist coerente com tipo; AC-CRUD.
