# FUN-VIS-001 — Vistoria / Consultar

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Vistoria Técnica**  
Origem funcional: seção “Multas, vistorias, licenciamento e processos” do catálogo  
Dependência de acesso do pai: `modulo.vistoria=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Vistoria / Consultar** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: empreendimento, equipe, agenda e status; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Vistoria Técnica**.
- Filho/tela: **Vistoria / Consultar**.
- Rotas alvo declaradas: /app/inspections, /$id/edit.
- Claim do filho: recurso.vistoria=*.
...
- `web/src/app/(app)/inspections/[id]/edit/page.tsx`
- `web/src/app/(app)/inspections/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Vistoria Técnica** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.vistoria=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /inspections.
...
- [ ] Regra específica validada: empreendimento, equipe, agenda e status; AC-CRUD.
