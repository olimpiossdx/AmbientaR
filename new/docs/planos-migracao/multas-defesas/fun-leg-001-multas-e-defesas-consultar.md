# FUN-LEG-001 — Multas e Defesas / Consultar

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Multas e Defesas**  
...
Migrar a funcionalidade **Multas e Defesas / Consultar** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: prazo processual e escopo; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Multas e Defesas**.
- Filho/tela: **Multas e Defesas / Consultar**.
- Rotas alvo declaradas: /app/multas-defesas, /$id.
- Claim do filho: recurso.multa-defesa=visualizar.
...
- `web/src/app/(app)/multas-defesas/[id]/page.tsx`
- `web/src/app/(app)/multas-defesas/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Multas e Defesas** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.multa-defesa=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /environmental-fines, GET /{id}.
...
- [ ] Regra específica validada: prazo processual e escopo; AC-READ.
