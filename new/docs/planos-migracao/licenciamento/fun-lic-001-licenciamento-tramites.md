# FUN-LIC-001 — Licenciamento / Trâmites

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Licenciamento**  
Origem funcional: seção “Multas, vistorias, licenciamento e processos” do catálogo  
Dependência de acesso do pai: `modulo.licenciamento=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Licenciamento / Trâmites** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: protocolo, órgão, fase e transições; AC-WORKFLOW, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Licenciamento**.
- Filho/tela: **Licenciamento / Trâmites**.
- Rotas alvo declaradas: /app/requests, /$id/edit.
- Claim do filho: recurso.tramite-licenciamento=*.
...
- `web/src/app/(app)/requests/[id]/edit/page.tsx`
- `web/src/app/(app)/requests/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Licenciamento** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.tramite-licenciamento=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /licensing-requests; ações.
...
- [ ] Regra específica validada: protocolo, órgão, fase e transições; AC-WORKFLOW, AC-FILE.
