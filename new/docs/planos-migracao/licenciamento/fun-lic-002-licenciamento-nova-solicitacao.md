# FUN-LIC-002 — Licenciamento / Nova solicitação
...
Migrar a funcionalidade **Licenciamento / Nova solicitação** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: pré-requisitos e titularidade; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Licenciamento**.
- Filho/tela: **Licenciamento / Nova solicitação**.
- Rotas alvo declaradas: /app/requests/new.
- Claim do filho: recurso.tramite-licenciamento=criar.
...
- `web/src/app/(app)/requests/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.tramite-licenciamento=criar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /licensing-requests.
...
- [ ] Regra específica validada: pré-requisitos e titularidade; AC-WORKFLOW.
