...
Migrar a funcionalidade **Licenciamento / AIA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: checklist AIA e vínculo com solicitação; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Licenciamento**.
- Filho/tela: **Licenciamento / AIA**.
- Rotas alvo declaradas: /app/requests/$id/aia.
- Claim do filho: recurso.aia com visualizar/editar.
...
- `web/src/app/(app)/requests/[id]/aia/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.aia com visualizar/editar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PUT /licensing-requests/{id}/aia.
...
- [ ] Regra específica validada: checklist AIA e vínculo com solicitação; AC-CRUD.
