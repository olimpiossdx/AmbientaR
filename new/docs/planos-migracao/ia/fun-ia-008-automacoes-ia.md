...
Migrar a funcionalidade **Automações IA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: agenda, limites e execução auditada; AC-WORKFLOW, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Automações IA**.
- Rotas alvo declaradas: /app/ai-lab/automations.
- Claim do filho: recurso.automacao-ia=*.
...
- `web/src/app/(app)/ai-lab/automations/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.automacao-ia=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /ai-automations; ações run/enable/disable.
...
- [ ] Regra específica validada: agenda, limites e execução auditada; AC-WORKFLOW, AC-JOB.
