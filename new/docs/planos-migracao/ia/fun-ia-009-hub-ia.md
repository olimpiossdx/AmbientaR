...
Migrar a funcionalidade **Hub IA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: só mostra capacidades com claim; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Hub IA**.
- Rotas alvo declaradas: /app/ai-lab.
- Claim do filho: modulo.ia=acessar.
...
- `web/src/app/(app)/ai-lab/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para modulo.ia=acessar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /ai/capabilities.
...
- [ ] Regra específica validada: só mostra capacidades com claim; AC-READ.
