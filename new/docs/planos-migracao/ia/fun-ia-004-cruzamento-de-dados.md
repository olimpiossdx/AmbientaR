# FUN-IA-004 — Cruzamento de dados
...
Migrar a funcionalidade **Cruzamento de dados** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: tools allowlist e rate limit; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Cruzamento de dados**.
- Rotas alvo declaradas: /app/studies/assistant?type=mcp.
- Claim do filho: recurso.assistente-cruzamento=executar.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.assistente-cruzamento=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /ai/assistants/data-crossing/runs.
...
- [ ] Regra específica validada: tools allowlist e rate limit; AC-JOB.
