# FUN-IA-002 — Análise Geoespacial
...
Migrar a funcionalidade **Análise Geoespacial** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: geometria válida, catálogo versionado e resultado reproduzível; AC-JOB, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Análise Geoespacial**.
- Rotas alvo declaradas: /app/analise-ambiental.
- Claim do filho: recurso.analise-geoespacial=executar.
...
- `web/src/app/(app)/analise-ambiental/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **IA** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.analise-geoespacial=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /geo-analysis-jobs.
...
- [ ] Regra específica validada: geometria válida, catálogo versionado e resultado reproduzível; AC-JOB, AC-FILE.
