# FUN-IA-005 — Legislação e estudos
...
Migrar a funcionalidade **Legislação e estudos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: citações e fontes retornadas; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Legislação e estudos**.
- Rotas alvo declaradas: /app/studies/assistant?type=geral.
- Claim do filho: recurso.assistente-legislacao=executar.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.assistente-legislacao=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /ai/assistants/legal/runs.
...
- [ ] Regra específica validada: citações e fontes retornadas; AC-JOB.
