...
Migrar a funcionalidade **Síntese de texto** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: arquivos autorizados e retenção definida; AC-JOB, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Síntese de texto**.
- Rotas alvo declaradas: /app/studies/assistant?type=rag.
- Claim do filho: recurso.assistente-sintese=executar.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.assistente-sintese=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /ai/assistants/summarization/runs.
...
- [ ] Regra específica validada: arquivos autorizados e retenção definida; AC-JOB, AC-FILE.
