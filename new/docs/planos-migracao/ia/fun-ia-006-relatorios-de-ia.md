...
Migrar a funcionalidade **Relatórios de IA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: escopo por autor/organização; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Relatórios de IA**.
- Rotas alvo declaradas: /app/reporting.
- Claim do filho: recurso.relatorio-ia=visualizar.
...
- `web/src/app/(app)/reporting/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.relatorio-ia=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /ai/reports, GET /{id}.
...
- [ ] Regra específica validada: escopo por autor/organização; AC-READ.
