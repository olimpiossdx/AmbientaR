...
Migrar a funcionalidade **Águas / MIRA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fontes e escopo explícitos; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **IA**.
- Filho/tela: **Águas / MIRA**.
- Rotas alvo declaradas: /app/studies/assistant?type=mira.
- Claim do filho: recurso.assistente-mira=executar.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.assistente-mira=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /ai/assistants/mira/runs.
...
- [ ] Regra específica validada: fontes e escopo explícitos; AC-JOB.
