...
Migrar a funcionalidade **DRE Contábil** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: competência/caixa explícito, categorias e totalização; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **DRE Contábil**.
- Rotas alvo declaradas: /app/financial/dre-contabil.
- Claim do filho: recurso.dre=visualizar.
...
- `web/src/app/(app)/financial/dre-contabil/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.dre=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /financial/reports/income-statement.
...
- [ ] Regra específica validada: competência/caixa explícito, categorias e totalização; AC-READ.
