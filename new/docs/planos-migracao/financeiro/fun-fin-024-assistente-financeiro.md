# FUN-FIN-024 — Assistente Financeiro
...
Migrar a funcionalidade **Assistente Financeiro** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fontes autorizadas, custo/rate limit e sem acesso implícito a dados; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Assistente Financeiro**.
- Rotas alvo declaradas: /app/studies/assistant?type=financeiro.
- Claim do filho: recurso.assistente-financeiro=executar.
...
- `web/src/app/(app)/studies/assistant/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.assistente-financeiro=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /ai/assistants/financial/runs.
...
- [ ] Regra específica validada: fontes autorizadas, custo/rate limit e sem acesso implícito a dados; AC-JOB.
