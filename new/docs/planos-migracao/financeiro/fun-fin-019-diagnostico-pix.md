...
Migrar a funcionalidade **Diagnóstico PIX** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: sem segredo/payload sensível; retry idempotente; AC-READ, AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Diagnóstico PIX**.
- Rotas alvo declaradas: /app/financial/billing-debug.
- Claim do filho: recurso.diagnostico-cobranca=visualizar ou executar.
...
- `web/src/app/(app)/financial/billing-debug/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.diagnostico-cobranca=visualizar ou executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /billing/diagnostics, POST /billing/charges/{id}/retry.
...
- [ ] Regra específica validada: sem segredo/payload sensível; retry idempotente; AC-READ, AC-WORKFLOW.
