# FUN-FIN-004 — Contratos da Plataforma
...
Migrar a funcionalidade **Contratos da Plataforma** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cobrança e acesso mudam atomicamente; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Contratos da Plataforma**.
- Rotas alvo declaradas: /app/financial/platform-subscription-contracts.
- Claim do filho: recurso.contrato-plataforma=*.
...
- `web/src/app/(app)/financial/platform-subscription-contracts/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.contrato-plataforma=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /platform-contracts, GET /{id}, ações activate/suspend/cancel.
...
- [ ] Regra específica validada: cobrança e acesso mudam atomicamente; AC-WORKFLOW.
