...
Migrar a funcionalidade **Acesso Bancário** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: segredos cifrados e nunca retornados completos; auditoria de leitura; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Acesso Bancário**.
- Rotas alvo declaradas: /app/bank-access.
- Claim do filho: recurso.acesso-bancario=*.
...
- `web/src/app/(app)/bank-access/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.acesso-bancario=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /bank-access.
...
- [ ] Regra específica validada: segredos cifrados e nunca retornados completos; auditoria de leitura; AC-CRUD.
