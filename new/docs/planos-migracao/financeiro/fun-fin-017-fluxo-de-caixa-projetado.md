# FUN-FIN-017 — Fluxo de Caixa Projetado
...
Migrar a funcionalidade **Fluxo de Caixa Projetado** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: cenário, período e premissas explícitos; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Fluxo de Caixa Projetado**.
- Rotas alvo declaradas: /app/financial/fluxo-projetado.
- Claim do filho: recurso.fluxo-projetado=visualizar.
...
- `web/src/app/(app)/financial/fluxo-projetado/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fluxo-projetado=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /financial/reports/cash-flow-forecast.
...
- [ ] Regra específica validada: cenário, período e premissas explícitos; AC-READ.
