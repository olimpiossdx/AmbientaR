...
Migrar a funcionalidade **Painel Financeiro** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: período, moeda e origem de cada KPI; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Painel Financeiro**.
- Rotas alvo declaradas: /app/financial/painel.
- Claim do filho: recurso.dashboard-financeiro=visualizar.
...
- `web/src/app/(app)/financial/painel/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.dashboard-financeiro=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /financial/dashboard.
...
- [ ] Regra específica validada: período, moeda e origem de cada KPI; AC-READ.
