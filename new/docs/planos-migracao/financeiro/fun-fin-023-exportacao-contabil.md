# FUN-FIN-023 — Exportação Contábil
...
Migrar a funcionalidade **Exportação Contábil** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: layout, período, checksum e reprocessamento idempotente; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Exportação Contábil**.
- Rotas alvo declaradas: /app/financial/export-contabil.
- Claim do filho: recurso.exportacao-contabil=exportar.
...
- `web/src/app/(app)/financial/export-contabil/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.exportacao-contabil=exportar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /accounting-exports, GET /jobs/{id}.
...
- [ ] Regra específica validada: layout, período, checksum e reprocessamento idempotente; AC-JOB.
