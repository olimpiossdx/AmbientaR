...
Migrar a funcionalidade **Clientes** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: CPF/CNPJ único, vínculo comercial e escopo; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Clientes**.
- Rotas alvo declaradas: /app/clients, /new, /$id/edit.
- Claim do filho: recurso.cliente-financeiro=*.
...
- `web/src/app/(app)/clients/[id]/edit/page.tsx`
- `web/src/app/(app)/clients/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.cliente-financeiro=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /financial/clients.
...
- [ ] Regra específica validada: CPF/CNPJ único, vínculo comercial e escopo; AC-CRUD.
