# FUN-FIN-006 — Curva ABC de Clientes
...
Migrar a funcionalidade **Curva ABC de Clientes** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: período, critério e totais reconciliáveis; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Curva ABC de Clientes**.
- Rotas alvo declaradas: /app/financial/abc-curve.
- Claim do filho: recurso.relatorio-financeiro=visualizar.
...
- `web/src/app/(app)/financial/abc-curve/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.relatorio-financeiro=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /financial/reports/abc-clients.
...
- [ ] Regra específica validada: período, critério e totais reconciliáveis; AC-READ.
