# FUN-FIN-025 — Tabela de Serviços
...
Migrar a funcionalidade **Tabela de Serviços** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: código único, preço decimal e vigência; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Tabela de Serviços**.
- Rotas alvo declaradas: /app/services, /new, /$id/edit.
- Claim do filho: recurso.servico=*.
...
- `web/src/app/(app)/services/[id]/edit/page.tsx`
- `web/src/app/(app)/services/new/page.tsx`
- `web/src/app/(app)/services/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.servico=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
- [ ] Regra específica validada: código único, preço decimal e vigência; AC-CRUD.
