...
Migrar a funcionalidade **NFe Eletrônica** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: destino cadastrado no API, sem parâmetro de URL livre; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **NFe Eletrônica**.
- Rotas alvo declaradas: /app/external?target=nfe.
- Claim do filho: recurso.nfe=acessar.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.nfe=acessar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/nfe.
...
- [ ] Regra específica validada: destino cadastrado no API, sem parâmetro de URL livre; AC-INT.
