# FUN-FIN-014 — Orçamentos e Propostas Comerciais
...
Migrar a funcionalidade **Orçamentos e Propostas Comerciais** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: aceite guarda autor/data/versão; AC-WORKFLOW, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Orçamentos e Propostas Comerciais**.
- Rotas alvo declaradas: /app/commercial-proposals, /new, /$id/edit.
- Claim do filho: recurso.proposta-comercial=*.
...
- `web/src/app/(app)/commercial-proposals/[id]/edit/page.tsx`
- `web/src/app/(app)/commercial-proposals/new/page.tsx`
- `web/src/app/(app)/commercial-proposals/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.proposta-comercial=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /commercial-proposals; ações send/accept/reject/expire.
...
- [ ] Regra específica validada: aceite guarda autor/data/versão; AC-WORKFLOW, AC-JOB.
