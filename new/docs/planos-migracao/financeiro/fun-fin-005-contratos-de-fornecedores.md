# FUN-FIN-005 — Contratos de Fornecedores
...
Migrar a funcionalidade **Contratos de Fornecedores** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fornecedor, centro de custo, vigência e anexos; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Contratos de Fornecedores**.
- Rotas alvo declaradas: /app/contracts-suppliers.
- Claim do filho: recurso.contrato-fornecedor=*.
...
- `web/src/app/(app)/contracts-suppliers/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.contrato-fornecedor=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /supplier-contracts.
...
- [ ] Regra específica validada: fornecedor, centro de custo, vigência e anexos; AC-CRUD, AC-FILE.
