...
Migrar a funcionalidade **Bens e Patrimônio** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: tombamento único, depreciação e baixa por ação; AC-CRUD, AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Bens e Patrimônio**.
- Rotas alvo declaradas: /app/financial/bens-patrimonio, /new, /$id/edit.
- Claim do filho: recurso.patrimonio=*.
...
- `web/src/app/(app)/financial/bens-patrimonio/[id]/edit/page.tsx`
- `web/src/app/(app)/financial/bens-patrimonio/new/page.tsx`
- `web/src/app/(app)/financial/bens-patrimonio/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.patrimonio=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
- [ ] Regra específica validada: tombamento único, depreciação e baixa por ação; AC-CRUD, AC-WORKFLOW.
