# FUN-FIN-018 — Conciliação Bancária
...
Migrar a funcionalidade **Conciliação Bancária** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: match idempotente, diferença e desfazer auditado; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Conciliação Bancária**.
- Rotas alvo declaradas: /app/financial/conciliacao.
- Claim do filho: recurso.conciliacao-bancaria=executar.
...
- `web/src/app/(app)/financial/conciliacao/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.conciliacao-bancaria=executar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /bank-reconciliation, POST /matches, DELETE /matches/{id}.
...
- [ ] Regra específica validada: match idempotente, diferença e desfazer auditado; AC-WORKFLOW.
