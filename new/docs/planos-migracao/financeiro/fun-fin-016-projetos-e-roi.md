# FUN-FIN-016 — Projetos e ROI

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Financeiro**  
Origem funcional: seção “Financeiro” do catálogo  
Dependência de acesso do pai: `modulo.financeiro=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Projetos e ROI** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: custos restritos por claim específica; ROI consistente; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Projetos e ROI**.
- Rotas alvo declaradas: /app/financial/projetos-roi, /$caseId.
- Claim do filho: recurso.projeto-roi=*.
...
- `web/src/app/(app)/financial/projetos-roi/[caseId]/page.tsx`
- `web/src/app/(app)/financial/projetos-roi/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Financeiro** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.projeto-roi=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /roi-cases; /time-entries; ações de aprovação/estorno.
...
- [ ] Regra específica validada: custos restritos por claim específica; ROI consistente; AC-WORKFLOW.
