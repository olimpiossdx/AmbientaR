...
Migrar a funcionalidade **Orçamentos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: numeração, itens, impostos, validade e PDF; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Orçamentos**.
- Rotas alvo declaradas: /app/proposals, /new, /$id/edit.
- Claim do filho: recurso.orcamento=*.
...
- `web/src/app/(app)/proposals/[id]/edit/page.tsx`
- `web/src/app/(app)/proposals/new/page.tsx`
- `web/src/app/(app)/proposals/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.orcamento=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /quotes; POST /quotes/{id}/exports.
...
- [ ] Regra específica validada: numeração, itens, impostos, validade e PDF; AC-CRUD, AC-JOB.
