...
Migrar a funcionalidade **Contratos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: versões, partes, vigência, aprovação/assinatura; AC-WORKFLOW, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Contratos**.
- Rotas alvo declaradas: /app/contracts, /new, /$id/edit.
- Claim do filho: recurso.contrato=*.
...
- `web/src/app/(app)/contracts/[id]/edit/page.tsx`
- `web/src/app/(app)/contracts/new/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.contrato=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /contracts; POST /contracts/{id}/actions/{action} com approve/sign/cancel.
...
- [ ] Regra específica validada: versões, partes, vigência, aprovação/assinatura; AC-WORKFLOW, AC-FILE.
