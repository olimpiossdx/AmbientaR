# FUN-FIN-011 — Lançamentos de Caixa
...
Migrar a funcionalidade **Lançamentos de Caixa** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: valor monetário decimal, competência, conta e estorno imutável; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Lançamentos de Caixa**.
- Rotas alvo declaradas: /app/cash-flow, /new, /$id/edit.
- Claim do filho: recurso.lancamento-caixa=*.
...
- `web/src/app/(app)/cash-flow/[id]/edit/page.tsx`
- `web/src/app/(app)/cash-flow/new/page.tsx`
- `web/src/app/(app)/cash-flow/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.lancamento-caixa=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /cash-transactions; ação reverse.
...
- [ ] Regra específica validada: valor monetário decimal, competência, conta e estorno imutável; AC-WORKFLOW.
