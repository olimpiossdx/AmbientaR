...
Migrar a funcionalidade **Orçamento Anual** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: versão por exercício e centro de custo; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Financeiro**.
- Filho/tela: **Orçamento Anual**.
- Rotas alvo declaradas: /app/financial/orcamento.
- Claim do filho: recurso.orcamento-anual=*.
...
- `web/src/app/(app)/financial/orcamento/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.orcamento-anual=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /annual-budgets; ação approve.
...
- [ ] Regra específica validada: versão por exercício e centro de custo; AC-WORKFLOW.
