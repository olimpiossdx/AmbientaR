# FUN-GEO-008 — Documentação técnica
...
Migrar a funcionalidade **Documentação técnica** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: tipo, projeto e versão; AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Documentação técnica**.
- Rotas alvo declaradas: /app/georeferenciamento/documentos.
- Claim do filho: proposta: recurso.georreferenciamento-documentacao-tecnica=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/documentos/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-documentacao-tecnica=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/documents; API-FILE.
...
- [ ] Regra específica validada: tipo, projeto e versão; AC-FILE.
