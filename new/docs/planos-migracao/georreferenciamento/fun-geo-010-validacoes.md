...
Migrar a funcionalidade **Validações** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: topologia, sobreposição e precisão; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Validações**.
- Rotas alvo declaradas: /app/georeferenciamento/validacoes.
- Claim do filho: proposta: recurso.georreferenciamento-validacoes=executar; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/validacoes/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-validacoes=executar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /georeferencing/validation-jobs.
...
- [ ] Regra específica validada: topologia, sobreposição e precisão; AC-JOB.
