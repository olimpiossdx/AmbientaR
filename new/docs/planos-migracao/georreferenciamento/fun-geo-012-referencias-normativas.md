# FUN-GEO-012 — Referências normativas
...
Migrar a funcionalidade **Referências normativas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: vigência e fonte; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Referências normativas**.
- Rotas alvo declaradas: /app/georeferenciamento/referencias.
- Claim do filho: proposta: recurso.georreferenciamento-referencias-normativas=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/referencias/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-referencias-normativas=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/references.
...
- [ ] Regra específica validada: vigência e fonte; AC-CRUD.
