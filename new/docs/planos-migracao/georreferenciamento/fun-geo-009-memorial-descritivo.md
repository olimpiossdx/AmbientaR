# FUN-GEO-009 — Memorial descritivo
...
Migrar a funcionalidade **Memorial descritivo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: fechamento, área, azimute e CRS; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Memorial descritivo**.
- Rotas alvo declaradas: /app/georeferenciamento/memorial-descritivo.
- Claim do filho: proposta: recurso.georreferenciamento-memorial-descritivo=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/memorial-descritivo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-memorial-descritivo=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: Contrato ainda não identificado.
...
- [ ] Regra específica validada: fechamento, área, azimute e CRS; AC-JOB.
