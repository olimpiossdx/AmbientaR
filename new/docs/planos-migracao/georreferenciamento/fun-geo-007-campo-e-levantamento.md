# FUN-GEO-007 — Campo e levantamento
...
Migrar a funcionalidade **Campo e levantamento** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: equipamento, precisão e pontos; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Campo e levantamento**.
- Rotas alvo declaradas: /app/georeferenciamento/campo.
- Claim do filho: proposta: recurso.georreferenciamento-campo-e-levantamento=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/campo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-campo-e-levantamento=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/surveys.
...
- [ ] Regra específica validada: equipamento, precisão e pontos; AC-CRUD, AC-FILE.
