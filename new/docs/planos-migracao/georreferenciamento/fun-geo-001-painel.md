...
Migrar a funcionalidade **Painel** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.
...
- Filho/tela: **Painel**.
- Rotas alvo declaradas: /app/georeferenciamento.
- Claim do filho: proposta: recurso.georreferenciamento-painel=visualizar; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-painel=visualizar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /georeferencing/dashboard.
