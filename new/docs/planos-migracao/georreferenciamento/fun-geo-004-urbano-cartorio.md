...
Migrar a funcionalidade **Urbano cartório** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: matrícula, quadra/lote e memorial; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Urbano cartório**.
- Rotas alvo declaradas: /app/georeferenciamento/urbano.
- Claim do filho: proposta: recurso.georreferenciamento-urbano-cartorio=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/urbano/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-urbano-cartorio=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/urban-projects.
...
- [ ] Regra específica validada: matrícula, quadra/lote e memorial; AC-CRUD, AC-JOB.
