# FUN-GEO-002 — Trâmites fundiários

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Georreferenciamento**  
Origem funcional: seção “Georreferenciamento” do catálogo  
Dependência de acesso do pai: `modulo.georreferenciamento=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Trâmites fundiários** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: estados, responsável e prazos; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Trâmites fundiários**.
- Rotas alvo declaradas: /app/georeferenciamento/processos, /$id.
- Claim do filho: proposta: recurso.georreferenciamento-tramites-fundiarios=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/processos/[id]/page.tsx`
- `web/src/app/(app)/georeferenciamento/processos/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Georreferenciamento** como pai.
...
- [ ] Implementar todas as variações de tela declaradas (2), preservando busca, filtros, paginação, seleção, ações, anexos e exportações existentes que forem confirmados no refinamento.
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-tramites-fundiarios=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/processes; ações.
...
- [ ] Regra específica validada: estados, responsável e prazos; AC-WORKFLOW.
