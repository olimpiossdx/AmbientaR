...
Migrar a funcionalidade **Rural SIGEF/INCRA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: datum, vértices, confrontantes e normas; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Rural SIGEF/INCRA**.
- Rotas alvo declaradas: /app/georeferenciamento/rural.
- Claim do filho: proposta: recurso.georreferenciamento-rural-sigef-incra=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/rural/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-rural-sigef-incra=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/rural-projects; validation jobs.
...
- [ ] Regra específica validada: datum, vértices, confrontantes e normas; AC-JOB.
