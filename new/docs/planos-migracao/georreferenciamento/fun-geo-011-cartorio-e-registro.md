# FUN-GEO-011 — Cartório e registro
...
Migrar a funcionalidade **Cartório e registro** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: protocolo e status; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Cartório e registro**.
- Rotas alvo declaradas: /app/georeferenciamento/registro.
- Claim do filho: proposta: recurso.georreferenciamento-cartorio-e-registro=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/registro/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-cartorio-e-registro=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/registrations; ações.
...
- [ ] Regra específica validada: protocolo e status; AC-WORKFLOW.
