...
Migrar a funcionalidade **Histórico CAR** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: snapshots imutáveis e comparação; AC-READ, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **Histórico CAR**.
- Rotas alvo declaradas: /app/georeferenciamento/historico-car.
- Claim do filho: proposta: recurso.georreferenciamento-historico-car=visualizar; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/historico-car/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-historico-car=visualizar; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /car-history; snapshot jobs.
...
- [ ] Regra específica validada: snapshots imutáveis e comparação; AC-READ, AC-JOB.
