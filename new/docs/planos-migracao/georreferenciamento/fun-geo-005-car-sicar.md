...
Migrar a funcionalidade **CAR/SICAR** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recibo, imóvel e geometria; AC-INT, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Georreferenciamento**.
- Filho/tela: **CAR/SICAR**.
- Rotas alvo declaradas: /app/georeferenciamento/ambiental.
- Claim do filho: proposta: recurso.georreferenciamento-car-sicar=*; validar no refinamento.
...
- `web/src/app/(app)/georeferenciamento/ambiental/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para proposta: recurso.georreferenciamento-car-sicar=*; validar no refinamento; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /georeferencing/car-projects; integration jobs.
...
- [ ] Regra específica validada: recibo, imóvel e geometria; AC-INT, AC-JOB.
