...
Migrar a funcionalidade **Empresas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: CNPJ, responsáveis e apenas uma empresa ativa quando aplicável; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Cadastro**.
- Filho/tela: **Empresas**.
- Rotas alvo declaradas: /app/responsible-company, /new, /$id/edit.
- Claim do filho: recurso.empresa-responsavel=*.
...
- `web/src/app/(app)/responsible-company/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/responsible-company/(.)new/page.tsx`
- `web/src/app/(app)/responsible-company/[id]/edit/page.tsx`
- `web/src/app/(app)/responsible-company/new/page.tsx`
- `web/src/app/(app)/responsible-company/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.empresa-responsavel=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /empresas-responsaveis.
...
- [ ] Regra específica validada: CNPJ, responsáveis e apenas uma empresa ativa quando aplicável; AC-CRUD.
