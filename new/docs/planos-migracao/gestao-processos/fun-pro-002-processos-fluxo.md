...
Migrar a funcionalidade **Processos / Fluxo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: kanban deriva estados válidos; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Gestão de Projetos e Processos**.
- Filho/tela: **Processos / Fluxo**.
- Rotas alvo declaradas: /app/gestao-processos/fluxo.
- Claim do filho: recurso.fluxo-processo=*.
...
- `web/src/app/(app)/gestao-processos/fluxo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fluxo-processo=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /processes; ações de transição.
...
- [ ] Regra específica validada: kanban deriva estados válidos; AC-WORKFLOW.
