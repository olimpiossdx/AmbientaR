# FUN-PRO-001 — Processos / Projetos

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Gestão de Projetos e Processos**  
...
Migrar a funcionalidade **Processos / Projetos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: escopo por carteira/titular; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Gestão de Projetos e Processos**.
- Filho/tela: **Processos / Projetos**.
- Rotas alvo declaradas: /app/gestao-processos/projetos, /$id.
- Claim do filho: recurso.projeto-processo=*.
...
- `web/src/app/(app)/gestao-processos/projetos/[id]/page.tsx`
- `web/src/app/(app)/gestao-processos/projetos/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Gestão de Projetos e Processos** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.projeto-processo=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /process-projects.
...
- [ ] Regra específica validada: escopo por carteira/titular; AC-CRUD.
