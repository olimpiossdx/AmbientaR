...
Migrar a funcionalidade **Processos / Tarefas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: participante, prazo e visibilidade; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Gestão de Projetos e Processos**.
- Filho/tela: **Processos / Tarefas**.
- Rotas alvo declaradas: /app/gestao-processos/tarefas.
- Claim do filho: recurso.tarefa=*.
...
- `web/src/app/(app)/gestao-processos/tarefas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.tarefa=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /tasks; ações assign/complete/reopen.
...
- [ ] Regra específica validada: participante, prazo e visibilidade; AC-WORKFLOW.
