# FUN-PRO-005 — Processos / Indicadores / Análise
...
Migrar a funcionalidade **Processos / Indicadores / Análise** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: séries e filtros reproduzíveis; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Gestão de Projetos e Processos**.
- Filho/tela: **Processos / Indicadores / Análise**.
- Rotas alvo declaradas: /app/gestao-processos/indicadores/analise.
- Claim do filho: recurso.indicador-processo=visualizar.
...
- `web/src/app/(app)/gestao-processos/indicadores/analise/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.indicador-processo=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /process-indicators/analysis.
...
- [ ] Regra específica validada: séries e filtros reproduzíveis; AC-READ.
