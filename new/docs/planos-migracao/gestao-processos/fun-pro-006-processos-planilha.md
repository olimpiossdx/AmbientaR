# FUN-PRO-006 — Processos / Planilha
...
Migrar a funcionalidade **Processos / Planilha** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: preview, validação por linha e importação idempotente; AC-JOB, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Gestão de Projetos e Processos**.
- Filho/tela: **Processos / Planilha**.
- Rotas alvo declaradas: /app/gestao-processos/planilha.
- Claim do filho: recurso.processo=importar.
...
- `web/src/app/(app)/gestao-processos/planilha/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.processo=importar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /process-import-jobs.
...
- [ ] Regra específica validada: preview, validação por linha e importação idempotente; AC-JOB, AC-FILE.
