...
Dependência de acesso do pai: `recurso.webmail=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Webmail** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.webmail=acessar; allowlist; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Webmail**.
- Filho/tela: **Webmail**.
- Rotas alvo declaradas: /app/external?target=webmail.
- Claim do filho: recurso.webmail=acessar; allowlist; AC-INT.
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Webmail** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.webmail=acessar; allowlist; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/webmail.
...
- [ ] Regra específica validada: recurso.webmail=acessar; allowlist; AC-INT.
