...
Migrar a funcionalidade **Sessão** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: refresh concorrente único, retorno à rota e limpeza completa no logout.

## Hierarquia e rotas

- Módulo/menu pai: **Acesso e sessão**.
- Filho/tela: **Sessão**.
- Rotas alvo declaradas: modal e /sessao-bloqueada.
- Claim do filho: usuário conhecido.
...
- nenhuma correspondência direta; verificar alias, card de hub ou fluxo incorporado
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Acesso e sessão** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para usuário conhecido; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /auth/refresh, POST /auth/relogin, POST /auth/logout.
...
- [ ] Regra específica validada: refresh concorrente único, retorno à rota e limpeza completa no logout.
