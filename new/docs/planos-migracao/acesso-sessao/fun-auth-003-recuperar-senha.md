...
Migrar a funcionalidade **Recuperar senha** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: resposta genérica, expiração, revogação e troca real da senha.

## Hierarquia e rotas

- Módulo/menu pai: **Acesso e sessão**.
- Filho/tela: **Recuperar senha**.
- Rotas alvo declaradas: /forgot-password.
- Claim do filho: pública; token único e expirável.
...
Contrato alvo: POST /auth/password-reset, POST /auth/password-reset/confirm.
...
- [ ] Regra específica validada: resposta genérica, expiração, revogação e troca real da senha.
