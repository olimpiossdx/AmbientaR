...
Migrar a funcionalidade **Estado offline** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: distingue API indisponível de navegador offline; retry sem loop.

## Hierarquia e rotas

- Módulo/menu pai: **Acesso e sessão**.
- Filho/tela: **Estado offline**.
- Rotas alvo declaradas: /offline.
...
Contrato alvo: GET /health/ready.

Situação encontrada: **Parcial/existente — validar aderência ao contrato alvo**.

- `../ambientaR-api/src/health/health.controller.ts`
...
- [ ] Regra específica validada: distingue API indisponível de navegador offline; retry sem loop.
