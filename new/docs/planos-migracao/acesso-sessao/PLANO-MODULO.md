# Plano do módulo — Acesso e sessão

Status: análise inicial  
Diretório: `acesso-sessao`  
Dependência de acesso do pai: `rota pública ou sessão autenticada`  
Filhos mapeados: **6**

## Objetivo

Migrar o menu **Acesso e sessão** como módulo coeso do `new`, mantendo cada submenu/tela como entrega vertical separada. O módulo pai é responsável por registro, navegação e contexto compartilhado; regras de negócio permanecem nos domínios dos filhos e no `ambientaR-api`.

## Filhos do módulo

| Plano da funcionalidade | Rotas alvo | Contrato alvo |
| --- | --- | --- |
| [FUN-AUTH-001 — Login](fun-auth-001-login.md) | /login | POST /auth/login, GET /auth/session |
| [FUN-AUTH-002 — Cadastro](fun-auth-002-cadastro.md) | /register | POST /auth/register, POST /auth/register/document-preview |
| [FUN-AUTH-003 — Recuperar senha](fun-auth-003-recuperar-senha.md) | /forgot-password | POST /auth/password-reset, POST /auth/password-reset/confirm |
| [FUN-AUTH-004 — Política de privacidade](fun-auth-004-politica-de-privacidade.md) | /politica-privacidade | GET /public/legal/privacy-policy |
| [FUN-AUTH-005 — Estado offline](fun-auth-005-estado-offline.md) | /offline | GET /health/ready |
| [FUN-AUTH-006 — Sessão](fun-auth-006-sessao.md) | modal e /sessao-bloqueada | POST /auth/refresh, POST /auth/relogin, POST /auth/logout |
