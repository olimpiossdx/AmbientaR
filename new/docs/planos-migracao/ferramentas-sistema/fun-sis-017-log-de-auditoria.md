...
Migrar a funcionalidade **Log de auditoria** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.auditoria=visualizar; somente leitura, paginação e filtros.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Log de auditoria**.
- Rotas alvo declaradas: /app/audit-log.
- Claim do filho: recurso.auditoria=visualizar; somente leitura, paginação e filtros.
...
- `web/src/app/(app)/audit-log/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.auditoria=visualizar; somente leitura, paginação e filtros; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /audit-events.
...
- [ ] Regra específica validada: recurso.auditoria=visualizar; somente leitura, paginação e filtros.
