# FUN-SIS-018 — Canais WhatsApp/Instagram
...
Migrar a funcionalidade **Canais WhatsApp/Instagram** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.canal-comunicacao=*; segredos cifrados; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Canais WhatsApp/Instagram**.
- Rotas alvo declaradas: /app/canais.
- Claim do filho: recurso.canal-comunicacao=*; segredos cifrados; AC-INT.
...
- `web/src/app/(app)/canais/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.canal-comunicacao=*; segredos cifrados; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /communication-channels; OAuth/webhooks.
...
- [ ] Regra específica validada: recurso.canal-comunicacao=*; segredos cifrados; AC-INT.
