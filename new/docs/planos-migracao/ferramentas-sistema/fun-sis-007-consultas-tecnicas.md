...
Migrar a funcionalidade **Consultas técnicas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.consulta-tecnica=*; vínculo opcional com laudo; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Consultas técnicas**.
- Rotas alvo declaradas: /app/consultas, /new, /$id, /$id/edit.
- Claim do filho: recurso.consulta-tecnica=*; vínculo opcional com laudo; AC-WORKFLOW.
...
- `web/src/app/(app)/consultas/[id]/edit/page.tsx`
- `web/src/app/(app)/consultas/[id]/page.tsx`
- `web/src/app/(app)/consultas/new/page.tsx`
- `web/src/app/(app)/consultas/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Ferramentas do Sistema** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.consulta-tecnica=*; vínculo opcional com laudo; AC-WORKFLOW; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /technical-consultations; ações.
...
- [ ] Regra específica validada: recurso.consulta-tecnica=*; vínculo opcional com laudo; AC-WORKFLOW.
