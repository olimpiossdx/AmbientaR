...
Migrar a funcionalidade **Laboratório RAG** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.laboratorio-rag=executar; isolamento de índice.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Laboratório RAG**.
- Rotas alvo declaradas: /app/ai-lab/rag.
- Claim do filho: recurso.laboratorio-rag=executar; isolamento de índice.
...
- `web/src/app/(app)/ai-lab/rag/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.laboratorio-rag=executar; isolamento de índice; não usar `role`, `isAdmin` ou Firebase como autorização.
...
- [ ] Regra específica validada: recurso.laboratorio-rag=executar; isolamento de índice.
