# FUN-SIS-016 — Explorador de arquivos
...
Migrar a funcionalidade **Explorador de arquivos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.arquivo com visualizar/excluir; nunca aceita path físico.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Explorador de arquivos**.
- Rotas alvo declaradas: /app/settings/files.
- Claim do filho: recurso.arquivo com visualizar/excluir; nunca aceita path físico.
...
- `web/src/app/(app)/settings/files/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.arquivo com visualizar/excluir; nunca aceita path físico; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /files; API-FILE.
...
- [ ] Regra específica validada: recurso.arquivo com visualizar/excluir; nunca aceita path físico.
