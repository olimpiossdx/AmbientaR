...
Migrar a funcionalidade **Ferramentas MCP** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.ferramenta-mcp=*; allowlist e segredo write-only.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Ferramentas MCP**.
- Rotas alvo declaradas: /app/ai-lab/mcp.
- Claim do filho: recurso.ferramenta-mcp=*; allowlist e segredo write-only.
...
- `web/src/app/(app)/ai-lab/mcp/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.ferramenta-mcp=*; allowlist e segredo write-only; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /mcp-tools; test action.
...
- [ ] Regra específica validada: recurso.ferramenta-mcp=*; allowlist e segredo write-only.
