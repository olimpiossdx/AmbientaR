# FUN-SIS-019 — App/Coleta de campo
...
Migrar a funcionalidade **App/Coleta de campo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.app-campo=acessar; escopo mínimo e conflitos explícitos.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **App/Coleta de campo**.
- Rotas alvo declaradas: /app/app-campo.
- Claim do filho: recurso.app-campo=acessar; escopo mínimo e conflitos explícitos.
...
- `web/src/app/(app)/app-campo/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.app-campo=acessar; escopo mínimo e conflitos explícitos; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /field-app/bootstrap; sync commands se offline aprovado.
...
- [ ] Regra específica validada: recurso.app-campo=acessar; escopo mínimo e conflitos explícitos.
