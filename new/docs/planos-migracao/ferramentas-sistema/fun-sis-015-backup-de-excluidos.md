# FUN-SIS-015 — Backup de excluídos
...
Migrar a funcionalidade **Backup de excluídos** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.registro-excluido com visualizar/restaurar; restauração auditada.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Backup de excluídos**.
- Rotas alvo declaradas: /app/settings/deleted-backups.
- Claim do filho: recurso.registro-excluido com visualizar/restaurar; restauração auditada.
...
- `web/src/app/(app)/settings/deleted-backups/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.registro-excluido com visualizar/restaurar; restauração auditada; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /deleted-records; ação restore.
...
- [ ] Regra específica validada: recurso.registro-excluido com visualizar/restaurar; restauração auditada.
