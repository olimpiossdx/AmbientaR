# FUN-SIS-011 — Integração OneDrive
...
Migrar a funcionalidade **Integração OneDrive** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.integracao-onedrive=configurar; token cifrado; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Integração OneDrive**.
- Rotas alvo declaradas: /app/settings/onedrive-integration.
- Claim do filho: recurso.integracao-onedrive=configurar; token cifrado; AC-INT.
...
- `web/src/app/(app)/settings/onedrive-integration/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.integracao-onedrive=configurar; token cifrado; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: OAuth begin/callback/status/disconnect no API.
...
- [ ] Regra específica validada: recurso.integracao-onedrive=configurar; token cifrado; AC-INT.
