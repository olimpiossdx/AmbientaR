# FUN-SIS-001 — Informações da empresa
...
Migrar a funcionalidade **Informações da empresa** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.configuracao-empresa=configurar; versão/auditoria.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Informações da empresa**.
- Rotas alvo declaradas: /app/settings/company.
- Claim do filho: recurso.configuracao-empresa=configurar; versão/auditoria.
...
- `web/src/app/(app)/settings/company/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.configuracao-empresa=configurar; versão/auditoria; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /organization/settings.
...
- [ ] Regra específica validada: recurso.configuracao-empresa=configurar; versão/auditoria.
