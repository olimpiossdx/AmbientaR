...
Migrar a funcionalidade **MCP + RAG** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.configuracao-ia=configurar; segredos write-only.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **MCP + RAG**.
- Rotas alvo declaradas: /app/configuracoes/mcp-rag.
- Claim do filho: recurso.configuracao-ia=configurar; segredos write-only.
...
- `web/src/app/(app)/configuracoes/mcp-rag/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.configuracao-ia=configurar; segredos write-only; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /ai-platform/settings; sync jobs.
...
- [ ] Regra específica validada: recurso.configuracao-ia=configurar; segredos write-only.
