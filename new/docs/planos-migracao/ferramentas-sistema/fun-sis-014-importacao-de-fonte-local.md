# FUN-SIS-014 — Importação de fonte local
...
Migrar a funcionalidade **Importação de fonte local** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.fonte-conhecimento=importar; sem dependência de caminho do legado; AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Importação de fonte local**.
- Rotas alvo declaradas: /app/settings/ai-local-source.
- Claim do filho: recurso.fonte-conhecimento=importar; sem dependência de caminho do legado; AC-JOB.
...
- `web/src/app/(app)/settings/ai-local-source/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fonte-conhecimento=importar; sem dependência de caminho do legado; AC-JOB; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: POST /knowledge-import-jobs.
...
- [ ] Regra específica validada: recurso.fonte-conhecimento=importar; sem dependência de caminho do legado; AC-JOB.
