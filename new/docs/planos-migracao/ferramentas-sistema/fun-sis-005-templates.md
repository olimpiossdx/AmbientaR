...
Migrar a funcionalidade **Templates** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.template-documento=*; placeholders/schema versionados.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Templates**.
- Rotas alvo declaradas: /app/settings/templates, /rca.
- Claim do filho: recurso.template-documento=*; placeholders/schema versionados.
...
- `web/src/app/(app)/settings/templates/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.template-documento=*; placeholders/schema versionados; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /document-templates; API-FILE.
...
- [ ] Regra específica validada: recurso.template-documento=*; placeholders/schema versionados.
