...
Migrar a funcionalidade **Identidade visual** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.identidade-visual=configurar; AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Identidade visual**.
- Rotas alvo declaradas: /app/settings#identidade-visual.
- Claim do filho: recurso.identidade-visual=configurar; AC-FILE.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.identidade-visual=configurar; AC-FILE; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /organization/branding, API-FILE.
...
- [ ] Regra específica validada: recurso.identidade-visual=configurar; AC-FILE.
