# FUN-SIS-010 — Biblioteca IA OneDrive
...
Migrar a funcionalidade **Biblioteca IA OneDrive** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.biblioteca-nuvem com visualizar/sincronizar; AC-INT, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Biblioteca IA OneDrive**.
- Rotas alvo declaradas: /app/ai-lab/cloud-library.
- Claim do filho: recurso.biblioteca-nuvem com visualizar/sincronizar; AC-INT, AC-JOB.
...
- `web/src/app/(app)/ai-lab/cloud-library/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.biblioteca-nuvem com visualizar/sincronizar; AC-INT, AC-JOB; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /cloud-library; sync jobs.
...
- [ ] Regra específica validada: recurso.biblioteca-nuvem com visualizar/sincronizar; AC-INT, AC-JOB.
