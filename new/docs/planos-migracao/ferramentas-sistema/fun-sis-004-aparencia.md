...
Migrar a funcionalidade **Aparência** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.preferencia-propria=editar; sempre escopo próprio.

## Hierarquia e rotas

- Módulo/menu pai: **Ferramentas do Sistema**.
- Filho/tela: **Aparência**.
- Rotas alvo declaradas: /app/settings/appearance.
- Claim do filho: recurso.preferencia-propria=editar; sempre escopo próprio.
...
- `web/src/app/(app)/settings/appearance/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.preferencia-propria=editar; sempre escopo próprio; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /users/me/preferences.
...
- [ ] Regra específica validada: recurso.preferencia-propria=editar; sempre escopo próprio.
