# FUN-CRM-009 — Vendas e propostas
...
Migrar a funcionalidade **Vendas e propostas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.proposta-comercial=visualizar; é atalho, não API duplicada.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Vendas e propostas**.
- Rotas alvo declaradas: /app/crm/proposals.
- Claim do filho: recurso.proposta-comercial=visualizar; é atalho, não API duplicada.
...
- `web/src/app/(app)/crm/proposals/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.proposta-comercial=visualizar; é atalho, não API duplicada; não usar `role`, `isAdmin` ou Firebase como autorização.
...
- [ ] Regra específica validada: recurso.proposta-comercial=visualizar; é atalho, não API duplicada.
