# FUN-CRM-004 — Gestão de clientes
...
Migrar a funcionalidade **Gestão de clientes** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-cliente=*; deduplicação e ownership; AC-CRUD.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Gestão de clientes**.
- Rotas alvo declaradas: /app/crm/clients.
- Claim do filho: recurso.crm-cliente=*; deduplicação e ownership; AC-CRUD.
...
- `web/src/app/(app)/crm/clients/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-cliente=*; deduplicação e ownership; AC-CRUD; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /crm/clients.
...
- [ ] Regra específica validada: recurso.crm-cliente=*; deduplicação e ownership; AC-CRUD.
