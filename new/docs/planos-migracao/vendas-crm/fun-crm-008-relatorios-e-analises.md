# FUN-CRM-008 — Relatórios e análises
...
Migrar a funcionalidade **Relatórios e análises** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-relatorio=visualizar; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Relatórios e análises**.
- Rotas alvo declaradas: /app/crm/reports.
- Claim do filho: recurso.crm-relatorio=visualizar; AC-READ.
...
- `web/src/app/(app)/crm/reports/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-relatorio=visualizar; AC-READ; não usar `role`, `isAdmin` ou Firebase como autorização.
...
- [ ] Regra específica validada: recurso.crm-relatorio=visualizar; AC-READ.
