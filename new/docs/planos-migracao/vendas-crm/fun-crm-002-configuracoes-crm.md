...
Migrar a funcionalidade **Configurações CRM** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-configuracao=configurar; versão/auditoria.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Configurações CRM**.
- Rotas alvo declaradas: /app/crm/settings.
- Claim do filho: recurso.crm-configuracao=configurar; versão/auditoria.
...
- `web/src/app/(app)/crm/settings/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-configuracao=configurar; versão/auditoria; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET/PATCH /crm/settings.
...
- [ ] Regra específica validada: recurso.crm-configuracao=configurar; versão/auditoria.
