...
Migrar a funcionalidade **Mídias sociais** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.midia-social=*; AC-WORKFLOW, AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Mídias sociais**.
- Rotas alvo declaradas: /app/social-media.
- Claim do filho: recurso.midia-social=*; AC-WORKFLOW, AC-INT.
...
- `web/src/app/(app)/social-media/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.midia-social=*; AC-WORKFLOW, AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /social-media/posts; ações schedule/publish.
...
- [ ] Regra específica validada: recurso.midia-social=*; AC-WORKFLOW, AC-INT.
