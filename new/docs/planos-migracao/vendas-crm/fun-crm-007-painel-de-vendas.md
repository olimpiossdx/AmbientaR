...
Migrar a funcionalidade **Painel de vendas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-dashboard=visualizar; AC-READ.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Painel de vendas**.
- Rotas alvo declaradas: /app/crm.
- Claim do filho: recurso.crm-dashboard=visualizar; AC-READ.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-dashboard=visualizar; AC-READ; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /crm/dashboard.
...
- [ ] Regra específica validada: recurso.crm-dashboard=visualizar; AC-READ.
