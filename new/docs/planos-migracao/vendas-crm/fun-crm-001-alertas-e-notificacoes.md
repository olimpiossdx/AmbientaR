# FUN-CRM-001 — Alertas e notificações
...
Migrar a funcionalidade **Alertas e notificações** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-alerta=*; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Alertas e notificações**.
- Rotas alvo declaradas: /app/crm/alerts.
- Claim do filho: recurso.crm-alerta=*; AC-WORKFLOW.
...
- `web/src/app/(app)/crm/alerts/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-alerta=*; AC-WORKFLOW; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /crm/alerts; ações read/dismiss.
...
- [ ] Regra específica validada: recurso.crm-alerta=*; AC-WORKFLOW.
