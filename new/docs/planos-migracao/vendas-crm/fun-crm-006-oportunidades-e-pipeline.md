# FUN-CRM-006 — Oportunidades e pipeline

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Vendas & CRM**  
Origem funcional: seção “Vendas e CRM” do catálogo  
Dependência de acesso do pai: `modulo.crm=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Oportunidades e pipeline** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.crm-oportunidade=*; estágio validado; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Vendas & CRM**.
- Filho/tela: **Oportunidades e pipeline**.
- Rotas alvo declaradas: /app/crm/opportunities, /new, /$id/edit.
- Claim do filho: recurso.crm-oportunidade=*; estágio validado; AC-WORKFLOW.
...
- `web/src/app/(app)/crm/opportunities/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Vendas & CRM** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.crm-oportunidade=*; estágio validado; AC-WORKFLOW; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /crm/opportunities; ação move-stage.
...
- [ ] Regra específica validada: recurso.crm-oportunidade=*; estágio validado; AC-WORKFLOW.
