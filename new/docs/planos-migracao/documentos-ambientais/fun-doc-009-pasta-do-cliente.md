# FUN-DOC-009 — Pasta do cliente

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Documentos Ambientais**  
Origem funcional: seção “Documentos Ambientais” do catálogo  
Dependência de acesso do pai: `modulo.documentos-ambientais=acessar`

## Objetivo e escopo

Migrar a funcionalidade **Pasta do cliente** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: árvore virtual por metadados, não path livre; AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Pasta do cliente**.
- Rotas alvo declaradas: /app/documentos-ambientais/pasta-cliente.
- Claim do filho: recurso.pasta-cliente com visualizar/enviar/excluir.
...
- `web/src/app/(app)/documentos-ambientais/pasta-cliente/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Documentos Ambientais** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.pasta-cliente com visualizar/enviar/excluir; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /client-folders/{clientId}, API-FILE.
...
- [ ] Regra específica validada: árvore virtual por metadados, não path livre; AC-FILE.
