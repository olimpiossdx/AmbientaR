# FUN-DOC-013 — Usos Insignificantes
...
Migrar a funcionalidade **Usos Insignificantes** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: enquadramento, vazão, validade e vínculo; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Usos Insignificantes**.
- Rotas alvo declaradas: /app/usos-insignificantes.
- Claim do filho: recurso.uso-insignificante=*.
...
- `web/src/app/(app)/usos-insignificantes/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.uso-insignificante=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /insignificant-water-uses.
...
- [ ] Regra específica validada: enquadramento, vazão, validade e vínculo; AC-CRUD, AC-FILE.
