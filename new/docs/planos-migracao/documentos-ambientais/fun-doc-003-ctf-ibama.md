...
Migrar a funcionalidade **CTF/IBAMA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: validade, titular e certificado; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **CTF/IBAMA**.
- Rotas alvo declaradas: /app/ctf-ibama.
- Claim do filho: recurso.ctf-ibama=*.
...
- `web/src/app/(app)/ctf-ibama/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.ctf-ibama=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /ctf-ibama-records.
...
- [ ] Regra específica validada: validade, titular e certificado; AC-CRUD, AC-FILE.
