...
Migrar a funcionalidade **MTR Declaração** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: período, manifesto, integração e deduplicação; AC-CRUD, AC-JOB, AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **MTR Declaração**.
- Rotas alvo declaradas: /app/mtr-declaracao.
- Claim do filho: recurso.mtr-declaracao=*.
...
- `web/src/app/(app)/mtr-declaracao/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.mtr-declaracao=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /mtr-declarations; POST /sync-jobs.
...
- [ ] Regra específica validada: período, manifesto, integração e deduplicação; AC-CRUD, AC-JOB, AC-INT.
