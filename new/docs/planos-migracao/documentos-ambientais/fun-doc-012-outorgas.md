...
Migrar a funcionalidade **Outorgas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: ponto, finalidade, vazão, portaria e validade; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Outorgas**.
- Rotas alvo declaradas: /app/outorgas, /new, /$id/edit.
- Claim do filho: recurso.outorga=*.
...
- `web/src/app/(app)/outorgas/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/outorgas/(.)new/page.tsx`
- `web/src/app/(app)/outorgas/[id]/edit/page.tsx`
- `web/src/app/(app)/outorgas/new/page.tsx`
- `web/src/app/(app)/outorgas/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.outorga=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /water-grants.
...
- [ ] Regra específica validada: ponto, finalidade, vazão, portaria e validade; AC-CRUD, AC-FILE.
