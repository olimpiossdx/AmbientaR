...
Migrar a funcionalidade **Licenças** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: número, órgão, modalidade, validade e alertas; AC-WORKFLOW, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Licenças**.
- Rotas alvo declaradas: /app/licenses, /new, /$id/edit.
- Claim do filho: recurso.licenca=*.
...
- `web/src/app/(app)/licenses/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/licenses/(.)new/page.tsx`
- `web/src/app/(app)/licenses/[id]/edit/page.tsx`
- `web/src/app/(app)/licenses/new/page.tsx`
- `web/src/app/(app)/licenses/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.licenca=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /licenses; ações de status.
...
- [ ] Regra específica validada: número, órgão, modalidade, validade e alertas; AC-WORKFLOW, AC-FILE.
