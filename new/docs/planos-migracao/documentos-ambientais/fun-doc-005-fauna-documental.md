...
Migrar a funcionalidade **Fauna documental** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: tipo, vigência, projeto e anexo; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Fauna documental**.
- Rotas alvo declaradas: /app/fauna.
- Claim do filho: recurso.fauna-documento=*.
...
- `web/src/app/(app)/fauna/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.fauna-documento=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /fauna-documents.
...
- [ ] Regra específica validada: tipo, vigência, projeto e anexo; AC-CRUD, AC-FILE.
