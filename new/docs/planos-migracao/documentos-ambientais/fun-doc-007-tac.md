...
Migrar a funcionalidade **TAC** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: partes, obrigações, prazo e evidências; AC-WORKFLOW, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **TAC**.
- Rotas alvo declaradas: /app/tacs, /new, /$id/edit.
- Claim do filho: recurso.tac=*.
...
- `web/src/app/(app)/tacs/(.)[id]/edit/page.tsx`
- `web/src/app/(app)/tacs/(.)new/page.tsx`
- `web/src/app/(app)/tacs/[id]/edit/page.tsx`
- `web/src/app/(app)/tacs/new/page.tsx`
- `web/src/app/(app)/tacs/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.tac=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /tacs; compromissos.
...
- [ ] Regra específica validada: partes, obrigações, prazo e evidências; AC-WORKFLOW, AC-FILE.
