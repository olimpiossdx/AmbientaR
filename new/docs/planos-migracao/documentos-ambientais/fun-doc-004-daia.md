...
Migrar a funcionalidade **DAIA** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: área, modalidade, validade e empreendimento; AC-CRUD, AC-FILE.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **DAIA**.
- Rotas alvo declaradas: /app/intervencoes.
- Claim do filho: recurso.daia=*.
...
- `web/src/app/(app)/intervencoes/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.daia=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /environmental-interventions.
...
- [ ] Regra específica validada: área, modalidade, validade e empreendimento; AC-CRUD, AC-FILE.
