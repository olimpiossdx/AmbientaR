# FUN-DOC-010 — Monitoramento manual
...
Migrar a funcionalidade **Monitoramento manual** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: unidade, data, ponto e limites; AC-CRUD, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Monitoramento manual**.
- Rotas alvo declaradas: /app/monitoring/manual.
- Claim do filho: recurso.monitoramento-outorga=*.
...
- `web/src/app/(app)/monitoring/manual/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.monitoramento-outorga=*; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: CRUD /water-monitoring/manual-readings; export.
...
- [ ] Regra específica validada: unidade, data, ponto e limites; AC-CRUD, AC-JOB.
