...
Migrar a funcionalidade **Telemetria** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: série temporal paginada/agregada e gaps explícitos; AC-READ, AC-JOB.

## Hierarquia e rotas

- Módulo/menu pai: **Documentos Ambientais**.
- Filho/tela: **Telemetria**.
- Rotas alvo declaradas: /app/monitoring/telemetric.
- Claim do filho: recurso.telemetria com visualizar/exportar.
...
- `web/src/app/(app)/monitoring/telemetric/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.telemetria com visualizar/exportar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /water-monitoring/telemetry; ingestão protegida separada.
...
- [ ] Regra específica validada: série temporal paginada/agregada e gaps explícitos; AC-READ, AC-JOB.
