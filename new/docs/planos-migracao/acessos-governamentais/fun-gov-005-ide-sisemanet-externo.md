# FUN-GOV-005 — IDE-SisemaNet externo
...
Migrar a funcionalidade **IDE-SisemaNet externo** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.acesso-governo-ide=acessar; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Acessos Governamentais**.
- Filho/tela: **IDE-SisemaNet externo**.
- Rotas alvo declaradas: /app/external?target=gov-ide.
- Claim do filho: recurso.acesso-governo-ide=acessar; AC-INT.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.acesso-governo-ide=acessar; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/gov-ide.
...
- [ ] Regra específica validada: recurso.acesso-governo-ide=acessar; AC-INT.
