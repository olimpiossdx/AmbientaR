# FUN-GOV-001 — Consulta Intervenção Ambiental

Status: análise inicial; implementação não iniciada por este plano  
Módulo pai: **Acessos Governamentais**  
...
Migrar a funcionalidade **Consulta Intervenção Ambiental** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.acesso-governo-intervencao=acessar; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Acessos Governamentais**.
- Filho/tela: **Consulta Intervenção Ambiental**.
- Rotas alvo declaradas: /app/external?target=gov-intervencao.
- Claim do filho: recurso.acesso-governo-intervencao=acessar; AC-INT.
...
- `web/src/app/(app)/external/page.tsx`
...
- [ ] Registrar módulo, rotas lazy, item filho e breadcrumb no registro central, mantendo **Acessos Governamentais** como pai.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.acesso-governo-intervencao=acessar; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/gov-intervencao.
...
- [ ] Regra específica validada: recurso.acesso-governo-intervencao=acessar; AC-INT.
