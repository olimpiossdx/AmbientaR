...
Migrar a funcionalidade **SEI-MG** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.acesso-governo-sei-mg=acessar; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Acessos Governamentais**.
- Filho/tela: **SEI-MG**.
- Rotas alvo declaradas: /app/external?target=gov-sei-mg.
- Claim do filho: recurso.acesso-governo-sei-mg=acessar; AC-INT.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.acesso-governo-sei-mg=acessar; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/gov-sei-mg.
...
- [ ] Regra específica validada: recurso.acesso-governo-sei-mg=acessar; AC-INT.
