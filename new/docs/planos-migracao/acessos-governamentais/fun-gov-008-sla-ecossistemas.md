...
Migrar a funcionalidade **SLA Ecossistemas** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.acesso-governo-sla=acessar; AC-INT.

## Hierarquia e rotas

- Módulo/menu pai: **Acessos Governamentais**.
- Filho/tela: **SLA Ecossistemas**.
- Rotas alvo declaradas: /app/external?target=gov-sla.
- Claim do filho: recurso.acesso-governo-sla=acessar; AC-INT.
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.acesso-governo-sla=acessar; AC-INT; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /external-targets/gov-sla.
...
- [ ] Regra específica validada: recurso.acesso-governo-sla=acessar; AC-INT.
