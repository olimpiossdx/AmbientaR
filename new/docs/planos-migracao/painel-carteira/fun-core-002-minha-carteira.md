...
Migrar a funcionalidade **Minha Carteira** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: titular, representante ou consultor só recebe vínculos aprovados; AC-READ, AC-FRONT.

## Hierarquia e rotas

- Módulo/menu pai: **Painel e Carteira**.
- Filho/tela: **Minha Carteira**.
- Rotas alvo declaradas: /app/carteira, /app/carteira/$clientId.
- Claim do filho: recurso.carteira=visualizar.
...
- `web/src/app/(app)/carteira/[clientId]/page.tsx`
- `web/src/app/(app)/carteira/page.tsx`
...
- [ ] Aplicar guard antes do carregamento e guard de ação para recurso.carteira=visualizar; não usar `role`, `isAdmin` ou Firebase como autorização.
...
Contrato alvo: GET /carteira, GET /carteira/{clientId}.
...
- [ ] Regra específica validada: titular, representante ou consultor só recebe vínculos aprovados; AC-READ, AC-FRONT.
