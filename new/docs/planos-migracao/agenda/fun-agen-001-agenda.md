...
Status: frontend parcial; bloqueado pela ausência do domínio e endpoints na API  
Módulo pai: **Agenda**  
Origem funcional: seção “Ofícios, governo e ferramentas do sistema” do catálogo  
Dependência de acesso do pai: `recurso.agenda=*`

## Objetivo e escopo

Migrar a funcionalidade **Agenda** do `web` para o `new`, sem dependência de Firebase nem das rotas Next.js do legado. O corte deve ser vertical: navegação, telas, serviço HTTP, contrato, autorização, persistência, testes e observabilidade entram na mesma entrega.

Regra funcional levantada: recurso.agenda=*; eventos próprios/públicos/organização escopados; AC-WORKFLOW.

## Hierarquia e rotas

- Módulo/menu pai: **Agenda**.
- Filho/tela: **Agenda**.
- Rotas alvo declaradas: /app/calendar.
- Claim do filho: recurso.agenda=*; eventos próprios/públicos/organização escopados; AC-WORKFLOW.
...
- [x] Registrar módulo, rota e item de navegação no registro modular.
- [x] Criar tipos, validação de entrada e service próprio usando exclusivamente o cliente HTTP central.
- [x] Implementar agenda responsiva com calendário, busca, filtros, paginação, CRUD, convite e resposta.
- [x] Aplicar guard de rota e claims independentes de visualizar, criar, editar, excluir, convidar e responder.
...
Contrato alvo: CRUD /calendar-events; ações invite/respond.
...
- [ ] Regra específica validada: recurso.agenda=*; eventos próprios/públicos/organização escopados; AC-WORKFLOW.
...
- Implementar no `ambientaR-api` o domínio, persistência e endpoints `/calendar-events`, incluindo `invite` e `respond`.
- Definir e testar o escopo de eventos próprios, públicos e da organização antes de homologar a tela.
