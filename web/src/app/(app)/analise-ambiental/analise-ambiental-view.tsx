...
function formatAnalysisFetchError(error: unknown, kind: "factual" | "ia"): string {
  ...
  if (/failed to fetch|networkerror|load failed/i.test(msg)) {
    if (kind === "factual") {
      return "Sem resposta do servidor (an├ílise factual demora 1ÔÇô3 min com 37 camadas). Confirme que npm run dev est├í activo em http://localhost:9002 e tente de novo.";
    }
    return "Sem resposta do servidor. Confirme que npm run dev est├í activo em http://localhost:9002 e tente de novo.";
  }
  return msg || (kind === "factual"
    ? "N├úo foi poss├¡vel consultar as camadas IDE-Sisema."
...
export function AnaliseAmbientalView() {
  ...
  const buildFactsPrompt = React.useCallback(() => {
    ...
    const facts = analysisResult.factualData
      .map(
        (item, idx) =>
          `${idx + 1}. Camada: ${item.camada} | Fonte: ${item.fonte} | M├®todo: ${item.metodo} | Resultado: ${item.resultado}`,
      )
      .join("\n");
    return [
      "Use estes dados geoespaciais factuais para cruzamento e recomenda├º├Áes:",
      facts,
      "",
      "Preciso de parecer t├®cnico com riscos regulat├│rios, pend├¬ncias e pr├│ximos passos para regulariza├º├úo em MG.",
    ].join("\n");
  }, [analysisResult]);
  ...
  const saveWaveASnapshot = React.useCallback(
    async (
      ...
      try {
        ...
        toast({
          variant: "destructive",
          title: "An├ílise n├úo foi salva",
          description:
            "O relat├│rio foi gerado, mas n├úo gravou no Firestore. Verifique regras (deploy:rules) ou permiss├Áes.",
        });
        return null;
      }
    },
  ...
  const handleStartWaveA = async () => {
    ...
    if (!input || !waveInput) {
      toast({
        ...
        description: needsLocalization(inputMode)
          ? "Localize e confirme o im├│vel antes de consultar as camadas."
          : "Preencha CAR, coordenadas, pol├¡gono, KML ou SHP para iniciar a an├ílise.",
      });
      return;
    }
    if (
      ...
      toast({
        variant: "destructive",
        title: "Confirme o im├│vel",
        description: "Revise o card SICAR e clique em Confirmar im├│vel.",
      });
      return;
    }
    if (
      ...
      toast({
        title: "Coordenada com buffer m├¡nimo",
        description:
          "Para relat├│rio Onda A com % confi├ível, prefira localizar o CAR ou desenhar o pol├¡gono no mapa.",
      });
    }
    ...
    try {
      ...
      const onLayer = (layer: GeoLayerResult, total: number) => {
        ...
        setWaveAResult((prev) => {
          ...
          return {
            ...
            layers,
            factualSummary: prev?.factualSummary ?? "A consultar camadasÔÇª",
      ...
      toast({
        title: "An├ílise factual conclu├¡da",
        description: docId
          ? `${okCount} camada(s) OK, ${partialCount} parcial/indispon├¡vel. Salva para Etapa 2. Exporte o PDF ou complemente com IA.`
          : `${okCount} camada(s) OK. Etapa 2 dispon├¡vel nesta sess├úo (n├úo gravou no Firestore ÔÇö veja o alerta).`,
      });
    } catch (error) {
      console.error("Wave A failed:", error);
      toast({
        variant: "destructive",
        title: "Erro na an├ílise factual",
  ...
  const handleStartAnalysis = async () => {
    ...
    if (!input || (needsLocalization(inputMode) && !waveInput)) {
      toast({
        ...
        title: "Dados insuficientes",
        description: needsLocalization(inputMode)
          ? "Localize e confirme o im├│vel antes da an├ílise IA."
          : "Preencha CAR, coordenadas, pol├¡gono, KML ou SHP para iniciar a an├ílise.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `Relat├│rio gerado com ${actionResult.provider === "deepseek" ? "DeepSeek" : "Gemini"}. Pronto para exporta├º├úo.`,
      });
    } catch (error) {
  ...
  const handleLocalizacaoConfirmed = React.useCallback(
    (resolved: LocalizacaoResolvida) => {
      ...
      toast({
        title: "Im├│vel confirmado",
        description: "Geometria SICAR pronta para an├ílise geoespacial.",
      });
    },
    [toast],
  );
  ...
  const analysisPreviewValue = React.useMemo(() => {
    if (needsLocalization(inputMode)) {
      if (localizacao?.status === "ok") {
        ...
        return `${cod ?? "CAR"} ┬À ${localizacao.areaHa.toFixed(2)} ha ┬À confian├ºa ${localizacao.confianca}${localizacaoConfirmada ? " ┬À confirmado" : ""}`;
      }
      return "";
    }
    ...
    if (built.data.length > 2000) {
      return `${built.data.slice(0, 2000)}ÔÇª (${built.data.length} caracteres)`;
    }
    return built.data;
  ...
  const handleUseProjectPerimetro = async () => {
    ...
    if (!perimetro) {
      toast({
        variant: "destructive",
        title: "Sem per├¡metro no cadastro",
        description: "Este empreendimento n├úo tem per├¡metro de refer├¬ncia salvo.",
      });
      return;
    }
    ...
    try {
      ...
      if (!resolved) {
        throw new Error("N├úo foi poss├¡vel interpretar o per├¡metro de refer├¬ncia.");
      }
      ...
      setShpFileName("");
      toast({
        title: "Per├¡metro carregado do cadastro",
        description: `${resolved.areaHa.toFixed(2)} ha ÔÇö voc├¬ pode editar no mapa antes de analisar.`,
      });
    } catch (error) {
      toast({
        variant: "destructive",
        title: "Falha ao carregar per├¡metro",
  ...
  return (
    <StudyGeospatialStackedShell
      title="An├ílise Geoespacial (IA)"
      description={`Desenhe o per├¡metro, execute as ${WAVE_ALL_LAYER_COUNT} camadas SIG (MG, incl. potencialidade CECAV), exporte mapas no layout Pimenta (PDF, PNG, JPEG ou Word) e complemente com IA em Relat├│rios de IA.`}
      topExtras={
        <>
          ...
          {empreendimentoId ? (
            <p className="rounded-md border border-primary/30 bg-primary/5 px-3 py-2 text-sm text-muted-foreground">
              ...
              {loadingLinkedProject ? " (carregandoÔÇª)" : null}. A an├ílise gravada ficar├í
              dispon├¡vel no PEA deste projeto.
            </p>
          ) : null}
        </>
      }
      mapPane={
        <Card className="flex w-full flex-col overflow-hidden">
          <CardHeader className="shrink-0 space-y-1 pb-3">
            <CardTitle>Captura por coordenada / pol├¡gono</CardTitle>
            <CardDescription>
              Mapa em largura total: desenhe o per├¡metro, capture coordenadas ou
        ...
        <>
          <Collapsible defaultOpen={false} className="group rounded-lg border bg-card shadow-sm">
            ...
            <CollapsibleContent className="space-y-2 border-t px-3 py-3 text-xs leading-relaxed text-muted-foreground">
              <p>
                Etapa 1: relat├│rio factual SIG ({WAVE_ALL_LAYER_COUNT} camadas, sem custo de IA).
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">An├ílise geoespacial factual</CardTitle>
              <CardDescription>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2 rounded-md border border-dashed p-3">
                ...
                >
                  <SelectTrigger id="aa-empreendimento">
                    <SelectValue placeholder="Nenhum ÔÇö an├ílise avulsa" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="_none">Nenhum ÔÇö an├ílise avulsa</SelectItem>
                    {loadingProjects ? (
                      <SelectItem value="_loading" disabled>
                        Carregando empreendimentosÔÇª
                      </SelectItem>
                    ) : null}
                ...
                {linkedProject && projectHasPerimetroReferencia(linkedProject) ? (
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    >
                      ...
                      Usar per├¡metro do cadastro
                    </Button>
                  </div>
                ) : empreendimentoId && !loadingLinkedProject ? (
                  <p className="text-xs text-muted-foreground">
                    Este empreendimento n├úo tem per├¡metro de refer├¬ncia no cadastro.
                  </p>
                ) : null}
              </div>
              ...
              <div className="space-y-2 md:col-span-1">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="car">N├║mero do CAR</SelectItem>
                    <SelectItem value="coordinates">Coordenadas</SelectItem>
                    <SelectItem value="gps">Localiza├º├úo GPS (celular)</SelectItem>
                    <SelectItem value="polygon">Pol├¡gono (WKT/GeoJSON)</SelectItem>
                    <SelectItem value="kml">Per├¡metro KML (.kml)</SelectItem>
                    <SelectItem value="shp">Per├¡metro SHP (.zip)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {inputMode === "polygon" && (
                <div className="space-y-2">
                  <Label htmlFor="polygon-input">Pol├¡gono (WKT ou GeoJSON)</Label>
                  <Textarea
                    id="polygon-input"
              ...
              {inputMode === "kml" && (
                <div className="space-y-2">
                    ...
                    onChange={async (e) => {
                      ...
                      if (file.size > 4 * 1024 * 1024) {
                        toast({
                          ...
                          description: "Use um KML at├® 4 MB ou extraia o .kml de um KMZ.",
                        });
                        return;
                      }
                  ...
                  {kmlFileName ? (
                    <p className="text-xs text-muted-foreground">
                      Carregado: {kmlFileName}. O per├¡metro ser├í lido do Placemark/Polygon no KML.
                    </p>
                  ) : null}
                </div>
              )}
                    ...
                    onChange={async (e) => {
                      ...
                      if (file.size > 8 * 1024 * 1024) {
                        toast({
                          variant: "destructive",
                          title: "Arquivo grande demais",
                          description: "Use um ZIP at├® 8 MB com .shp, .shx e .dbf.",
                        });
                        return;
                      }
              ...
              <div className="space-y-2 md:col-span-3">
                <Label htmlFor="analysis-preview">Resumo do per├¡metro enviado</Label>
                <Textarea
                  id="analysis-preview"
              ...
              >
                {isWaveALoading ? (
                  ...
                  <>
                    ...
                    Gerar relat├│rio factual ({WAVE_ALL_LAYER_COUNT} camadas MG + federal)
                  </>
                )}
              </Button>
              </div>
      ...
      resultsPane={
        <Card>
            <CardHeader>
              <CardTitle className="text-base">Resultados da an├ílise</CardTitle>
              <CardDescription>
                Dados SIG por camada. Use o painel de exporta├º├úo para PDF, PNG, JPEG ou Word ÔÇö
                por camada ou relat├│rio completo (com parecer IA se j├í gerou a Etapa 2).
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isWaveALoading || isLoading ? (
                <div className="flex flex-col items-center justify-center py-10 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    {isWaveALoading
                      ? waveALayersDone > 0
                        ? `Camada ${waveALayersDone} de ${waveALayersTotal} (IDE-Sisema MG + SICAR + IBAMA)ÔÇª`
                        : `Consultando ${WAVE_ALL_LAYER_COUNT} camadas (IDE-Sisema MG + SICAR + IBAMA)ÔÇª`
                      : "Processando dados e gerando an├ílise geoespacial..."}
                  </p>
                </div>
                  ...
                  {waveAResult ? (
                    <>
                      <div className="rounded-lg border bg-muted/30 px-4 py-3">
                        ...
                        <p className="mt-2 text-sm font-semibold text-primary">
                          ├ürea: {waveAResult.perimeter.areaHa.toFixed(2)} ha ┬À An├ílise SIG MG (
                    ...
                    <>
                      ...
                      <div className="rounded-md border p-3">
                        <p className="mb-2 text-sm font-medium">Evid├¬ncias factuais</p>
                        <div className="space-y-2">
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Configure o per├¡metro e clique em &quot;Gerar relat├│rio factual ({WAVE_ALL_LAYER_COUNT} camadas MG)&quot;.
