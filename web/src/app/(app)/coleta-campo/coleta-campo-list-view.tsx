...
function campanhaTitulo(
  ...
  const emp = projectsMap.get(inv.empreendimentoId ?? '') ?? inv.empreendimentoId ?? 'ÔÇö';
...
export function ColetaCampoListView() {
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Parcelas e ├írvores vinculadas foram removidas.',
      });
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Coleta offline de parcelas e ├írvores. Ao concluir, exporte o Excel e importe em{' '}
                <strong>Invent├írio Florestal</strong> (menu irm├úo).
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TooltipProvider>
                      ...
                      filtered.map((inv) => {
                        ...
                        const periodo =
                          inv.dataInicio || inv.dataFim
                            ? `${formatDate(inv.dataInicio)} ÔÇô ${formatDate(inv.dataFim)}`
                            : null;
                        return (
                          <Card
                            key={inv.id}
                              ...
                              <div className="flex flex-col gap-4">
                                <div className="min-w-0 space-y-1.5">
                                  ...
                                  <p className="text-sm text-muted-foreground">
                                    {inv.modo === 'solta' ? 'Solta' : 'Vinculada'} ┬À{' '}
                                    {inv.tipoInventario === 'multinivel' ? 'Multin├¡vel' : 'Simples'}
                                  </p>
                                  {periodo ? (
                    ...
                    {!isLoading && filtered.length === 0 && (
                      <div className="flex h-24 items-center justify-center rounded-md border-2 border-dashed border-muted-foreground/25 text-sm text-muted-foreground">
                        {sorted.length
                          ...
                          : 'Nenhuma campanha. Crie uma campanha vinculada ou solta para come├ºar.'}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deleteTarget
                ? `A campanha "${campanhaTitulo(deleteTarget, projectsMap, empreendedoresMap)}" e todas as parcelas e ├írvores vinculadas ser├úo removidas permanentemente.`
