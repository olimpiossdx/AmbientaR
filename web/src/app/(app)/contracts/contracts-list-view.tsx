...
const DetailItem = ({
  ...
  <div className="space-y-1">
    ...
    <p className="text-sm text-muted-foreground">{value || "N├úo informado"}</p>
  </div>
);
...
export function ContractsListView() {
  ...
  const { uploadFile, dialogProps } = useStorageFileUpload({
    ...
    buildStoragePath: (_file, safe) => {
      if (!uploadingItem) throw new Error("Contrato n├úo selecionado.");
  ...
  // Resolve os clientIds que o usu├írio (cliente ou representante) pode ver.
  React.useEffect(() => {
    ...
    if (isClientePortalRole(user.role)) {
      ...
      if (isSelfRegistered) {
        ...
        // Fallback: se `userId` n├úo casar com os dados do client no Firestore,
        // tenta tamb├®m por CPF/CNPJ.
        const qByDoc =
          userDocs.length > 0
  ...
  const contractsQuery = useMemoFirebase(() => {
    ...
    // Busca geral; o filtro por perfil ├® aplicado em mem├│ria
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "N├úo foi poss├¡vel enviar o arquivo.",
      });
    } finally {
        ...
        try {
          ...
          toast({
            ...
            description:
              ...
              (persistErr.message.includes("permiss├úo") ||
                persistErr.message.includes("Storage"))
                ? "Sem permiss├úo para salvar no Storage; o PDF foi baixado no seu computador."
                : "N├úo foi poss├¡vel salvar no Storage; o PDF foi baixado localmente.",
          });
        }
      } else {
        downloadContractPdfBlob(blob, contract.contratante?.nome);
      ...
      toast({
        ...
        description:
          err instanceof Error
            ? err.message
            : "N├úo foi poss├¡vel gerar o PDF do contrato.",
        variant: "destructive",
      });
    } finally {
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6 space-y-8">
          <div className="flex flex-wrap items-end gap-2 p-3 rounded-lg bg-muted/50">
            ...
            <div className="flex flex-wrap items-end gap-2">
              ...
              <div>
                <Label className="text-xs">Valor m├¡n.</Label>
                <Input
                  type="number"
                  placeholder="0"
                  className="h-8 w-24"
              ...
              <div>
                <Label className="text-xs">Valor m├íx.</Label>
                <Input
                  type="number"
                  placeholder="0"
                  className="h-8 w-24"
          ...
          {canWriteContractsCommercial(user?.role) && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Crie, edite e gerencie seus contratos de presta├º├úo de servi├ºo.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <TooltipProvider>
          ...
          {isPortalUser && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Baixe o PDF do contrato gerado a partir da proposta aceita. Ap├│s
                  assinar, a consultoria far├í o upload da vers├úo assinada.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <TooltipProvider>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Visualize contratos aprovados e fa├ºa upload da vers├úo assinada.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TooltipProvider>
      ...
      <Dialog open={isFromProposalOpen} onOpenChange={setIsFromProposalOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione uma proposta aceita para reaproveitar os servi├ºos no
              contrato.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            {availableAcceptedProposals.length === 0 && (
              <p className="text-sm text-muted-foreground">
                Nenhuma proposta aceita dispon├¡vel sem contrato vinculado.
              </p>
            )}
          </div>
          <DialogFooter>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {uploadingItem?.contratante?.nome ?? "ÔÇö"}.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Visualiza├º├úo do contrato com{" "}
              {viewingItem?.contratante?.nome ?? "ÔÇö"}.
            </DialogDescription>
          </DialogHeader>
          {viewingItem && (
            <div className="form-scroll-body max-h-[60vh] space-y-4">
              ...
              <DetailItem
                label="Nome"
                value={viewingItem.contratante?.nome ?? "ÔÇö"}
              />
              <DetailItem
                label="CPF/CNPJ"
                value={viewingItem.contratante?.cpfCnpj ?? "ÔÇö"}
              />
              <DetailItem
                label="Endere├ºo"
              ...
              <DetailItem
                label="Nome"
                value={viewingItem.contratado?.name ?? "ÔÇö"}
              />
              <DetailItem
                label="CNPJ"
                value={viewingItem.contratado?.cnpj ?? "ÔÇö"}
              />
              <Separator />
              ...
              <DetailItem
                label="Nome"
                value={viewingItem.responsavelTecnico?.name ?? "ÔÇö"}
              />
              <DetailItem
                label="Profiss├úo"
                value={viewingItem.responsavelTecnico?.profession ?? "ÔÇö"}
              />
              <Separator />
              ...
              <DetailItem
                ...
                value={viewingItem.objeto?.empreendimento ?? "ÔÇö"}
              />
              <p className="text-sm font-medium">Servi├ºos</p>
              <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                {viewingItem.objeto?.servicos ?? "ÔÇö"}
              </p>
              ...
              <AttachmentPreviewSection
                ...
                emptyLabel="PDF do contrato ainda n├úo gerado. Use ┬½Gerar PDF┬╗ na lista."
              ...
              <AttachmentPreviewSection
                ...
                zoomDescription="Vers├úo assinada enviada pela consultoria ou cliente."
              />
            </div>
          )}
          <DialogFooter>
