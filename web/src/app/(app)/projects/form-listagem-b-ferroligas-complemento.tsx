...
export function FormListagemBFerroligasComplemento({ form }: { form: any }) {
    ...
    <div className="space-y-6">
      ...
      <SectionCard title="24. Área do empreendimento">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <NumField form={form} name={`${base}.areasEmpreendimento.areaTotalM2`} label="Área total do terreno (m²)" />
          <NumField form={form} name={`${base}.areasEmpreendimento.areaUtilM2`} label="Área útil (m²)" />
          <NumField form={form} name={`${base}.areasEmpreendimento.areaConstruidaM2`} label="Área construída (m²)" />
        </div>
      </SectionCard>
      ...
      <SectionCard title="25. Recursos humanos">
        ...
        {['producao', 'administrativo', 'outrosSetores'].map((setor) => (
          <div key={setor} className="space-y-3 rounded-md border p-3">
            ...
            <NumField form={form} name={`${base}.recursosHumanos.${setor}.numeroFuncionarios`} label="Nº de funcionários" />
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <NumField form={form} name={`${base}.recursosHumanos.${setor}.percentProprioMunicipio`} label="% próprio município" />
              <NumField form={form} name={`${base}.recursosHumanos.${setor}.percentOutroMg`} label="% outro município MG" />
              <NumField form={form} name={`${base}.recursosHumanos.${setor}.percentOutrosEstados`} label="% outros estados" />
            </div>
          </div>
        ))}
      </SectionCard>
      ...
      <SectionCard title="26. Regime de operação do empreendimento">
        ...
        {['administrativo', 'operacaoTurno1', 'operacaoTurno2'].map((turno) => (
          <div key={turno} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            ...
            <NumField form={form} name={`${base}.regimeOperacao.${turno}.funcionarios`} label="Nº funcionários/turno" />
            <TextField form={form} name={`${base}.regimeOperacao.${turno}.horario`} label="Horário (início/fim)" />
            <TextField form={form} name={`${base}.regimeOperacao.${turno}.pausa`} label="Pausa (início/fim)" />
          </div>
        ))}
        <CheckboxOptions form={form} name={`${base}.regimeOperacao.diasOperacao`} options={diasSemana.map((d) => ({ id: d, label: d }))} />
        <CheckboxOptions form={form} name={`${base}.regimeOperacao.mesesOperacao`} options={mesesAno.map((m) => ({ id: m, label: m }))} />
        <FormField
          control={form.control}
      ...
      <SectionCard title="27. Fornecedores com instalações dentro do empreendimento">
        ...
        {possuiFornecedores && (
          <>
            <NumField form={form} name={`${base}.fornecedoresInternos.quantidadeEmpresas`} label="Quantas empresas?" />
            <FormField
              control={form.control}
          ...
          <>
            ...
            {fornecedoresMadeira.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
                <TextField form={form} name={`${base}.usoMadeira.fornecedores.${index}.material`} label="Material utilizado" />
                <TextField form={form} name={`${base}.usoMadeira.fornecedores.${index}.razaoSocial`} label="Razão social" />
                <TextField form={form} name={`${base}.usoMadeira.fornecedores.${index}.cnpjCpf`} label="CNPJ/CPF" />
                <TextField form={form} name={`${base}.usoMadeira.fornecedores.${index}.endereco`} label="Endereço" />
                <FormField
                  control={form.control}
      ...
      <SectionCard title="29. Uso de minério em estado bruto">
        ...
        {usaMinerioBruto && (
          <>
            {fornecedoresMinerio.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
                <TextField form={form} name={`${base}.usoMinerioBruto.fornecedores.${index}.minerio`} label="Minério utilizado" />
                <TextField form={form} name={`${base}.usoMinerioBruto.fornecedores.${index}.razaoSocial`} label="Razão social" />
                <TextField form={form} name={`${base}.usoMinerioBruto.fornecedores.${index}.cnpj`} label="CNPJ" />
                <TextField form={form} name={`${base}.usoMinerioBruto.fornecedores.${index}.endereco`} label="Endereço" />
                <FormField
                  control={form.control}
      ...
      <SectionCard title="30. Uso de insumo radioativo">
        <FormField
          control={form.control}
        ...
        {usaRadioativo && (
          <>
            <TextField form={form} name={`${base}.insumoRadioativo.quaisInsumos`} label="Quais insumos?" />
            <FormField
              control={form.control}
      ...
      <SectionCard title="31. Uso de resíduos – Resolução CONAMA nº 023/1996">
        <FormField
          control={form.control}
        ...
        {usaResiduosConama && (
          <>
            <TextField form={form} name={`${base}.residuosConama023.nomesCodigos`} label="Resíduos (nomes e códigos)" />
            <FormField
              control={form.control}
      ...
      <SectionCard title="32. Uso de resíduos gerados por terceiros dentro do país">
        <FormField
          control={form.control}
        ...
        {usaResiduosTerceiros && (
          <>
            {residuosTerceiros.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
                <TextField form={form} name={`${base}.residuosTerceiros.itens.${index}.nomeResiduo`} label="Nome do resíduo" />
                <TextField form={form} name={`${base}.residuosTerceiros.itens.${index}.razaoSocial`} label="Razão social" />
                <TextField form={form} name={`${base}.residuosTerceiros.itens.${index}.cnpj`} label="CNPJ" />
                <TextField form={form} name={`${base}.residuosTerceiros.itens.${index}.endereco`} label="Endereço" />
                <FormField
                  control={form.control}
      ...
      <SectionCard title="33. Consolidação de matérias-primas e materiais intermediários">
        <FormDescription>
          ...
          return (
            <div key={slug} className="space-y-3 rounded-md border p-3">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                ...
                />
                <TextField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.estadoFisico`} label="Estado físico" />
                <TextField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.embalagem`} label="Código embalagem (*)" />
                <TextField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.armazenamento`} label="Código armazenamento (**)" />
                <NumField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.consumoMaximo`} label="Consumo mensal máximo" />
                <NumField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.consumoMedio`} label="Consumo mensal médio" />
                <TextField form={form} name={`${base}.materiasPrimasConsolidacao.itens.${slug}.unidade`} label="Unidade" />
              </div>
            </div>
          );
        })}
        {linhasMateriasPrimas.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <TextField form={form} name={`${base}.materiasPrimasConsolidacao.linhas.${index}.descricao`} label="Outro insumo" className="md:col-span-2" />
            <NumField form={form} name={`${base}.materiasPrimasConsolidacao.linhas.${index}.consumoMaximo`} label="Consumo máximo" />
            <NumField form={form} name={`${base}.materiasPrimasConsolidacao.linhas.${index}.consumoMedio`} label="Consumo médio" />
