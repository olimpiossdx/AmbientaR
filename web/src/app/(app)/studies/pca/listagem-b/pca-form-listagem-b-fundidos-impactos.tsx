...
export function PcaFormListagemBFundidosImpactos({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      <PcaSectionCard title="52. Possibilidades de acidentes com danos ambientais">
      ...
      <PcaSectionCard title="53. Outros agentes causadores de impactos ambientais">
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="54. Melhorias significativas">
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="55. Cronograma de implantação">
        <FormDescription>Apresentar cronograma no Anexo XXXIII.</FormDescription>
        <PcaTextField form={form} name="listagemB.cronogramaImplantacao.referenciaAnexo" label="Referência / observações" />
      </PcaSectionCard>

      <PcaSectionCard title="56 a 58. Possíveis impactos ambientais (Módulo 6)">
        <PcaCheckboxOptions form={form} name="listagemB.impactos.meioFisico" options={impactosMeioFisico.map((i) => ({ id: i, label: i }))} />
        <PcaTextField form={form} name="listagemB.impactos.meioFisicoOutros" label="Meio físico – outros" />
        <PcaCheckboxOptions form={form} name="listagemB.impactos.meioBiotico" options={impactosMeioBiotico.map((i) => ({ id: i, label: i }))} />
        <PcaTextField form={form} name="listagemB.impactos.meioBioticoOutros" label="Meio biótico – outros" />
        <PcaCheckboxOptions form={form} name="listagemB.impactos.meioSocioeconomico" options={impactosMeioSocioeconomico.map((i) => ({ id: i, label: i }))} />
        <PcaTextField form={form} name="listagemB.impactos.meioSocioeconomicoOutros" label="Meio socioeconômico – outros" />
        <FormDescription>Identificação e avaliação detalhada no Anexo XXXIV; medidas mitigadoras no Anexo XXXV.</FormDescription>
      </PcaSectionCard>

      <PcaSectionCard title="59. Outros impactos não abordados no TR">
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="61. Componente geofísico e biótico (ZEE)">
        <FormDescription>
        ...
        {zeeGeofisico.map((camada) => {
          ...
          return (
            <div key={slug} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
              <PcaTextField form={form} name={`listagemB.zeeGeofisico.${slug}.classificacao`} label={camada} />
              <PcaNumField form={form} name={`listagemB.zeeGeofisico.${slug}.percentual`} label="Distribuição (%)" />
            </div>
          );
        })}
      </PcaSectionCard>

      <PcaSectionCard title="62. Componente socioeconômico (ZEE)">
        {municipiosZee.map((item, index) => (
          <div key={item.id} className="space-y-3 rounded-md border p-3">
            ...
            <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.municipio`} label="Município" />
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.ips`} label="IPS" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.populacao`} label="População" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.distribuicaoEspacial`} label="Distribuição espacial" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.razaoDependencia`} label="Razão de dependência" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.malhaRodoviaria`} label="Índice malha rodoviária" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.vaIndustria`} label="Índice VA indústria" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.vaServicos`} label="Índice VA serviços" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.vaAgropecuaria`} label="Índice VA agropecuária" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.exportacoes`} label="Índice exportações" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.renda`} label="Índice renda" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.saude`} label="Índice saúde" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.educacao`} label="Índice educação" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.idhM`} label="IDH-M" />
              <PcaTextField form={form} name={`listagemB.zeeSocioeconomico.${index}.gestaoAmbiental`} label="Gestão ambiental" />
            </div>
          </div>
        ))}
