...
export function PcaFormListagemBFundidosComplemento({ form }: { form: any }) {
    ...
    <div className="space-y-6">
      ...
      <PcaSectionCard title="24. Área do empreendimento">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemB.areasEmpreendimento.areaTotalM2" label="Área total do terreno (m²)" />
          <PcaNumField form={form} name="listagemB.areasEmpreendimento.areaUtilM2" label="Área útil (m²)" />
          <PcaNumField form={form} name="listagemB.areasEmpreendimento.areaConstruidaM2" label="Área construída (m²)" />
        </div>
      </PcaSectionCard>
      ...
      <PcaSectionCard title="27. Fornecedores com instalações dentro do empreendimento">
        ...
        />
        {possuiFornecedores && (
          <PcaNumField form={form} name="listagemB.fornecedoresInternos.quantidadeEmpresas" label="Quantas empresas há nestas condições?" />
        )}
        <FormDescription>
          ...
          <>
            ...
            {fornecedoresMadeira.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
                <PcaTextField form={form} name={`listagemB.usoMadeira.fornecedores.${index}.material`} label="Material utilizado" />
                <PcaTextField form={form} name={`listagemB.usoMadeira.fornecedores.${index}.razaoSocial`} label="Razão social do fornecedor" />
                <PcaTextField form={form} name={`listagemB.usoMadeira.fornecedores.${index}.cnpjCpf`} label="CNPJ/CPF" />
                <PcaTextField form={form} name={`listagemB.usoMadeira.fornecedores.${index}.endereco`} label="Endereço" />
                <FormField
                  control={form.control}
      ...
      <PcaSectionCard title="29. Uso de insumo radioativo">
        <FormField
          control={form.control}
        ...
        {usaRadioativo && (
          <>
            <PcaTextField form={form} name="listagemB.insumoRadioativo.quaisInsumos" label="Quais são esses insumos?" />
            <FormField
              control={form.control}
      ...
      <PcaSectionCard title="30. Uso de resíduos listados na Resolução CONAMA nº 023/1996">
        <FormField
          control={form.control}
        ...
        {usaResiduosConama && (
          <>
            <PcaTextField form={form} name="listagemB.residuosConama023.nomesCodigos" label="Resíduos (nomes e códigos)" />
            <FormField
              control={form.control}
      ...
      <PcaSectionCard title="31. Uso de resíduos gerados por terceiros dentro do país">
        <FormField
          control={form.control}
        ...
        {usaResiduosTerceiros && (
          <>
            {residuosTerceiros.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
                <PcaTextField form={form} name={`listagemB.residuosTerceiros.itens.${index}.nomeResiduo`} label="Nome do resíduo" />
                <PcaTextField form={form} name={`listagemB.residuosTerceiros.itens.${index}.razaoSocial`} label="Razão social do fornecedor" />
                <PcaTextField form={form} name={`listagemB.residuosTerceiros.itens.${index}.cnpj`} label="CNPJ" />
                <PcaTextField form={form} name={`listagemB.residuosTerceiros.itens.${index}.endereco`} label="Endereço" />
                <FormField
                  control={form.control}
      ...
      <PcaSectionCard title="32. Consolidação de matérias-primas e materiais intermediários">
        <FormDescription>
          ...
          return (
            <div key={slug} className="space-y-3 rounded-md border p-3">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.descricao`} label="Matéria-prima / insumo" />
                <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.estadoFisico`} label="Estado físico" />
                <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.embalagem`} label="Código embalagem (*)" />
                <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.armazenamento`} label="Código armazenamento (**)" />
                <PcaNumField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.consumoMaximo`} label="Consumo mensal máximo" />
                <PcaNumField form={form} name={`listagemB.materiasPrimasConsolidacao.setores.${slug}.consumoMedio`} label="Consumo mensal médio" />
              </div>
            </div>
          );
        })}
        {linhasMateriasPrimas.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.linhas.${index}.setor`} label="Setor (outros)" />
            <PcaTextField form={form} name={`listagemB.materiasPrimasConsolidacao.linhas.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemB.materiasPrimasConsolidacao.linhas.${index}.consumoMensal`} label="Consumo mensal" />
