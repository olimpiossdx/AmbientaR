...
export function PcaFormListagemCTecnico({
  form,
  variant = 'borracha',
}: {
  form: any;
  ...
  return (
    <div className="space-y-6">
      {!isTrExtended && (
      <PcaSectionCard title="21. Caracterização técnica do empreendimento">
        <FormDescription>
      ...
      {!isTrExtended && (
      <PcaSectionCard title="22. Área do empreendimento">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemC.areaEmpreendimento.areaTotalM2" label="Área total do terreno (m²)" />
          <PcaNumField form={form} name="listagemC.areaEmpreendimento.areaUtilM2" label="Área útil (m²)" />
          <PcaNumField form={form} name="listagemC.areaEmpreendimento.areaConstruidaM2" label="Área construída (m²)" />
        </div>
      </PcaSectionCard>
      )}

      <PcaSectionCard title={`${item(23)}. Recursos humanos`}>
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {setoresRh.map((setor) => (
            <div key={setor.key} className="rounded-md border p-3">
              <p className="mb-2 font-medium">{setor.label}</p>
              <PcaNumField form={form} name={`listagemC.recursosHumanos.${setor.key}.quantidade`} label="Nº de funcionários" />
      ...
      <PcaSectionCard title={`${item(24)}. Regime de operação do empreendimento`}>
        {turnosFields.map((item, index) => (
          <div key={item.id} className="mb-3 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-6">
            <PcaTextField form={form} name={`listagemC.regimeOperacao.turnos.${index}.setor`} label="Setor" />
            <PcaNumField form={form} name={`listagemC.regimeOperacao.turnos.${index}.funcionariosTurno`} label="Nº funcionários/turno" />
            <PcaTextField form={form} name={`listagemC.regimeOperacao.turnos.${index}.horarioInicio`} label="Horário início" />
            <PcaTextField form={form} name={`listagemC.regimeOperacao.turnos.${index}.horarioFim`} label="Horário fim" />
            <PcaTextField form={form} name={`listagemC.regimeOperacao.turnos.${index}.pausaInicio`} label="Pausa início" />
            <PcaTextField form={form} name={`listagemC.regimeOperacao.turnos.${index}.pausaFim`} label="Pausa fim" />
      ...
      <PcaSectionCard title={`${item(25)}. Equipamentos e sistemas de apoio ao processo produtivo`}>
        <FormField
          control={form.control}
        ...
        {form.watch('listagemC.equipamentosApoio.resfriamento.ativa') && (
          <div className="mb-4 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
            <PcaNumField form={form} name="listagemC.equipamentosApoio.resfriamento.capacidadeKcalH" label="Capacidade nominal (kcal/h)" />
            <PcaTextField form={form} name="listagemC.equipamentosApoio.resfriamento.fluido" label="Fluido refrigerante" />
            <PcaNumField form={form} name="listagemC.equipamentosApoio.resfriamento.volumeFluidoM3" label="Volume de fluido no sistema (m³)" />
            <PcaNumField form={form} name="listagemC.equipamentosApoio.resfriamento.volumeDescartadoM3Ano" label="Volume descartado (m³/ano)" />
            <PcaNumField form={form} name="listagemC.equipamentosApoio.resfriamento.volumeReposicaoM3Ano" label="Volume de reposição (m³/ano)" />
          </div>
        )}
        ...
        {form.watch('listagemC.equipamentosApoio.compressor.ativa') && (
          <div className="mb-4 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
            <PcaNumField form={form} name="listagemC.equipamentosApoio.compressor.capacidadeM3H" label="Capacidade nominal (m³/h)" />
            <PcaNumField form={form} name="listagemC.equipamentosApoio.compressor.purgaLDia" label="Água de purga (l/dia)" />
          </div>
        )}
        ...
        {possuiCaldeira && (
          <div className="mb-4 space-y-3 rounded-md border p-3">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.tempoOperacaoHDia" label="Tempo médio de operação (h/dia)" />
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.capacidadeVaporKgH" label="Capacidade de vapor (kg/h)" />
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.concentracaoArPercent" label="Concentração de ar na combustão (%)" />
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.purgaLDia" label="Água de purga (l/dia)" />
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.alturaChamineM" label="Altura da chaminé (m)" />
            </div>
            <PcaCheckboxOptions
              form={form}
            ...
            {combustivelCaldeira?.includes('madeira') && (
              <div className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
                <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.madeiraVolumeM3Mes" label="Volume madeira (m³/mês)" />
                <PcaCheckboxOptions
                  form={form}
                ...
                />
                <PcaTextField form={form} name="listagemC.equipamentosApoio.caldeira.madeiraNumeroIef" label="Nº certificado IEF" />
              </div>
            )}
            {combustivelCaldeira?.includes('licor_negro') && isPapel && (
              <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.licorNegroVolumeM3Mes" label="Volume licor negro (m³/mês)" />
            )}
            {combustivelCaldeira?.includes('oleo') && (
              <div className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
                ...
                />
                <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.oleoVolumeM3Mes" label="Volume óleo (m³/mês)" />
                <PcaNumField form={form} name="listagemC.equipamentosApoio.caldeira.oleoTanqueM3" label="Volume tanque (m³)" />
                <FormField
                  control={form.control}
            ...
            <PcaTextField form={form} name="listagemC.equipamentosApoio.caldeira.combustivelDetalhes" label="Outros detalhes de combustível e armazenamento" />
          </div>
        )}
        ...
        {possuiPosto && (
          <div className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-2">
            <PcaNumField form={form} name="listagemC.equipamentosApoio.postoAbastecimento.capacidadeM3" label="Capacidade de armazenamento (m³)" />
            <FormField
              control={form.control}
      ...
      >
        ...
        {outrosEquipamentos.map((item, index) => (
          <div key={item.id} className="mt-2 grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-6">
            <PcaTextField form={form} name={`listagemC.equipamentosOutros.linhasExtras.${index}.nome`} label="Equipamento" />
            <PcaTextField form={form} name={`listagemC.equipamentosOutros.linhasExtras.${index}.descricao`} label="Descrição" />
            <PcaNumField form={form} name={`listagemC.equipamentosOutros.linhasExtras.${index}.capacidadeMaxima`} label="Capacidade" />
            <PcaNumField form={form} name={`listagemC.equipamentosOutros.linhasExtras.${index}.tempoOperacaoHDia`} label="h/dia" />
            <PcaNumField form={form} name={`listagemC.equipamentosOutros.linhasExtras.${index}.nivelRuidoDba`} label="dBA" />
      ...
      <PcaSectionCard title={`${item(27)}. Produtos fabricados e/ou processados`}>
        <PcaTabelaLinhasFixas
          form={form}
      ...
      <PcaSectionCard title={`${item(28)}. Fluxograma do processo`}>
        <FormDescription>
        ...
        <PcaTextField form={form} name="listagemC.fluxograma.referenciaAnexo" label="Referência / observações ao anexo" />
      </PcaSectionCard>

      <PcaSectionCard title={`${item(29)}. Uso de água`}>
        <FormField
          control={form.control}
        ...
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <PcaNumField form={form} name="listagemC.usoAgua.volumeRecirculadoM3Mes" label="Volume recirculado (m³/mês)" />
            <PcaNumField form={form} name="listagemC.usoAgua.percentualRecirculada" label="Porcentagem recirculada (%)" />
          </div>
        )}
        <Table>
          <TableHeader>
      ...
      <PcaSectionCard title={`${item(30)}. Tratamento de água`}>
        <FormField
          control={form.control}
        ...
        {trataAgua && (
          <>
            <PcaNumField form={form} name="listagemC.tratamentoAgua.quantidadeM3Mes" label="Quantidade tratada (m³/mês)" />
            <PcaCheckboxOptions form={form} name="listagemC.tratamentoAgua.etapas" options={etapasTratamentoAgua} />
            <PcaTextField form={form} name="listagemC.tratamentoAgua.etapasOutros" label="Outras etapas – especificar" />
          </>
        )}
      </PcaSectionCard>

      <PcaSectionCard title={`${item(31)}. Efluentes sanitários`}>
        <PcaNumField form={form} name="listagemC.efluentesSanitarios.volumeM3Dia" label="Volume gerado (m³/dia)" />
        <PcaNumField form={form} name="listagemC.efluentesSanitarios.numSistemas" label="Quantidade de sistemas de tratamento" />
        {pontosSanitarios.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemC.efluentesSanitarios.pontos.${index}.identificacao`} label="Identificação" />
            <PcaTextField form={form} name={`listagemC.efluentesSanitarios.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemC.efluentesSanitarios.pontos.${index}.volumeM3Dia`} label="Volume (m³/dia)" />
      ...
      <PcaSectionCard title={`${item(32)}. Efluentes industriais`}>
        <FormField
          control={form.control}
        ...
        {geraEfluenteIndustrial && (
          <>
            <PcaNumField form={form} name="listagemC.efluentesIndustriais.numSistemas" label="Quantidade de sistemas de tratamento" />
            {pontosIndustriais.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
                <PcaTextField form={form} name={`listagemC.efluentesIndustriais.pontos.${index}.identificacao`} label="Identificação" />
                <PcaTextField form={form} name={`listagemC.efluentesIndustriais.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
                <PcaNumField form={form} name={`listagemC.efluentesIndustriais.pontos.${index}.volumeM3Dia`} label="Volume (m³/dia)" />
                <div className="md:col-span-4 flex justify-end">
              ...
              {parametrosEfluenteIndustrial.map((param) => {
                ...
                return (
                  <div key={slug} className="grid grid-cols-2 gap-2 rounded border p-2">
                    ...
                    <PcaTextField form={form} name={`listagemC.efluentesIndustriais.caracteristicas.${slug}.minimo`} label="Mín." />
                    <PcaTextField form={form} name={`listagemC.efluentesIndustriais.caracteristicas.${slug}.maximo`} label="Máx." />
                  </div>
                );
              })}
            </div>
      ...
      <PcaSectionCard title={`${item(32, '2')} Destino final dos efluentes`}>
        <p className="text-sm font-medium">Industrial</p>
        <PcaTextField form={form} name="listagemC.destinoEfluentes.industrial.corpoHidrico" label="Corpo hídrico (se aplicável)" />
        <PcaCheckboxOptions
          form={form}
        ...
        />
        <PcaNumField form={form} name="listagemC.destinoEfluentes.percentualEsgotoTratado" label="Percentual de esgoto tratado (%)" />
      </PcaSectionCard>

      {isPapel && (
        <PcaSectionCard title={`${item(35)}. Emissões atmosféricas`}>
          <FormField
            control={form.control}
          ...
          {fontesPontuais &&
            fontesEmissao.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
                <PcaTextField form={form} name={`listagemC.emissoesAtmosfericas.fontes.${index}.fonte`} label="Fonte" />
                <PcaTextField form={form} name={`listagemC.emissoesAtmosfericas.fontes.${index}.combustivel`} label="Combustível" />
                <PcaTextField form={form} name={`listagemC.emissoesAtmosfericas.fontes.${index}.poluentes`} label="Poluentes emitidos" />
                <div className="flex items-end justify-end">
          ...
          {controleEmissoes &&
            controleEquip.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
                <PcaTextField form={form} name={`listagemC.emissoesAtmosfericas.equipamentosControle.${index}.nome`} label="Equipamento" />
                <PcaNumField form={form} name={`listagemC.emissoesAtmosfericas.equipamentosControle.${index}.tempoOperacaoHDia`} label="Tempo médio (h/dia)" />
                <PcaTextField form={form} name={`listagemC.emissoesAtmosfericas.equipamentosControle.${index}.capacidade`} label="Capacidade nominal" />
                <div className="flex items-end justify-end">
      ...
      >
        ...
        {outrosResiduos.map((item, index) => (
          <div key={item.id} className="mt-2 grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-5">
            <PcaTextField form={form} name={`listagemC.residuosSolidos.linhasExtras.${index}.nome`} label="Resíduo" />
            <PcaTextField form={form} name={`listagemC.residuosSolidos.linhasExtras.${index}.equipamentoGerador`} label="Gerador" />
            <PcaTextField form={form} name={`listagemC.residuosSolidos.linhasExtras.${index}.classeResiduo`} label="Classe" />
            <PcaTextField form={form} name={`listagemC.residuosSolidos.linhasExtras.${index}.taxaMaximaGeracao`} label="Taxa máx." />
            <div className="flex items-end justify-end">
      ...
      {!isTrExtended && (
      <PcaSectionCard title="35. Documentação junto ao Corpo de Bombeiros">
        <FormField
          control={form.control}
      ...
      {!isTrExtended && (
      <PcaSectionCard title="36. Passivos ambientais">
        <FormField
          control={form.control}
