...
const GoogleLogo = () => (
  ...
  >
    <path
      d="M21.9999 12.2272C21.9999 11.3926 21.9272 10.5726 21.7863 9.7725H12.2226V14.3835H17.8599C17.6272 15.8617 16.9181 17.1476 15.8181 17.9476V20.8498H19.6454C21.1499 19.2612 21.9999 17.0658 21.9999 14.4135V12.2272Z"
      fill="#4285F4"
    />
    <path
      d="M12.2226 22.0001C15.1181 22.0001 17.5954 21.0501 19.6454 19.4614L15.8181 16.5591C14.9045 17.1819 13.6817 17.5819 12.2226 17.5819C9.45446 17.5819 7.10446 15.7182 6.24082 13.1364H2.29082V16.0909C4.26355 20.0091 7.91809 22.0001 12.2226 22.0001Z"
      fill="#34A853"
    />
    <path
      d="M6.24081 13.1364C6.01354 12.5046 5.88172 11.8319 5.88172 11.1364C5.88172 10.4409 6.01354 9.76818 6.23626 9.13636V6.18181H2.29081C1.49081 7.80454 0.999909 9.63181 0.999909 11.5454C0.999909 13.4591 1.49081 15.2864 2.29081 16.9091L6.24081 13.1364Z"
      fill="#FBBC05"
    />
    <path
      d="M12.2226 5.61818C13.8045 5.61818 15.2272 6.18182 16.3272 7.22727L19.7226 3.83182C17.5908 1.83182 15.1181 1 12.2226 1C7.91809 1 4.26355 2.99091 2.29082 6.90909L6.23626 9.86364C7.10446 7.28182 9.45446 5.61818 12.2226 5.61818Z"
      fill="#EA4335"
    />
  </svg>
);
...
const rcaListagens: TemplateList[] = [
  {
    field: "templatesRcaListagemA",
    title: "RCA - Listagem A - Atividades Minerárias",
  },
  {
    field: "templatesRcaListagemB",
    title: "RCA - Listagem B - Indústria Metalúrgica",
  },
  {
    field: "templatesRcaListagemC",
    title: "RCA - Listagem C - Indústria Química",
  },
  {
    field: "templatesRcaListagemD",
    title: "RCA - Listagem D - Indústria Alimentícia",
  },
  {
    field: "templatesRcaListagemE",
    title: "RCA - Listagem E - Infraestrutura",
  },
  {
    field: "templatesRcaListagemF",
    title: "RCA - Listagem F - Resíduos e Serviços",
  },
  {
    field: "templatesRcaListagemG",
    title: "RCA - Listagem G - Agrossilvipastoris",
  },
];
...
const outrosTemplates: TemplateList[] = [
  ...
  {
    field: "templatesRequerimentoIntervencao",
    title: "Requerimento de Intervenção Ambiental",
  },
  ...
  {
    field: "templatesProjetoTecnicoBarragem",
    title: "Projeto Técnico de Barragem",
  },
];
...
const templateGroups = [
  ...
  {
    title: "Relatório Ambiental Simplificado (LAS/RAS)",
    templates: lasRasListagens,
  },
...
export function SettingsView() {
  ...
  const [currentTemplateList, setCurrentTemplateList] =
  ...
  const featureFlagsDocRef = useMemoFirebase(
    () =>
      firestore ? doc(firestore, "companySettings", "featureFlags") : null,
    [firestore],
  );
  const {
    data: brandingData,
  ...
  const handleOpenTemplateDialog = (field: TemplateField) => {
    setCurrentTemplateList(field);
  ...
  React.useEffect(() => {
    if (
      ...
      window.location.hash === "#identidade-visual"
    ) {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          const templateToRemove = currentTemplates.find(
            (t: { name: string }) => t.name === itemToDelete!.id,
          );
          if (templateToRemove) {
            await updateDoc(brandingDocRef, {
              [fieldName]: arrayRemove(templateToRemove),
            });
  ...
  const handleSaveBrandingChoice = async (
  ...
  const handleToggleExternalChat = async (checked: boolean) => {
    if (!firestore || !featureFlagsDocRef) return;
    setIsSavingFeatureFlags(true);
    try {
      await setDoc(
        featureFlagsDocRef,
        { allowExternalChat: checked },
        { merge: true },
      );
      toast({
        title: checked ? "Chat externo liberado" : "Chat externo bloqueado",
        description: checked
          ? "Perfis cliente e representante agora podem usar o chat."
          : "Chat disponível apenas para uso interno.",
      });
    } catch (error) {
      console.error("Erro ao salvar flag de chat externo:", error);
            ...
            {authUser?.role === "admin" && (
              <>
                <Card
                  ...
                  className="border-primary/50 border-2 scroll-mt-4"
                >
                  <CardHeader>
                    ...
                    {isLoadingBranding ? (
                      ...
                      <>
                        <BrandingImageUploader
                          ...
                          imageUrl={brandingData?.headerImageUrl ?? null}
                          onUploadComplete={refetchBranding}
                        />
                        <BrandingImageUploader
                          label="Rodapé (Footer)"
                          fieldName="footerImageUrl"
                          imageUrl={brandingData?.footerImageUrl ?? null}
                          onUploadComplete={refetchBranding}
                        />
                        <BrandingImageUploader
                          label="Marca d'água (Watermark)"
                          fieldName="watermarkImageUrl"
                          imageUrl={brandingData?.watermarkImageUrl ?? null}
                          onUploadComplete={refetchBranding}
                        />
                      </>
                    )}
                ...
                <Card className="border-primary/50 border-2">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <span>Uso da Logo da Empresa</span>
                      <Badge className="bg-primary hover:bg-primary/90">
                        Premium
                      </Badge>
                    </CardTitle>
                    <CardDescription>
                      Decida como a identidade visual da sua empresa será
                      aplicada no sistema.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    <div>
                      ...
                      <RadioGroup
                        key={`logo-${brandingData?.logoUsage ?? "pdf_only"}`}
                        defaultValue={brandingData?.logoUsage || "pdf_only"}
                        onValueChange={(value: "pdf_only" | "system_wide") =>
                          handleSaveBrandingChoice("logoUsage", value)
                        }
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <RadioGroupItem
                            ...
                            id="system_wide"
                          />
                          <Label htmlFor="system_wide" className="font-normal">
                            Em todo o sistema (White-label)
                          </Label>
                        </div>
                        <p className="text-xs text-muted-foreground pl-6">
                          Substitui o logo &quot;AmbientaR&quot; pela sua marca
                          em todo o sistema, incluindo login e menus.
                        </p>
                      </RadioGroup>
                    </div>
                    {brandingData?.logoUsage === "system_wide" && (
                      <div className="pt-4 border-t space-y-2">
                        ...
                        <RadioGroup
                          key={`source-${brandingData?.systemLogoSource ?? "header"}`}
                          defaultValue={
                            brandingData?.systemLogoSource || "header"
                          }
                          onValueChange={(value: "header" | "watermark") =>
                            handleSaveBrandingChoice("systemLogoSource", value)
                          }
                          className="mt-2 space-y-2"
                        >
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem value="header" id="source_header" />
                            <Label
                              htmlFor="source_header"
                              className="font-normal"
                            >
                              Usar imagem do Cabeçalho
                            </Label>
                          </div>
                          <div className="flex items-center space-x-2">
                            <RadioGroupItem
                            ...
                            />
                            <Label
                              htmlFor="source_watermark"
                              className="font-normal"
                            >
                              Usar imagem da Marca d&apos;água
                            </Label>
                          </div>
                        </RadioGroup>
                      </div>
                ...
                <Card className="border-primary/30">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      <span>Liberação do Chat Interno</span>
                    </CardTitle>
                    <CardDescription>
                      Controle gradual de rollout: mantenha o chat só para
                      equipe interna e libere externamente quando desejar.
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3">
                      <div>
                        <p className="text-sm font-medium">
                          Permitir chat para Cliente e Representante
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Desativado: somente perfis internos acessam o chat.
                        </p>
                      </div>
                      <Switch
                        checked={Boolean(featureFlagsData?.allowExternalChat)}
                        onCheckedChange={handleToggleExternalChat}
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação não pode ser desfeita. Isso irá deletar o item
              permanentemente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
