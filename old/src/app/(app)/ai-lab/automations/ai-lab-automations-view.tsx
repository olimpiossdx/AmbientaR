...
export function AiLabAutomationsView() {
  const {
    data: brandingData,
    pdfImages,
    isPdfImagesLoading,
  ...
  if (user && user.role !== "admin") {
    return (
      <div className="flex flex-col h-full">
        <PageHeader title="Automa├º├Áes IA" />
        <main className="flex-1 overflow-auto p-4 md:p-6">
          <Card>
            <CardHeader>
              <CardTitle>Acesso restrito</CardTitle>
              <CardDescription>
                Este m├│dulo est├í dispon├¡vel apenas para administradores nesta
                fase.
              </CardDescription>
            </CardHeader>
          </Card>
  ...
  const handleGenerate = async () => {
    if (!reportTitle.trim() || !objective.trim()) {
      toast({
        ...
        description: "Informe t├¡tulo e objetivo do relat├│rio.",
      });
      return;
    }
    ...
    if (costControl.hardStopEnabled && willExceedBudget) {
      toast({
        ...
        description: `Proje├º├úo acima do teto mensal de R$ ${monthlyBudgetBRL.toFixed(2)}. Ajuste fontes/URLs ou aumente o teto.`,
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Falha ao gerar relat├│rio.");
      }
      ...
      toast({
        ...
        description: "Relat├│rio e refer├¬ncias ABNT dispon├¡veis abaixo.",
      });
    } catch (error) {
      console.error("Erro ao gerar relat├│rio IA+RAG:", error);
      toast({
        variant: "destructive",
  ...
  const handleSaveCostControl = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Or├ºamento mensal e bloqueio autom├ítico atualizados.",
      });
    } catch (error) {
      toast({
        variant: "destructive",
        ...
        description:
          error instanceof Error
            ? error.message
            : "N├úo foi poss├¡vel salvar o controle de custo.",
      });
    } finally {
  ...
  const handleExportPdf = async (title: string, report: string, refs: string[]) => {
    ...
    writeTitle(title || "Relat├│rio IA + RAG");
    writeParagraph(report || "Relat├│rio sem conte├║do.");
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main className="flex-1 overflow-auto p-4 md:p-6 space-y-4">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Objetivo do m├│dulo</CardTitle>
            <CardDescription>
              Planejar e priorizar automa├º├Áes com IA por ganho real de opera├º├úo.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <p>
              - Backlog inicial: alertas inteligentes, resumo documental e
              prioriza├º├úo de pend├¬ncias.
            </p>
            <p>
              - Crit├®rios: impacto, risco, custo por token e esfor├ºo de
              manuten├º├úo.
            </p>
            <p>
              - Rollout: piloto no hub, valida├º├úo e posterior distribui├º├úo nos
              menus de neg├│cio.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Or├ºamento mensal de IA</CardTitle>
            <CardDescription>
              Painel de custo estimado mensal com bloqueio autom├ítico por teto.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              <div className="rounded-md border p-3">
                <p className="text-muted-foreground">Gasto estimado no m├¬s</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                Ativar bloqueio autom├ítico ao atingir teto
              </label>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                {isSavingCostControl ? "Salvando..." : "Salvar or├ºamento"}
              </Button>
              {willExceedBudget && costControl.hardStopEnabled && (
                <span className="text-xs text-destructive">
                  A pr├│xima gera├º├úo ser├í bloqueada por exceder o teto mensal.
                </span>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Gerador de relat├│rio (interno + externo)</CardTitle>
            <CardDescription>
              Gera relat├│rio com base em fontes internas (RAG) e, opcionalmente,
              pesquisa externa por URL com cita├º├Áes ABNT.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-900 dark:text-amber-300">
              Guardrails de custo ativos: m├íximo de 8 fontes internas, 3 URLs
              externas e limite de tamanho por requisi├º├úo.
            </div>
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder="Ex.: Estudo t├®cnico preliminar de conformidade ambiental"
              />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label>Instru├º├Áes adicionais (opcional)</Label>
              <Textarea
                value={instructions}
                onChange={(e) => setInstructions(e.target.value)}
                placeholder="Ex.: destacar riscos regulat├│rios para MG e recomenda├º├Áes pr├íticas."
                className="min-h-20"
              />
            </div>
        ...
        {(reportResult ||
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Sa├¡da gerada com trilha de fontes para uso t├®cnico.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {lastReportProvider ? (
              ...
              <Button
                ...
                onClick={() =>
                  handleExportPdf(
                    lastReportTitle || reportTitle || "Relat├│rio IA + RAG",
                    reportResult,
                    references,
                  )
                }
              ...
              >
                Exportar PDF com refer├¬ncias ABNT
              </Button>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Relat├│rios gerados no hub para auditoria e reaproveitamento.
            </CardDescription>
          </CardHeader>
