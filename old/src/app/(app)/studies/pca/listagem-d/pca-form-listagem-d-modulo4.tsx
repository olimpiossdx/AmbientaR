    ...
    <div className="space-y-2">
      ...
      {fields.map((item, index) => (
        <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-6">
          ...
          <PcaTextField form={form} name={`${basePath}.${index}.marca`} label="Marca" />
          <PcaTextField form={form} name={`${basePath}.${index}.modelo`} label="Modelo" />
          <PcaTextField form={form} name={`${basePath}.${index}.anoFabricacao`} label="Ano de fabricação" />
          <PcaTextField form={form} name={`${basePath}.${index}.capacidade`} label={colunaExtra} />
...
export function PcaFormListagemDModulo4({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <PcaSectionCard title="12. Outras atividades não descritas">
        {outrasFields.map((item, index) => (
          <div key={item.id} className="mb-3 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-5">
            <PcaTextField form={form} name={`listagemD.outrasAtividades.${index}.atividade`} label="Atividade" />
            <PcaTextField form={form} name={`listagemD.outrasAtividades.${index}.codigo`} label="Código DN 217/17" />
            <PcaTextField form={form} name={`listagemD.outrasAtividades.${index}.unidade`} label="Unidade" />
            <PcaTextField form={form} name={`listagemD.outrasAtividades.${index}.quantidade`} label="Quantidade" />
            <PcaTextField form={form} name={`listagemD.outrasAtividades.${index}.inicioAtividade`} label="Início da atividade" />
      ...
      <PcaSectionCard title="13. Trabalhadores / empregados / funcionários">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemD.trabalhadores.duranteSafra" label="Durante a safra" />
          <PcaNumField form={form} name="listagemD.trabalhadores.foraSafra" label="Fora de safra" />
          <PcaTextField form={form} name="listagemD.trabalhadores.periodoSafra" label="Período da safra" />
        </div>
      </PcaSectionCard>

      <PcaSectionCard title="14. Área do empreendimento">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <PcaNumField form={form} name="listagemD.areaEmpreendimento.areaTotalHa" label="Área total do terreno (ha)" />
          <PcaNumField form={form} name="listagemD.areaEmpreendimento.areaUtilHa" label="Área útil do empreendimento (ha)" />
          <PcaNumField form={form} name="listagemD.areaEmpreendimento.areaPlantioCanaHa" label="Área de plantio de cana própria (ha)" />
          <PcaNumField form={form} name="listagemD.areaEmpreendimento.areaConstruidaHa" label="Área construída (ha)" />
          <PcaNumField form={form} name="listagemD.areaEmpreendimento.areaAmpliacaoM2" label="Área a construir na ampliação (m²)" />
        </div>
        <FormField
          control={form.control}
        ...
        />
        <PcaTextField form={form} name="listagemD.areaEmpreendimento.numerosLicencasPlantio" label="Nº licenças ou FOB (se aplicável)" />
      </PcaSectionCard>

      <PcaSectionCard title="15. Regime de operação">
        {['fabricacaoCachaca', 'envase'].map((key) => (
          <div key={key} className="mb-4 rounded-md border p-3">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <PcaNumField form={form} name={`listagemD.regimeOperacao.${key}.turnos`} label="Nº de turnos" />
              <PcaNumField form={form} name={`listagemD.regimeOperacao.${key}.horasTurno`} label="Horas/turno" />
              <PcaNumField form={form} name={`listagemD.regimeOperacao.${key}.diasSemana`} label="Dias/semana" />
              <PcaNumField form={form} name={`listagemD.regimeOperacao.${key}.mesesAno`} label="Meses/ano" />
            </div>
          </div>
        ))}
      </PcaSectionCard>

      <PcaSectionCard title="16. Capacidade nominal instalada">
        {[
          ...
          <div key={row.id} className="mb-3 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            ...
            <PcaNumField form={form} name={`listagemD.capacidadeInstalada.${row.id}.diario`} label={row.ud} />
            <PcaNumField form={form} name={`listagemD.capacidadeInstalada.${row.id}.anual`} label={row.ua} />
          </div>
        ))}
        <p className="text-sm text-muted-foreground">Percentual médio de utilização (somente LO corretiva):</p>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <PcaNumField form={form} name="listagemD.capacidadeInstalada.utilizacaoCachacaPct" label="Produção de cachaça (%)" />
          <PcaNumField form={form} name="listagemD.capacidadeInstalada.utilizacaoEnvasePct" label="Envase (%)" />
        </div>
      </PcaSectionCard>

      <PcaSectionCard title="17. Tipo de ocupação da área de entorno">
        <div className="space-y-2">
          {ocupacaoEntornoTipos.map((tipo) => (
            <div key={tipo.id} className="grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-3">
              ...
              <PcaNumField form={form} name={`listagemD.ocupacaoEntorno.${tipo.id}.distanciaM`} label="Distância (m)" />
            </div>
          ))}
        </div>
        <FormField
      ...
      <PcaSectionCard title="18. Recursos hídricos">
        <PcaTextField
          form={form}
      ...
      <PcaSectionCard title="20. Matérias-primas e insumos">
        {materiasFields.map((item, index) => (
          <div key={item.id} className="mb-3 grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-3">
            <PcaTextField form={form} name={`listagemD.materiasPrimas.linhas.${index}.nome`} label="Nome técnico/comercial" />
            <PcaTextField form={form} name={`listagemD.materiasPrimas.linhas.${index}.embalagem`} label="Tipo de embalagem" />
            <PcaTextField form={form} name={`listagemD.materiasPrimas.linhas.${index}.armazenamento`} label="Local de armazenamento" />
            <PcaNumField form={form} name={`listagemD.materiasPrimas.linhas.${index}.consumoMax`} label="Consumo mensal máximo" />
            <PcaNumField form={form} name={`listagemD.materiasPrimas.linhas.${index}.consumoMedio`} label="Consumo mensal médio" />
            <div className="flex items-end justify-end">
      ...
      <PcaSectionCard title="21. Forma de armazenamento">
      ...
      <PcaSectionCard title="22. Produtos">
        {produtosFields.map((item, index) => (
          <div key={item.id} className="mb-2 flex gap-2">
            <PcaTextField form={form} name={`listagemD.produtos.linhas.${index}.nome`} label="Nome comercial" />
      ...
      <PcaSectionCard title="23. Subprodutos">
        {subprodutosFields.map((item, index) => (
          <div key={item.id} className="mb-3 grid grid-cols-1 gap-2 md:grid-cols-2">
            <PcaTextField form={form} name={`listagemD.subprodutos.linhas.${index}.nome`} label="Subproduto" />
            <PcaNumField form={form} name={`listagemD.subprodutos.linhas.${index}.producaoAnualL`} label="Produção anual média (L/ano)" />
      ...
      <PcaSectionCard title="24. Equipamentos do processo industrial">
      ...
      <PcaSectionCard title="26. Insumos principais – água">
        ...
        {fontesAgua.map((f) => (
          <div key={f.id} className="mb-2 grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-4">
            ...
            <PcaNumField form={form} name={`listagemD.insumos.agua.fontes.${f.id}.consumoMax`} label="Consumo máx. (m³/mês)" />
            <PcaNumField form={form} name={`listagemD.insumos.agua.fontes.${f.id}.consumoMedio`} label="Consumo médio" />
            <PcaTextField form={form} name={`listagemD.insumos.agua.fontes.${f.id}.detalhe`} label="Nome / especificação" />
          </div>
        ))}
        ...
        {finalidadesAguaD.map((fin) => (
          <div key={fin.id} className="mb-2 grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-4">
            ...
            <PcaNumField form={form} name={`listagemD.insumos.agua.finalidades.${fin.id}.maxima`} label="Qtd. máxima (m³/mês)" />
            <PcaNumField form={form} name={`listagemD.insumos.agua.finalidades.${fin.id}.media`} label="Qtd. média" />
            <PcaTextField form={form} name={`listagemD.insumos.agua.finalidades.${fin.id}.origem`} label="Origem" />
          </div>
        ))}
        <FormField
          control={form.control}
