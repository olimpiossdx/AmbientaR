...
function EmissaoMatrix({
  ...
  return (
    <div className="space-y-4">
      ...
      <PcaTextField form={form} name={`${basePath}.outrasFontes`} label="Outras fontes – especificar" />
    </div>
  );
}

export function PcaFormListagemATecnicoSecoes({ form, parte = 'completo' }: FormListagemATecnicoProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <>
      {/* 27 */}
      <PcaSectionCard title="27. ENERGÉTICO">
        <PcaBooleanRadio form={form} name="listagemA.energetico.utilizaEnergia" label="O empreendimento utiliza quantitativamente energia?" />
        {utilizaEnergia && (
          <>
            <PcaCheckboxOptions
              form={form}
            ...
            {usaGeracaoDiesel && (
              <div className="grid grid-cols-1 gap-4 rounded-md border p-3 md:grid-cols-2">
                <PcaTextField form={form} name="listagemA.energetico.diesel.manutencaoFrequente" label="Manutenção frequente do gerador" />
                <PcaNumField form={form} name="listagemA.energetico.diesel.capacidadeGeradorKva" label="Capacidade do gerador (kVA)" />
                <PcaNumField form={form} name="listagemA.energetico.diesel.consumoMensalL" label="Consumo mensal (L)" />
                <PcaNumField form={form} name="listagemA.energetico.diesel.potenciaInstaladaKw" label="Potência instalada (kW)" />
              </div>
            )}
            {usaRedeEletrica && (
              <div className="grid grid-cols-1 gap-4 rounded-md border p-3 md:grid-cols-2">
                <PcaNumField form={form} name="listagemA.energetico.rede.energiaComercialMensalKwh" label="Energia comercial mensal (kWh)" />
                <PcaNumField form={form} name="listagemA.energetico.rede.potenciaInstaladaConsumoKw" label="Potência instalada consumo (kW)" />
                <PcaNumField form={form} name="listagemA.energetico.rede.potenciaBtKw" label="Potência BT (kW)" />
                <PcaNumField form={form} name="listagemA.energetico.rede.demandaKw" label="Demanda (kW)" />
                <PcaTextField form={form} name="listagemA.energetico.rede.barraReferencia" label="Barra de referência" />
                <PcaTextField form={form} name="listagemA.energetico.rede.tensao" label="Tensão" />
              </div>
            )}
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <PcaNumField form={form} name="listagemA.energetico.geracaoIndireta.consumoMensal" label="Geração indireta – consumo mensal" />
              <PcaNumField form={form} name="listagemA.energetico.geracaoTerceiros.consumoMensal" label="Geração terceiros – consumo mensal" />
              <PcaNumField form={form} name="listagemA.energetico.geracaoTerceiros.demanda" label="Geração terceiros – demanda" />
              <PcaTextField form={form} name="listagemA.energetico.outrasEspecificar" label="Outras – especificar" />
            </div>
          </>
        )}
      </PcaSectionCard>
      ...
      {/* 28 */}
      <PcaSectionCard title="28. DESENVOLVIMENTO DE BARRAGEM DE DESMONTE">
        <PcaBooleanRadio form={form} name="listagemA.barragemDesmonte.haBarragem" label="Há barragem de desmonte no empreendimento?" />
        {haBarragemDesmonte && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <PcaTextField form={form} name="listagemA.barragemDesmonte.identificacao" label="Identificação da barragem" />
            <PcaTextField form={form} name="listagemA.barragemDesmonte.tipo" label="Tipo de barragem" />
            <PcaTextField form={form} name="listagemA.barragemDesmonte.classificacaoRisco" label="Classificação de risco" />
            <PcaNumField form={form} name="listagemA.barragemDesmonte.alturaMaximaM" label="Altura máxima (m)" />
            <PcaNumField form={form} name="listagemA.barragemDesmonte.comprimentoCristaM" label="Comprimento da crista (m)" />
            <PcaNumField form={form} name="listagemA.barragemDesmonte.volumeProjetoM3" label="Volume de projeto (m³)" />
            <PcaNumField form={form} name="listagemA.barragemDesmonte.areaReservatorioM2" label="Área do reservatório (m²)" />
            <PcaTextField form={form} name="listagemA.barragemDesmonte.situacaoLicenciamento" label="Situação de licenciamento" />
            <PcaTextField form={form} name="listagemA.barragemDesmonte.processoDnpm" label="Processo DNPM" />
            <PcaTextField form={form} name="listagemA.barragemDesmonte.medidasControle" label="Medidas de controle" className="md:col-span-3" />
            <PcaTextAreaField form={form} name="listagemA.barragemDesmonte.observacoes" label="Observações complementares" />
          </div>
        )}
      </PcaSectionCard>

      {/* 29 */}
      <PcaSectionCard title="29. AUTORIZAÇÃO E LICENCIAMENTO PARA REFLORESTAMENTO">
        <PcaBooleanRadio form={form} name="listagemA.reflorestamento.houveSupressao" label="Houve supressão de vegetação para implantação da atividade?" />
        {houveSupressao && (
          <>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <PcaNumField form={form} name="listagemA.reflorestamento.areaSuprimidaHa" label="Área suprimida (ha)" />
              <PcaTextField form={form} name="listagemA.reflorestamento.autorizacaoSupressao" label="Autorização de supressão" />
              <PcaNumField form={form} name="listagemA.reflorestamento.areaReflorestamentoHa" label="Área de reflorestamento (ha)" />
            </div>
            {especiesReflorestamento.map((item, index) => (
              <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
                <PcaTextField form={form} name={`listagemA.reflorestamento.especies.${index}.especie`} label="Espécie" />
                <PcaNumField form={form} name={`listagemA.reflorestamento.especies.${index}.quantidade`} label="Quantidade" />
                <PcaTextField form={form} name={`listagemA.reflorestamento.especies.${index}.origem`} label="Origem do material" />
      ...
      <PcaSectionCard title="30. LAVRA">
        ...
        />
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemA.lavra.profundidadeMaximaM" label="Profundidade máxima (m)" />
          <PcaNumField form={form} name="listagemA.lavra.areaLavraHa" label="Área de lavra (ha)" />
          <PcaNumField form={form} name="listagemA.lavra.producaoDiariaT" label="Produção diária (t/dia)" />
          <PcaBooleanRadio form={form} name="listagemA.lavra.utilizaDetonacao" label="Utiliza detonação?" />
          <PcaTextField form={form} name="listagemA.lavra.frequenciaDetonacao" label="Frequência de detonação" />
          <PcaTextField form={form} name="listagemA.lavra.controleEmissoesDetonacao" label="Controle de emissões na detonação" />
        </div>
        <PcaTextAreaField form={form} name="listagemA.lavra.descricaoOperacional" label="Descrição operacional da lavra" />
      </PcaSectionCard>

      {/* 31 */}
      <PcaSectionCard title="31. PROCESSO DE BENEFICIAMENTO">
        <PcaCheckboxOptions
          form={form}
        ...
        />
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemA.beneficiamento.capacidadeInstaladaTpd" label="Capacidade instalada (t/dia)" />
          <PcaNumField form={form} name="listagemA.beneficiamento.consumoAguaM3Dia" label="Consumo de água (m³/dia)" />
          <PcaNumField form={form} name="listagemA.beneficiamento.consumoEnergiaKwhMes" label="Consumo de energia (kWh/mês)" />
        </div>
        <PcaTextField form={form} name="listagemA.beneficiamento.outrosEspecificar" label="Outros – especificar" />
      </PcaSectionCard>

      {/* 32 */}
      <PcaSectionCard title="32. CARACTERIZAÇÃO DO REJEITO">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaTextField form={form} name="listagemA.caracterizacaoRejeito.tipoRejeito" label="Tipo de rejeito" />
          <PcaNumField form={form} name="listagemA.caracterizacaoRejeito.producaoDiariaT" label="Produção diária (t/dia)" />
          <PcaNumField form={form} name="listagemA.caracterizacaoRejeito.umidadePercentual" label="Umidade (%)" />
          <PcaTextField form={form} name="listagemA.caracterizacaoRejeito.destinacao" label="Destinação" />
          <PcaTextField form={form} name="listagemA.caracterizacaoRejeito.armazenamento" label="Forma de armazenamento" className="md:col-span-2" />
        </div>
        <PcaTextAreaField form={form} name="listagemA.caracterizacaoRejeito.caracterizacaoGeoquimica" label="Caracterização geoquímica" />
      </PcaSectionCard>

      {/* 33 */}
      <PcaSectionCard title="33. MEIOS DEMONSTRATIVOS – ÁREAS CÁRSTICAS">
        <PcaBooleanRadio form={form} name="listagemA.areasCarsticas.localizadoEmAreaCarstica" label="Empreendimento localizado em área cárstica?" />
        <PcaTextField form={form} name="listagemA.areasCarsticas.meiosDemonstrativos" label="Meios demonstrativos de não invasão" className="md:col-span-2" />
      </PcaSectionCard>

      {/* 34 */}
      <PcaSectionCard title="34. CONSTRUÇÃO E OPERAÇÃO DE ESTÁCIO">
        <PcaBooleanRadio form={form} name="listagemA.estacio.possuiEstacio" label="Possui estácio de armazenamento?" />
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaNumField form={form} name="listagemA.estacio.areaHa" label="Área (ha)" />
          <PcaNumField form={form} name="listagemA.estacio.capacidadeM3" label="Capacidade (m³)" />
          <PcaTextField form={form} name="listagemA.estacio.tipoEstacio" label="Tipo de estácio" />
        </div>
      </PcaSectionCard>

      {/* 35 */}
      <PcaSectionCard title="35. COMPRESSORES DE AR">
        <FormField
          control={form.control}
        ...
        {form.watch('listagemA.compressores.possui') && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <PcaNumField form={form} name="listagemA.compressores.capacidadeNominalM3h" label="Capacidade nominal de compressão (m³/h)" />
            <PcaNumField form={form} name="listagemA.compressores.volumeAguaPurgaLdia" label="Volume de água de purga gerada (L/dia)" />
          </div>
        )}
      </PcaSectionCard>

      {/* 36 */}
      <PcaSectionCard title="36. ABASTECIMENTO DE COMBUSTÍVEIS">
        <FormField
          control={form.control}
        ...
        {form.watch('listagemA.abastecimento.possui') && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <PcaNumField form={form} name="listagemA.abastecimento.capacidadeArmazenamentoM3" label="Capacidade de armazenamento (m³)" />
            <PcaBooleanRadio form={form} name="listagemA.abastecimento.cumpreDn108" label="Cumpre integralmente a DN 108/2007?" />
          </div>
        )}
      </PcaSectionCard>
      </>
      ...
      <>
      {/* 37 */}
      <PcaSectionCard title="37. TRATAMENTO DE ÁGUA INDUSTRIAL DAS UNIDADES AUXILIARES DA LAVRA">
        <PcaBooleanRadio form={form} name="listagemA.tratamentoAguaIndustrial.promoveReuso" label="O empreendimento irá promover o reuso da água de processo?" />
        {reusoAguaIndustrial && (
          <PcaBooleanRadio form={form} name="listagemA.tratamentoAguaIndustrial.necessitaEta" label="Para o reuso, será necessário tratamento através de ETA industrial?" />
        )}
        {reusoAguaIndustrial && necessitaEtaIndustrial && (
          <>
            ...
            />
            <PcaCheckboxOptions form={form} name="listagemA.tratamentoAguaIndustrial.etapas" options={etapasTratamentoAgua} />
            <PcaTextField form={form} name="listagemA.tratamentoAguaIndustrial.etapasOutros" label="Outros – especificar" />
            <PcaNumField form={form} name="listagemA.tratamentoAguaIndustrial.capacidadeNominalM3h" label="Capacidade nominal da nova ETA industrial (m³/h)" />
          </>
        )}
      </PcaSectionCard>

      {/* 38 */}
      <PcaSectionCard title="38. SISTEMAS E EDIFICAÇÕES DAS UNIDADES AUXILIARES DA LAVRA">
        {[
      ...
      {/* 39 */}
      <PcaSectionCard title="39. USO DE ÁGUA">
        <PcaBooleanRadio form={form} name="listagemA.usoAgua.recirculaAgua" label="O empreendimento recircula a água utilizada?" />
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <PcaNumField form={form} name="listagemA.usoAgua.volumeRecirculadoM3Mes" label="Volume recirculado (m³/mês)" />
            <PcaNumField form={form} name="listagemA.usoAgua.percentualRecirculada" label="Porcentagem de água recirculada (%)" />
          </div>
        )}
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>
                <PcaNumField form={form} name="listagemA.usoAgua.consumoTotalMaximo" label="" />
              </TableCell>
              <TableCell>
                <PcaNumField form={form} name="listagemA.usoAgua.consumoTotalMedio" label="" />
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
        <PcaTextField form={form} name="listagemA.usoAgua.outrasFinalidadesEspecificar" label="Outras finalidades – especificar" />
      </PcaSectionCard>

      {/* 40 */}
      <PcaSectionCard title="40. TRATAMENTO DE ÁGUA NOVA">
        <PcaBooleanRadio form={form} name="listagemA.tratamentoAguaNova.trataAgua" label="O empreendimento trata água?" />
        {trataAguaNova && (
          <>
            <PcaNumField form={form} name="listagemA.tratamentoAguaNova.quantidadeTratadaM3Mes" label="Quantidade de água tratada (m³/mês)" />
            <PcaCheckboxOptions form={form} name="listagemA.tratamentoAguaNova.etapas" options={etapasTratamentoAgua} />
            <PcaTextField form={form} name="listagemA.tratamentoAguaNova.etapasOutros" label="Outros – especificar" />
          </>
        )}
      </PcaSectionCard>

      {/* 41 */}
      <PcaSectionCard title="41. EFLUENTES LÍQUIDOS GERADOS">
        <FormDescription>
          Marque fonte, efluente e medida de controle conforme o formulário de referência.
        </FormDescription>
        <PcaTextAreaField form={form} name="listagemA.efluentesLiquidos.descricao" label="Descrição dos efluentes e medidas de controle" />
        <PcaCheckboxOptions
          form={form}
      ...
      {/* 42 */}
      <PcaSectionCard title="42. EFLUENTES SANITÁRIOS">
        <PcaNumField form={form} name="listagemA.efluentesSanitarios.volumeM3Dia" label="Volume do efluente sanitário gerado (m³/dia)" />
        <PcaNumField form={form} name="listagemA.efluentesSanitarios.qtdSistemasTratamento" label="Quantidade de sistemas de tratamento receptores" />
        <p className="text-sm font-medium">Identificação e descrição de todos os pontos:</p>
        {pontosSanitarios.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemA.efluentesSanitarios.pontos.${index}.identificacao`} label="Identificação" />
            <PcaTextField form={form} name={`listagemA.efluentesSanitarios.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemA.efluentesSanitarios.pontos.${index}.volumeTratadoM3Dia`} label="Volume tratado (m³/dia)" />
      ...
      {/* 43 */}
      <PcaSectionCard title="43. EFLUENTES DE REFEITÓRIO">
        <PcaNumField form={form} name="listagemA.efluentesRefeitorio.volumeM3Dia" label="Volume do efluente gerado em refeitório (m³/dia)" />
        <PcaNumField form={form} name="listagemA.efluentesRefeitorio.qtdSistemasTratamento" label="Quantidade de sistemas de tratamento receptores" />
        {pontosRefeitorio.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemA.efluentesRefeitorio.pontos.${index}.identificacao`} label="Identificação" />
            <PcaTextField form={form} name={`listagemA.efluentesRefeitorio.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemA.efluentesRefeitorio.pontos.${index}.volumeTratadoM3Dia`} label="Volume tratado (m³/dia)" />
            <div className="md:col-span-4 flex justify-end">
      ...
      {/* 44 */}
      <PcaSectionCard title="44. EFLUENTES OLEOSOS">
        <PcaNumField form={form} name="listagemA.efluentesOleosos.volumeM3Dia" label="Volume do efluente oleoso gerado (m³/dia)" />
        <PcaNumField form={form} name="listagemA.efluentesOleosos.qtdSistemasTratamento" label="Quantidade de sistemas de tratamento receptores" />
        {pontosOleosos.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemA.efluentesOleosos.pontos.${index}.identificacao`} label="Identificação" />
            <PcaTextField form={form} name={`listagemA.efluentesOleosos.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemA.efluentesOleosos.pontos.${index}.volumeTratadoM3Dia`} label="Volume tratado (m³/dia)" />
            <div className="md:col-span-4 flex justify-end">
      ...
      {/* 45 */}
      <PcaSectionCard title="45. EFLUENTES INDUSTRIAIS">
        <PcaNumField form={form} name="listagemA.efluentesIndustriais.volumeM3Dia" label="Volume do efluente industrial gerado (m³/dia)" />
        <PcaNumField form={form} name="listagemA.efluentesIndustriais.qtdSistemasTratamento" label="Quantidade de sistemas de tratamento de efluentes industriais" />
        {pontosIndustriais.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemA.efluentesIndustriais.pontos.${index}.identificacao`} label="Identificação" />
            <PcaTextField form={form} name={`listagemA.efluentesIndustriais.pontos.${index}.descricao`} label="Descrição" className="md:col-span-2" />
            <PcaNumField form={form} name={`listagemA.efluentesIndustriais.pontos.${index}.volumeTratadoM3Dia`} label="Volume tratado (m³/dia)" />
            <div className="md:col-span-4 flex justify-end">
      ...
      {/* 46 */}
      <PcaSectionCard title="46. CARACTERÍSTICAS DE CADA EFLUENTE INDUSTRIAL BRUTO – Antes do Tratamento">
        <PcaTextField form={form} name="listagemA.caracteristicasEfluenteBruto.efluenteIdDescricao" label="Efluente (ID/descrição)" />
        <Table>
          <TableHeader>
            <TableRow>
        ...
        {outrosParametros.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`listagemA.caracteristicasEfluenteBruto.outrosParametros.${index}.parametro`} label="Parâmetro" />
            <PcaTextField form={form} name={`listagemA.caracteristicasEfluenteBruto.outrosParametros.${index}.unidade`} label="Unidade" />
            <PcaNumField form={form} name={`listagemA.caracteristicasEfluenteBruto.outrosParametros.${index}.minimo`} label="Mínimo" />
            <PcaNumField form={form} name={`listagemA.caracteristicasEfluenteBruto.outrosParametros.${index}.maximo`} label="Máximo" />
            <div className="md:col-span-4 flex justify-end">
      ...
      {/* 47 */}
      <PcaSectionCard title="47. TRATAMENTO DOS EFLUENTES SANITÁRIOS">
        <PcaCheckboxOptions
          form={form}
        ...
        />
        <PcaTextField form={form} name="listagemA.tratamentoEfluentesSanitarios.outrosListar" label="Outros – listar" />
      </PcaSectionCard>

      {/* 48 */}
      <PcaSectionCard title="48. TRATAMENTO DE EFLUENTES OLEOSOS">
        <PcaCheckboxOptions
          form={form}
        ...
        />
        <PcaTextField form={form} name="listagemA.tratamentoEfluentesOleosos.outrosEspecificar" label="Outros – especificar" />
      </PcaSectionCard>

      {/* 49 */}
      <PcaSectionCard title="49. TRATAMENTO DE EFLUENTES INDUSTRIAIS">
        <PcaBooleanRadio form={form} name="listagemA.tratamentoEfluentesIndustriais.necessitaTratamento" label="Será necessário tratamento de efluentes para lançamento em curso d&apos;água?" />
        {necessitaTratamentoEfluente && (
          <>
            ...
            />
            <PcaTextField form={form} name="listagemA.tratamentoEfluentesIndustriais.etapasOutros" label="Outros – descrever" />
            <PcaNumField form={form} name="listagemA.tratamentoEfluentesIndustriais.capacidadeNominalM3h" label="Capacidade nominal da nova ETE (m³/h)" />
          </>
        )}
      </PcaSectionCard>

      {/* 50 */}
      <PcaSectionCard title="50. DESTINO FINAL DOS EFLUENTES">
        <PcaCheckboxOptions
          form={form}
        ...
        />
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <PcaTextField form={form} name="listagemA.destinoFinalEfluentes.nomeCorpoHidrico" label="Nome do corpo hídrico" />
          <PcaCheckboxOptions
            form={form}
          ...
          />
          <PcaBooleanRadio form={form} name="listagemA.destinoFinalEfluentes.municipioTrataEsgoto" label="Município possui tratamento de esgoto urbano?" />
          <PcaNumField form={form} name="listagemA.destinoFinalEfluentes.percentualEsgotoTratado" label="Porcentagem do esgoto tratado (%)" />
          <PcaTextField form={form} name="listagemA.destinoFinalEfluentes.outroEspecificar" label="Outro – especificar" className="md:col-span-2" />
        </div>
      </PcaSectionCard>

      {/* 51 */}
      <PcaSectionCard title="51. EMISSÕES ATMOSFÉRICAS E MEDIDAS DE CONTROLE">
        <EmissaoMatrix form={form} basePath="listagemA.emissoesAtmosfericas" fontes={fontesEmissaoAtmosferica} />
      </PcaSectionCard>

      {/* 52 */}
      <PcaSectionCard title="52. RUÍDOS, VIBRAÇÕES E MEDIDAS DE CONTROLE">
        <EmissaoMatrix form={form} basePath="listagemA.ruidosVibracoes" fontes={fontesRuidoVibracao} />
      </PcaSectionCard>

      {/* 53 */}
      <PcaSectionCard title="53. RESÍDUOS SÓLIDOS">
        <Table>
          <TableHeader>
            <TableRow>
        ...
        <PcaTextField form={form} name="listagemA.residuosSolidos.outrosEspecificar" label="Outros resíduos – especificar" />
      </PcaSectionCard>
      </>
      ) : null}
      ...
      <>
      {/* 57 */}
      <PcaSectionCard title="57. QUALIDADE DAS ÁGUAS SUBTERRÂNEAS">
        <PcaTextAreaField form={form} name="listagemA.qualidadeAguaSubterranea.permeabilidadeSolo" label="Permeabilidade do solo (k)" />
        <PcaBooleanRadio form={form} name="listagemA.qualidadeAguaSubterranea.realizouLevantamento" label="Foi realizado levantamento da qualidade das águas subterrâneas?" />
        {levantamentoAguaSub && (
          <Table>
            <TableHeader>
              <TableRow>
      ...
      {/* 58 */}
      <PcaSectionCard title="58. QUALIDADE DAS ÁGUAS SUPERFICIAIS">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Parâmetro</TableHead>
      ...
      {/* 59 */}
      <PcaSectionCard title="59. PASSIVOS AMBIENTAIS">
        <PcaBooleanRadio form={form} name="listagemA.passivosAmbientais.existePassivo" label="Há passivo ambiental associado ao empreendimento requerente?" />
        {haPassivo && (
          <PcaTextAreaField form={form} name="listagemA.passivosAmbientais.descricao" label="Informar passivos existentes e alternativas de intervenção" />
        )}
