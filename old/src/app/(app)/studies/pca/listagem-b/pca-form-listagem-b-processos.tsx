...
export function PcaFormListagemBProcessos({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      <PcaSectionCard title="33. Uso de água">
        ...
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <PcaNumField form={form} name="listagemB.usoAguaFundicao.volumeRecirculadoM3Mes" label="Volume recirculado (m³/mês)" />
            <PcaNumField form={form} name="listagemB.usoAguaFundicao.percentualRecirculado" label="Porcentagem de água recirculada (%)" />
          </div>
        )}
        ...
        <Table>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>
                <PcaNumField form={form} name="listagemB.usoAguaFundicao.consumoTotalMaximo" label="" />
              </TableCell>
              <TableCell>
                <PcaNumField form={form} name="listagemB.usoAguaFundicao.consumoTotalMedio" label="" />
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
      ...
      <PcaSectionCard title="34. Linha de produção do empreendimento">
        <Table>
          <TableHeader>
            <TableRow>
        ...
        {linhasProducao.map((item, index) => (
          <div key={item.id} className="mt-3 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            <PcaTextField form={form} name={`listagemB.linhaProducao.itens.${index}.segmento`} label="Segmento (outros)" />
            <PcaTextField form={form} name={`listagemB.linhaProducao.itens.${index}.produto`} label="Produto" className="md:col-span-2" />
      ...
      <PcaSectionCard title="35. Produção diária">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <PcaNumField form={form} name="listagemB.producaoDiaria.pesoMinimoKg" label="Peso unitário mínimo (Kg)" />
          <PcaNumField form={form} name="listagemB.producaoDiaria.pesoIdealKg" label="Peso unitário ideal (Kg)" />
          <PcaNumField form={form} name="listagemB.producaoDiaria.pesoMaximoKg" label="Peso unitário máximo (Kg)" />
          <PcaNumField form={form} name="listagemB.producaoDiaria.producaoDiariaPecasKg" label="Produção diária peças acabadas (Kg)" />
        </div>
      </PcaSectionCard>

      <PcaSectionCard title="36. Processo de fusão">
        {combustiveisFusao.map((comb) => (
          <div key={comb.id} className="space-y-2 rounded-md border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
              ...
              />
              <PcaNumField form={form} name={`listagemB.fusao.${comb.id}.numFornos`} label="Nº de fornos" />
              <FormField
                control={form.control}
              ...
              />
              <PcaNumField form={form} name={`listagemB.fusao.${comb.id}.capacidadeTh`} label="Capacidade (T/h)" />
            </div>
          </div>
        ))}
        <PcaTextField form={form} name="listagemB.fusao.outrosCombustiveis" label="Outros combustíveis – especificar" />
      </PcaSectionCard>
      ...
      <PcaSectionCard title="38. Processo de macharia">
        ...
        />
        <PcaTextField form={form} name="listagemB.macharia.processosOutros" label="Outros – descrever" />
        {['ferroFundido', 'aco'].map((material) => (
          <div key={material} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            ...
            />
            <PcaTextField form={form} name={`listagemB.macharia.${material}.modeloMaquina`} label="Modelo/tipo de máquina" />
            <PcaNumField form={form} name={`listagemB.macharia.${material}.numMaquinas`} label="Nº de máquinas" />
            <PcaNumField form={form} name={`listagemB.macharia.${material}.produtividadeMachosH`} label="Produtividade machos/h" />
          </div>
        ))}
      </PcaSectionCard>
      ...
      <PcaSectionCard title="39. Processo de moldagem">
        ...
        />
        <PcaTextField form={form} name="listagemB.moldagem.tiposOutros" label="Outros – descrever" />
        {['ferroFundido', 'aco'].map((material) => (
          <div key={material} className="space-y-3 rounded-md border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <PcaTextField form={form} name={`listagemB.moldagem.${material}.modeloMaquina`} label="Modelo/tipo de máquina" />
              <PcaNumField form={form} name={`listagemB.moldagem.${material}.numMaquinas`} label="Nº de máquinas" />
              <PcaNumField form={form} name={`listagemB.moldagem.${material}.produtividadeMoldesH`} label="Produtividade moldes/h" />
            </div>
          </div>
        ))}
      </PcaSectionCard>

      <PcaSectionCard title="40. Tratamento térmico das peças fundidas">
        <PcaCheckboxOptions
          form={form}
        ...
        <PcaTextField form={form} name="listagemB.tratamentoTermico.outros" label="Outros – especificar" />
      </PcaSectionCard>
      ...
      <PcaSectionCard title="43. Processo de pintura das peças fundidas">
        ...
        {['primer', 'epoxi', 'verniz', 'outros'].map((tipo) => (
          <div key={tipo} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            ...
            />
            <PcaNumField form={form} name={`listagemB.pintura.${tipo}.consumoMensalL`} label="Consumo mensal (L)" />
          </div>
        ))}
      </PcaSectionCard>

      <PcaSectionCard title="44. Tratamento químico superficial das peças fundidas">
        ...
        />
        <PcaNumField form={form} name="listagemB.tratamentoQuimico.volumeMensalKg" label="Volume mensal de peças tratadas (Kg)" />
      </PcaSectionCard>
    </div>
  );
}
