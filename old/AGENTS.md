...
- **Performance / organização (check rápido):** `npm run perf:check` — `typecheck` + `audit:routes` + auditoria de imports pesados (`@turf/turf`, `run-wave-a-analysis`) em ficheiros `"use client"`. Conferência por fase: `npm run perf:phase-debug`; smoke HTTP (dev `:9002`): `npm run perf:smoke-http`. Após `npm run build`, se o dev der 500 em vendor-chunks: `npm run dev:clean-cache` e reiniciar o dev. Roadmap e registo: [`docs/PERF-ROADMAP-DETALHADO.md`](docs/PERF-ROADMAP-DETALHADO.md), [`docs/PERF-AUDIT.md`](docs/PERF-AUDIT.md). Medição de bundle: `npm run build` (heap 8 GB se OOM: `NODE_OPTIONS=--max-old-space-size=8192`), `npm run analyze`, `npm run apphosting:check`. **PRs de performance:** uma fase do roadmap (F04–F18) por PR — não misturar Bloco 2 (peso browser) com Bloco 3 (FormShell) na mesma PR.
