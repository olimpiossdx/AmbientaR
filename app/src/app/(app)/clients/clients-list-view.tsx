...
export function ClientsListView() {
  ...
  /** Representante: clientes encontrados por CPF/CNPJ (aprova├º├úo no empreendedor ou access_requests), quando o doc em `clients` n├úo tem approvedUserIds. */
  ...
  // Representante: unir clientes com approvedUserIds + clientes do titular pelo mesmo CPF/CNPJ (espelha l├│gica de empreendedores).
  useEffect(() => {
  ...
  const handleSyncEmpreendedoresToClients = async () => {
    ...
    try {
      ...
      const chunkSize = 400; // margem de seguran├ºa do limite de 500 opera├º├Áes por batch
      ...
      if (linkedExisting > 0) {
        parts.push(
          `${linkedExisting} vinculado(s) a cliente(s) j├í existente(s) (sem duplicar).`,
        );
      }
      ...
      toast({
        ...
        description:
          "N├úo foi poss├¡vel sincronizar os empreendedores em clientes.",
      });
    } finally {
          ...
          {clientToView && (
            <div className="form-scroll-body max-h-[60vh] space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <DetailItem label="Munic├¡pio" value={clientToView.municipio} />
