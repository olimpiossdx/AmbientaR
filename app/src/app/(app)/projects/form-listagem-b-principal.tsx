...
export function FormListagemBPrincipal({ form, hideEspecifico, slotEspecifico }: FormListagemBComumProps) {
  ...
  const { fields: atividadePrincipalFields, append: appendAtividadePrincipal, remove: removeAtividadePrincipal } =
    useFieldArray({ control: form.control, name: 'listagemB.atividadesPrincipal' });
  const { fields: outrasAtividadesFields, append: appendOutrasAtividades, remove: removeOutrasAtividades } =
    useFieldArray({ control: form.control, name: 'listagemB.outrasAtividades' });
  const { fields: nucleoPopulacionalFields, append: appendNucleoPopulacional, remove: removeNucleoPopulacional } =
    useFieldArray({ control: form.control, name: 'listagemB.legislacaoMunicipal.nucleosPopulacionais' });
  const { fields: ocupacaoEntornoFields, append: appendOcupacaoEntorno, remove: removeOcupacaoEntorno } =
    useFieldArray({ control: form.control, name: 'listagemB.ocupacaoEntorno.ocorrencias' });
  const { fields: recursosHidricosFields, append: appendRecursosHidricos, remove: removeRecursosHidricos } =
    useFieldArray({ control: form.control, name: 'listagemB.recursosHidricos.intervencoes' });
  const { fields: turnosFields, append: appendTurno, remove: removeTurno } =
    useFieldArray({ control: form.control, name: 'listagemB.regimeOperacao.turnos' });
  return (
