...
export function FormListagemBFerroligasProcessos({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      <SectionCard title="34. Uso de água">
        <FormField
          control={form.control}
        ...
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <NumField form={form} name={`${base}.usoAgua.percentualRecirculado`} label="Percentual de água recirculada (%)" />
          </div>
        )}
              ...
              {finalidadesAgua.map((f) => {
                ...
                return (
                  <TableRow key={slug}>
                    ...
                    <TableCell>
                      <NumField form={form} name={`${base}.usoAgua.consumo.${slug}.maximo`} label="" />
                    </TableCell>
                    <TableCell>
                      <NumField form={form} name={`${base}.usoAgua.consumo.${slug}.medio`} label="" />
                    </TableCell>
                  </TableRow>
                );
              })}
      ...
      <SectionCard title="35.1 Processo de redução">
        <CheckboxOptions
          form={form}
      ...
      <SectionCard title="35.2 Ligas ferrosas produzidas">
        {ligasProduzidas.map((liga) => (
          <div key={liga.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
            ...
            <TextField form={form} name={`${base}.ligasProduzidas.${liga.id}.formaComercializacao`} label="Forma de comercialização (granulometria mm)" />
            <NumField form={form} name={`${base}.ligasProduzidas.${liga.id}.producaoDiariaTon`} label="Produção diária (t/dia)" />
            {liga.id === 'outras' && (
              <TextField form={form} name={`${base}.ligasProduzidas.${liga.id}.especificacao`} label="Especificar liga" className="md:col-span-2" />
            )}
          </div>
        ))}
      </SectionCard>

      <SectionCard title="35.3 Especificações dos fornos de redução">
        <FormDescription>
        ...
        {fornos.map((item, index) => (
          <div key={item.id} className="space-y-3 rounded-md border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <TextField form={form} name={`${base}.fornosReducao.${index}.identificacao`} label="Identificação do forno" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.volumeInternoM3`} label="Volume interno (m³)" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.energiaKwhPorTon`} label="Energia elétrica (kWh/t)" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.redutorCarvaoKgPorTon`} label="Redutor – carvão vegetal (kg/t)" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.redutorCoqueKgPorTon`} label="Redutor – coque (kg/t)" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.redutorMetalKgPorTon`} label="Redutor – metal (kg/t)" />
              <NumField form={form} name={`${base}.fornosReducao.${index}.capacidadeProdutivaTonDia`} label="Capacidade produtiva (t/dia)" />
            </div>
          </div>
        ))}
      ...
      <SectionCard title="36. Fluxograma de produção">
        <FormDescription>
          Apresentar no Anexo XXX os fluxogramas e descrições detalhadas de cada etapa dos processos industriais.
        </FormDescription>
        <TextField form={form} name={`${base}.fluxograma.referenciaAnexo`} label="Referência / observações ao anexo" />
      </SectionCard>

      <SectionCard title="37. Máquinas e equipamentos principais">
        {etapasProcesso.map((etapa) => {
          ...
          return (
            <div key={slug} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
              ...
              <TextField form={form} name={`${base}.equipamentos.${slug}.maquina`} label="Máquina / equipamento" className="md:col-span-2" />
              <NumField form={form} name={`${base}.equipamentos.${slug}.quantidade`} label="Quantidade" />
              <FormField
                control={form.control}
      ...
      <SectionCard title="38. Equipamentos ou sistemas para geração de energia elétrica">
        <FormField
          control={form.control}
        ...
        />
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <TextField form={form} name={`${base}.energiaEletrica.empresaFornecedora`} label="Empresa fornecedora" />
        ...
        />
        <NumField form={form} name={`${base}.energiaEletrica.tensaoSubestacaoKv`} label="Tensão da subestação (kV)" />
      </SectionCard>

      <SectionCard title="39. Equipamentos de geração de ar comprimido">
        {compressores.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            <TextField form={form} name={`${base}.arComprimido.equipamentos.${index}.descricao`} label="Descrição do equipamento" className="md:col-span-2" />
            <NumField form={form} name={`${base}.arComprimido.equipamentos.${index}.capacidadeM3H`} label="Capacidade nominal (m³/h)" />
