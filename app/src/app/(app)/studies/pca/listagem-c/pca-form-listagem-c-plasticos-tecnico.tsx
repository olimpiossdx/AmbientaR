...
export function PcaFormListagemCPlasticosTecnico({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {[
            ...
            <div key={setor.key} className="rounded-md border p-3">
              ...
              <PcaNumField form={form} name={`${base}.recursosHumanos.${setor.key}.quantidade`} label="Nº de funcionários" />
      ...
      <PcaSectionCard title="26. Regime de operação do empreendimento">
        {turnos.map((item, index) => (
          <div key={item.id} className="mb-3 grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-6">
            <PcaTextField form={form} name={`${base}.regimeOperacao.turnos.${index}.setor`} label="Setor / turno" />
            <PcaNumField form={form} name={`${base}.regimeOperacao.turnos.${index}.funcionariosTurno`} label="Nº funcionários/turno" />
            <PcaTextField form={form} name={`${base}.regimeOperacao.turnos.${index}.horarioInicio`} label="Horário início" />
            <PcaTextField form={form} name={`${base}.regimeOperacao.turnos.${index}.horarioFim`} label="Horário fim" />
            <PcaTextField form={form} name={`${base}.regimeOperacao.turnos.${index}.pausaInicio`} label="Pausa início" />
            <PcaTextField form={form} name={`${base}.regimeOperacao.turnos.${index}.pausaFim`} label="Pausa fim" />
      ...
      <PcaSectionCard title="27. Uso de madeira ou carvão vegetal">
        <FormField
          control={form.control}
        ...
        {materiasPrimas.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-6">
            <PcaTextField form={form} name={`${base}.materiasPrimas.${index}.nome`} label="Matéria-prima / insumo" />
            <PcaTextField form={form} name={`${base}.materiasPrimas.${index}.codigoEmbalagem`} label="Cód. embalagem" />
            <PcaTextField form={form} name={`${base}.materiasPrimas.${index}.codigoArmazenamento`} label="Cód. armazenamento" />
            <PcaTextField form={form} name={`${base}.materiasPrimas.${index}.procedencia`} label="Procedência" />
            <PcaNumField form={form} name={`${base}.materiasPrimas.${index}.consumoMaximo`} label="Consumo máx. (mês)" />
            <PcaNumField form={form} name={`${base}.materiasPrimas.${index}.consumoMedio`} label="Consumo médio (mês)" />
            <div className="md:col-span-6 flex justify-end">
      ...
      <PcaSectionCard title="28. Produto principal e produtos secundários">
        {produtos.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
            <PcaTextField form={form} name={`${base}.produtos.${index}.especificacao`} label="Especificação do produto" />
            <PcaNumField form={form} name={`${base}.produtos.${index}.producaoMaxima`} label="Produção máx. (kg/mês)" />
            <PcaNumField form={form} name={`${base}.produtos.${index}.producaoAtual`} label="Produção atual (kg/mês)" />
      ...
      <PcaSectionCard title="29. Equipamentos geradores de calor">
        <FormField
          control={form.control}
        ...
        {usaCaldeira &&
          equipamentosCalor.map((item, index) => (
            <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
              <PcaTextField form={form} name={`${base}.equipamentosCalor.itens.${index}.descricao`} label="Nome/marca/ano/combustível" />
              <PcaNumField form={form} name={`${base}.equipamentosCalor.itens.${index}.quantidade`} label="Quantidade" />
              <PcaNumField form={form} name={`${base}.equipamentosCalor.itens.${index}.tempoOperacaoHDia`} label="Tempo médio (h/dia)" />
              <PcaNumField form={form} name={`${base}.equipamentosCalor.itens.${index}.consumoCombustivel`} label="Consumo máx. combustível" />
              <PcaNumField form={form} name={`${base}.equipamentosCalor.itens.${index}.capacidadeNominal`} label="Capacidade nominal" />
              <PcaNumField form={form} name={`${base}.equipamentosCalor.itens.${index}.alturaChamineM`} label="Altura chaminé (m)" />
              <PcaTextField form={form} name={`${base}.equipamentosCalor.itens.${index}.destinoCinzas`} label="Destino das cinzas" />
              <div className="flex items-end justify-end">
      ...
      <PcaSectionCard title="30. Principais equipamentos do processo industrial">
        {equipamentos.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-5">
            <PcaTextField form={form} name={`${base}.equipamentosProcesso.${index}.nome`} label="Equipamento" />
            <PcaNumField form={form} name={`${base}.equipamentosProcesso.${index}.capacidadeKgMes`} label="Capacidade nominal (kg/mês)" />
            <PcaNumField form={form} name={`${base}.equipamentosProcesso.${index}.potenciaMw`} label="Potência (MW)" />
            <PcaNumField form={form} name={`${base}.equipamentosProcesso.${index}.quantidade`} label="Nº equipamentos" />
            <div className="flex items-end justify-end">
      ...
      <PcaSectionCard title="31 a 33. Processo, fluxogramas e layout">
        <FormDescription>
          Anexo XXXII – processo produtivo; Anexo XXXIII – fluxogramas; Anexo XXXIV – layout da instalação industrial.
        </FormDescription>
        <PcaTextField form={form} name={`${base}.anexos.processo`} label="Referência processo (Anexo XXXII)" />
        <PcaTextField form={form} name={`${base}.anexos.fluxograma`} label="Referência fluxogramas (Anexo XXXIII)" />
        <PcaTextField form={form} name={`${base}.anexos.layout`} label="Referência layout (Anexo XXXIV)" />
      </PcaSectionCard>

      <PcaSectionCard title="34. Equipamentos para geração de energia elétrica">
        <FormField
          control={form.control}
        ...
        {geraEnergia && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            <PcaNumField form={form} name={`${base}.energiaEletrica.cogeracaoKw`} label="Cogeneração (kW)" />
            <PcaNumField form={form} name={`${base}.energiaEletrica.grupoGeradorKw`} label="Grupo gerador (kW)" />
            <PcaTextField form={form} name={`${base}.energiaEletrica.outrasFormas`} label="Outras formas" />
          </div>
        )}
        <FormDescription>Energia fornecida por terceiros</FormDescription>
        <PcaTextField form={form} name={`${base}.energiaEletrica.fornecedor`} label="Empresa fornecedora" />
        ...
        {form.watch(`${base}.energiaEletrica.subestacao`) && (
          <PcaNumField form={form} name={`${base}.energiaEletrica.tensaoKv`} label="Tensão (kV)" />
        )}
      </PcaSectionCard>

      <PcaSectionCard title="35. Ar comprimido">
        {arComprimido.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-3">
            <PcaTextField form={form} name={`${base}.arComprimido.${index}.equipamento`} label="Equipamento de geração" />
            <PcaTextField form={form} name={`${base}.arComprimido.${index}.capacidade`} label="Capacidade nominal" />
            <div className="flex items-end justify-end">
      ...
      <PcaSectionCard title="36. Instalações de abastecimento e manutenção de veículos">
        <FormField
          control={form.control}
        ...
        {postoVeiculos && (
          <div className="space-y-3">
            <PcaTextField form={form} name={`${base}.abastecimentoVeiculos.licencaAaf`} label="Nº licença/AAF (se licenciado)" />
            <PcaTextField form={form} name={`${base}.abastecimentoVeiculos.processoCopam`} label="Processo COPAM (se em licenciamento)" />
            <PcaTextField form={form} name={`${base}.abastecimentoVeiculos.anexoDescricao`} label="Referência Anexo XXXV (instalações existentes)" />
          </div>
        )}
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="37. Identificação e análise ambientais negativos da fase de instalação">
        <FormDescription>Apresentar conforme Anexo XXXVIII.</FormDescription>
        <PcaTextField form={form} name={`${base}.impactosInstalacao.referenciaAnexo`} label="Referência / observações" />
      </PcaSectionCard>

      <PcaSectionCard title="38. Uso de água">
        <FormField
          control={form.control}
        ...
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <PcaNumField form={form} name={`${base}.usoAgua.percentualRecirculado`} label="Porcentagem recirculada (%)" />
          </div>
        )}
        {finalidadesAguaPlasticos.map((finalidade) => {
          ...
          return (
            <div key={slug} className="grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-3">
              ...
              <PcaNumField form={form} name={`${base}.usoAgua.finalidades.${slug}.consumoMaxDia`} label="Consumo máx. (m³/dia)" />
              <PcaNumField form={form} name={`${base}.usoAgua.finalidades.${slug}.consumoMedDia`} label="Consumo médio (m³/dia)" />
            </div>
          );
        })}
      </PcaSectionCard>

      <PcaSectionCard title="39. Efluentes líquidos">
        <FormDescription>39.1 Efluentes sanitários</FormDescription>
        <PcaNumField form={form} name={`${base}.efluentes.volumeSanitarioMaxM3Dia`} label="Volume máx. esgotos sanitários (m³/dia)" />
        <FormField
          control={form.control}
        ...
        <PcaTextField form={form} name={`${base}.efluentes.industrialTotalVazao`} label="Efluente industrial total – vazão (m³/dia)" />
        {parametrosEfluente.map((param) => {
          const slug = slugify(param);
          return (
            <div key={slug} className="grid grid-cols-1 gap-2 rounded-md border p-2 md:grid-cols-3">
              <p className="text-sm">{param}</p>
              <PcaNumField form={form} name={`${base}.efluentes.parametros.${slug}.bruto`} label="Efluente bruto" />
              <PcaNumField form={form} name={`${base}.efluentes.parametros.${slug}.tratado`} label="Efluente tratado" />
            </div>
          );
        })}
        ...
        {sistemaEte && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <PcaNumField form={form} name={`${base}.efluentes.eteAreaM2`} label="Área da ETE (m²)" />
            <PcaTextField form={form} name={`${base}.efluentes.eteTipoSolo`} label="Tipo de solo" />
            <PcaTextField form={form} name={`${base}.efluentes.corpoReceptor`} label="Corpo receptor / classe" />
          </div>
        )}
      ...
      <PcaSectionCard title="40. Resíduos sólidos">
        <FormField
          control={form.control}
        ...
        {gestaoResiduos &&
          residuos.map((item, index) => (
            <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-6">
              <PcaTextField form={form} name={`${base}.residuos.caracterizacao.${index}.pontoGeracao`} label="Ponto de geração" />
              <PcaTextField form={form} name={`${base}.residuos.caracterizacao.${index}.nome`} label="Nome do resíduo" />
              <PcaTextField form={form} name={`${base}.residuos.caracterizacao.${index}.classificacao`} label="Classificação NBR 10.004" />
              <PcaNumField form={form} name={`${base}.residuos.caracterizacao.${index}.geradaKgMes`} label="Gerada (kg/mês)" />
              <PcaNumField form={form} name={`${base}.residuos.caracterizacao.${index}.estocadaKgMes`} label="Estocada (kg/mês)" />
              <PcaTextField form={form} name={`${base}.residuos.caracterizacao.${index}.destinacao`} label="Destinação final" />
              <div className="md:col-span-6 flex justify-end">
        ...
        <PcaTextField form={form} name={`${base}.residuos.anexoDisposicaoIndustria`} label="Referência Anexo XLIX (disposição na indústria)" />
      </PcaSectionCard>
      ...
      <PcaSectionCard title="41. Emissões atmosféricas">
        ...
        {fontesPontuais &&
          fontesEmissao.map((item, index) => (
            <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
              <PcaTextField form={form} name={`${base}.emissoes.fontes.${index}.fonte`} label="Fonte" />
              <PcaTextField form={form} name={`${base}.emissoes.fontes.${index}.combustivel`} label="Combustível" />
              <PcaTextField form={form} name={`${base}.emissoes.fontes.${index}.poluentes`} label="Poluentes emitidos" />
              <div className="flex items-end justify-end">
        ...
        {controleEmissoes &&
          controleEquip.map((item, index) => (
            <div key={item.id} className="grid grid-cols-1 gap-2 rounded-md border p-3 md:grid-cols-4">
              <PcaTextField form={form} name={`${base}.emissoes.equipamentosControle.${index}.nome`} label="Equipamento (nome/marca/qtd)" />
              <PcaNumField form={form} name={`${base}.emissoes.equipamentosControle.${index}.tempoOperacaoHDia`} label="Tempo médio (h/dia)" />
              <PcaTextField form={form} name={`${base}.emissoes.equipamentosControle.${index}.capacidade`} label="Capacidade nominal" />
              <div className="flex items-end justify-end">
      ...
      <PcaSectionCard title="42. Ruídos">
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="43. Avaliação das medidas de controle ambiental existentes">
        <FormDescription>Apresentar Anexos LVI e LVII.</FormDescription>
        <PcaTextField form={form} name={`${base}.medidasControle.referenciaAnexoLvi`} label="Referência Anexo LVI (desempenho 6 meses)" />
        <PcaTextField form={form} name={`${base}.medidasControle.referenciaAnexoLvii`} label="Referência Anexo LVII (outras medidas)" />
        <PcaCheckboxOptions
          form={form}
      ...
      <PcaSectionCard title="44 a 49. Emergência, passivos, paisagismo, educação e carga poluidora">
        <PcaTextField form={form} name={`${base}.emergencia.referenciaRisco`} label="44 – Avaliação de risco de acidentes" />
        <FormField
          control={form.control}
        ...
        {form.watch(`${base}.paisagismo.cinturaoVerde`) && (
          <PcaTextField form={form} name={`${base}.paisagismo.cinturaoVerdeDescricao`} label="Largura e espécies do cinturão verde" />
        )}
        <FormField
          control={form.control}
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <PcaNumField form={form} name={`${base}.cargaPoluidora.esgotoSanitarioM3Dia`} label="Esgoto sanitário bruto (m³/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.esgotoSanitarioDbo`} label="Carga orgânica esgoto (kg DBO5/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.efluenteIndustrialM3Dia`} label="Efluente industrial bruto (m³/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.efluenteIndustrialDqo`} label="Carga orgânica industrial (kg DQO/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.materialParticulado`} label="Material particulado (kg MP/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.dioxidoEnxofre`} label="Dióxido de enxofre (kg SO2/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.residuoClasseI`} label="Resíduos Classe I (kg/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.residuoClasseII`} label="Resíduos Classe II (kg/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.residuoClasseIII`} label="Resíduos Classe III (kg/dia)" />
          <PcaNumField form={form} name={`${base}.cargaPoluidora.energiaEletricaKwMes`} label="Energia elétrica (kW/mês)" />
        </div>
      </PcaSectionCard>
    </div>
  );
