...
function OutorgaIntervencao({ form, basePath }: { form: any; basePath: string }) {
  return (
    <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-3">
      <PcaNumField form={form} name={`${basePath}.volumeM3Mes`} label="Volume (m³/mês)" />
      <PcaTextField form={form} name={`${basePath}.portaria`} label="Portaria Nº (IGAM/ANA)" />
      <PcaTextField form={form} name={`${basePath}.processo`} label="Processo Nº" />
      <FormField
...
export function PcaFormListagemEModulo3({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      <PcaSectionCard title="Módulo 3 – Intervenções ambientais">
        <FormDescription>Itens 9 a 14 do RCA – dutos e gasodutos (DN 217/17).</FormDescription>
      </PcaSectionCard>

      <PcaSectionCard title="9. Restrições locacionais">
        <FormField
          control={form.control}
        ...
        />
        <PcaTextField form={form} name="listagemE.restricoesLocacionais.biomaOutro" label="Outro bioma (qual?)" />
        <FormField
          control={form.control}
        ...
        {form.watch('locationalRestrictions.inKarstArea') && (
          <PcaCheckboxOptions form={form} name="listagemE.restricoesLocacionais.tiposCarstica" options={tiposCarstica} />
        )}
        <FormField
          control={form.control}
      ...
      <PcaSectionCard title="10. Unidades de conservação">
        <FormField
          control={form.control}
        ...
        <PcaTextField form={form} name="conservationUnit.managementCategory" label="Categoria de manejo" />
        <PcaTextField form={form} name="conservationUnit.managingBody" label="Órgão gestor" />
      </PcaSectionCard>

      <PcaSectionCard title="11. Reserva legal">
        <FormField
          control={form.control}
        ...
        />
        <PcaTextField form={form} name="listagemE.reservaLegal.numeroProcesso" label="Nº processo / referência (se aplicável)" />
      </PcaSectionCard>

      <PcaSectionCard title="12. Intervenção em APP">
        <FormField
          control={form.control}
        ...
        {form.watch('listagemE.app.existeNoTerreno') && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <PcaNumField form={form} name="listagemE.app.areaHa" label="Área APP (ha)" />
            <PcaTextField form={form} name="listagemE.app.coberturaVegetal" label="Situação da cobertura vegetal" />
          </div>
        )}
        <PcaTextField
          form={form}
        ...
        {haSupressao && (
          <>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
              <PcaNumField form={form} name="listagemE.supressaoVegetacao.areaNativaHa" label="Nativa (ha)" />
              <PcaNumField form={form} name="listagemE.supressaoVegetacao.areaPlantadaHa" label="Plantada (ha)" />
              <PcaNumField form={form} name="listagemE.supressaoVegetacao.areaMistaHa" label="Mista (ha)" />
              <PcaNumField form={form} name="listagemE.supressaoVegetacao.arvoresIsoladas" label="Árvores isoladas" />
            </div>
            <PcaTextField form={form} name="listagemE.supressaoVegetacao.manifestacaoIbama" label="Manifestação IBAMA / CODEMA (resumo)" />
            <FormField
              control={form.control}
      ...
      <PcaSectionCard title="14. Intervenção em recursos hídricos">
        <FormField
          control={form.control}
        ...
        {form.watch('listagemE.recursosHidricos.usoConcessionaria') && (
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <PcaTextField form={form} name="listagemE.recursosHidricos.concessionariaNome" label="Empresa" />
            <PcaNumField form={form} name="listagemE.recursosHidricos.concessionariaVolume" label="Volume (m³/mês)" />
          </div>
        )}
        ...
        {corposAgua.map((row, index) => (
          <div key={row.id} className="mb-2 flex gap-2">
            <PcaTextField form={form} name={`listagemE.recursosHidricos.corposSuperficiais.${index}.nome`} label="Nome" />
            <PcaNumField
              form={form}
