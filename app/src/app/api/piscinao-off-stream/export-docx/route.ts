/**
 * POST /api/piscinao-off-stream/export-docx
 */

import { NextRequest, NextResponse } from 'next/server';
import { promises as fs } from 'fs';
import path from 'path';
import type { AmbientalContext, PiscinaoOffStream } from '@/lib/types';
import { buildPlaceholderDataForLaudo } from '@/lib/docx-placeholders';
import { buildPiscinaoPlaceholderExtras } from '@/lib/piscinao-off-stream/export-placeholders';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { context, cadastro, templateUrl, cadastroId } = body as {
    ...
    if (!context || !cadastro) {
      return NextResponse.json(
        { success: false, error: 'Envie context e cadastro.' },
        { status: 400 },
      );
    }
    ...
    let content: Buffer;
    try {
      content = await loadTemplateBuffer('piscinao-off-stream', templateUrl);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Erro ao carregar template.';
      return NextResponse.json(
        {
          success: false,
          error: `${msg} Envie o DOCX em Configurações > Templates (piscinao-off-stream).`,
        },
        { status: 404 },
      );
    }
    ...
    const data = mergePlaceholderData(base, buildPiscinaoPlaceholderExtras(cadastro));
    doc.render(data);
    ...
    const fileName = `piscinao_off_stream_${cadastroId ?? cadastro.id ?? Date.now()}.docx`;
    ...
    console.error('POST /api/piscinao-off-stream/export-docx:', e);
