...
import { fetchSicarCarGeometry } from "@/lib/study-maps/sicar-car-fetch";

/**
 * Parser de perímetro — submenu Mapas (Estudos Técnicos / MCA).
 * Autónomo de src/lib/geospatial/* (Análise Geoespacial IA).
 */
...
export type StudyMapsPerimeterInput = {
...
export async function parseStudyMapsPerimeter(
  input: StudyMapsPerimeterInput,
): Promise<{
  ...
  } else if (input.dataType === "car") {
    const sicar = await fetchSicarCarGeometry(trimmed);
    if (!sicar.ok || !sicar.polygon) return null;
    feature = sicar.polygon;
...
export function featureToStudyAreaGeoJSON(
