...
export type SegurancaDocxExportResult = { blob: Blob; fileName: string };
...
function coverParagraphs(
  ...
  const titleBeforeMm = Math.max(0, SEGURANCA_COVER_TITLE_TOP_MM - marginTopMm);
  const children: InstanceType<typeof Paragraph>[] = [
    new Paragraph({
      alignment: AlignmentType.LEFT,
      spacing: { before: convertMillimetersToTwip(titleBeforeMm), after: 280 },
      children: [new TextRun({ text: SEGURANCA_REPORT_TITLE, font, size: 32, bold: true })],
    }),
  ];
  for (const text of buildSegurancaCoverMetaLines(estudo)) {
    children.push(
      new Paragraph({
...
export async function generateSegurancaExportDocxBlobBranded(
  estudo: EstudoSegurancaBarragem,
  pdfImages: BrandingPdfImages,
): Promise<SegurancaDocxExportResult> {
  const {
    AlignmentType,
    Document,
    Packer,
  ...
  const children: InstanceType<typeof Paragraph>[] = [
    ...coverParagraphs(Paragraph, TextRun, AlignmentType, convertMillimetersToTwip, estudo),
    new Paragraph({ children: [new PageBreak()] }),
    new Paragraph({
      spacing: { after: 240 },
      children: [
        new TextRun({ text: SEGURANCA_TOC_HEADING, font: DOCX_BRANDING_FONT, size: 28, bold: true }),
      ],
    }),
  ];
  ...
  return { blob, fileName: `${buildSegurancaExportBaseName(estudo)}.docx` };
}

export async function generateSegurancaExportDocxFromTemplate(params: {
  estudo: EstudoSegurancaBarragem;
  firestore: Firestore;
  auth: Auth | null | undefined;
  templateUrl?: string;
}): Promise<SegurancaDocxExportResult> {
  const { estudo, firestore, auth, templateUrl } = params;
  const projectId = estudo.empreendimento?.projectId?.trim();
  ...
  const res = await fetch('/api/seguranca-barragens/export-docx', {
    method: 'POST',
    headers,
    body: JSON.stringify({
      estudoId: estudo.id,
