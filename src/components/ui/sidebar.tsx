            ...
            style={
              {
                ...
                "--sidebar-width-dynamic": `${desktopSidebarWidthPx ?? SIDEBAR_WIDTH_DESKTOP_DEFAULT_PX}px`,
  ...
  (
    ...
    if (isMobile) {
      ...
      const handleTouchEnd = (e: React.TouchEvent) => {
        ...
        if (deltaX < -60) setOpenMobile(false)
      }
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-full max-w-full bg-sidebar mt-0 h-full p-0 text-sidebar-foreground [&>button]:hidden border-0 shadow-xl data-[state=open]:duration-300 data-[state=closed]:duration-200"
          >
            ...
            >
              ...
              <div className="flex-1 overflow-auto px-2 py-1">{children}</div>
            </div>
          </SheetContent>
        </Sheet>
      )
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-11 w-11 min-h-11 min-w-11 md:h-7 md:w-7 md:min-h-0 md:min-w-0", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        "relative flex min-h-0 min-w-0 max-w-full flex-1 flex-col bg-background",
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    data-sidebar="menu"
    className={cn("flex w-full min-w-0 flex-col gap-0.5 py-1", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <li
    ...
    className={cn("group/menu-item relative border-b border-sidebar-border/40 last:border-b-0", className)}
    {...props}
  />
))
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full min-w-0 items-start gap-2 overflow-x-hidden rounded-md p-2.5 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding,colors] duration-200 ease-out bg-sidebar-accent/50 text-sidebar-accent-foreground hover:bg-sidebar-accent focus-visible:ring-2 active:bg-sidebar-accent disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[state=open]:bg-sidebar-accent/70 data-[state=open]:hover:bg-sidebar-accent group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:items-center md:items-center md:overflow-hidden [&>span]:min-w-0 [&>span]:flex-1 [&>span]:break-words [&>span]:whitespace-normal md:[&>span]:whitespace-nowrap md:[&>span]:break-normal [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-sidebar-accent/50 hover:bg-sidebar-accent",
        outline:
...
>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn(
      "mx-3 flex min-w-0 translate-x-px flex-col gap-0.5 border-l-2 border-sidebar-primary/30 bg-sidebar/30 rounded-r-md px-3 py-2 my-1",
      "group-data-[collapsible=icon]:hidden",
      className
    )}
    {...props}
