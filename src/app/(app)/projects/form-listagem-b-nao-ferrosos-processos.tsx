...
function TabelaLinhaProducaoLigas({
  ...
  return (
    <SectionCard title={titulo}>
      ...
      <TextField form={form} name={`${pathPrefix}.outrosSegmentos`} label="Outros segmentos (especificar)" />
    </SectionCard>
  );
}
    ...
    <SectionCard title={titulo}>
      {fields.map((item, index) => (
        <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-4">
          ...
          <TextField form={form} name={`${basePath}.${index}.caracteristicas`} label="Características" className="md:col-span-2" />
...
export function FormListagemBNaoFerrososProcessos({ form }: { form: any }) {
  ...
  return (
    <div className="space-y-6">
      <SectionCard title="33. Uso de água">
        ...
        {recirculaAgua && (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <NumField form={form} name={`${base}.usoAgua.percentualRecirculado`} label="Porcentagem de água recirculada (%)" />
          </div>
        )}
        ...
        <Table>
          ...
          <TableBody>
            {finalidadesAguaNf.map((finalidade) => {
              ...
              return (
                <TableRow key={slug}>
                  ...
                  <TableCell>
                    <NumField form={form} name={`${base}.usoAgua.finalidades.${slug}.maximo`} label="" />
                  </TableCell>
                  <TableCell>
                    <NumField form={form} name={`${base}.usoAgua.finalidades.${slug}.medio`} label="" />
                  </TableCell>
                </TableRow>
              );
            })}
            <TableRow>
              ...
              <TableCell>
                <NumField form={form} name={`${base}.usoAgua.consumoTotalMaximo`} label="" />
              </TableCell>
              <TableCell>
                <NumField form={form} name={`${base}.usoAgua.consumoTotalMedio`} label="" />
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
            ...
            {grupo.itens.map((liga) => {
              ...
              return (
                <div key={ligaSlug} className="grid grid-cols-1 gap-3 md:grid-cols-5">
                  ...
                  />
                  <NumField form={form} name={`${base}.producaoDiaria.${ligaSlug}.pesoMinimoKg`} label="Peso mín. (Kg)" />
                  <NumField form={form} name={`${base}.producaoDiaria.${ligaSlug}.pesoIdealKg`} label="Peso ideal (Kg)" />
                  <NumField form={form} name={`${base}.producaoDiaria.${ligaSlug}.pesoMaximoKg`} label="Peso máx. (Kg)" />
                  <NumField form={form} name={`${base}.producaoDiaria.${ligaSlug}.producaoDiariaKg`} label="Produção diária (Kg)" />
                </div>
              );
            })}
          </div>
      ...
      <SectionCard title="36. Processo de fusão – tipo de fornos">
        {combustiveisFusaoNf.map((comb) => (
          <div key={comb.id} className="space-y-2 rounded-md border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
              ...
              />
              <NumField form={form} name={`${base}.fusao.${comb.id}.numFornos`} label="Nº de fornos" />
              <TextField form={form} name={`${base}.fusao.${comb.id}.ligaProduzida`} label="Liga metálica produzida" />
              <NumField form={form} name={`${base}.fusao.${comb.id}.capacidadeTh`} label="Capacidade (T/h)" />
            </div>
          </div>
        ))}
        <TextField form={form} name={`${base}.fusao.outrosCombustiveis`} label="Outros combustíveis – especificar" />
      </SectionCard>
      ...
      <SectionCard title="37. Modelagem">
        ...
        {linhasModelagem.map((item, index) => (
          <div key={item.id} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            <TextField form={form} name={`${base}.modelagem.ligas.${index}.liga`} label="Liga produzida" />
            <CheckboxOptions
              form={form}
      ...
      <SectionCard title="38. Processo de macharia">
        ...
        />
        <TextField form={form} name={`${base}.macharia.processosOutros`} label="Outros – descrever" />
        <TextField form={form} name={`${base}.macharia.liga`} label="Liga produzida" />
        <CheckboxOptions
          form={form}
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <TextField form={form} name={`${base}.macharia.modeloMaquina`} label="Modelo/tipo de máquina" />
          <NumField form={form} name={`${base}.macharia.numMaquinas`} label="Nº de máquinas" />
          <NumField form={form} name={`${base}.macharia.produtividadeMachosH`} label="Produtividade machos/h" />
        </div>
      </SectionCard>
      ...
      <SectionCard title="39. Processo de moldagem">
        ...
        />
        <TextField form={form} name={`${base}.moldagem.tiposOutros`} label="Outros – descrever" />
        <TextField form={form} name={`${base}.moldagem.liga`} label="Liga produzida" />
        <CheckboxOptions
          form={form}
        ...
        />
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <TextField form={form} name={`${base}.moldagem.modeloMaquina`} label="Modelo/tipo de máquina" />
          <NumField form={form} name={`${base}.moldagem.numMaquinas`} label="Nº de máquinas" />
          <NumField form={form} name={`${base}.moldagem.produtividadeMoldesH`} label="Produtividade moldes/h" />
        </div>
      </SectionCard>

      <SectionCard title="40. Tratamento térmico das peças fundidas de metais não ferrosos">
        <CheckboxOptions
          form={form}
        ...
        />
        <TextField form={form} name={`${base}.tratamentoTermico.ligas`} label="Ligas tratadas" />
        <CheckboxOptions
          form={form}
        ...
        />
        <TextField form={form} name={`${base}.tratamentoTermico.outros`} label="Outros – especificar" />
      </SectionCard>
      ...
      <SectionCard title="43. Processo de pintura das peças fundidas">
        {['primer', 'epoxi', 'verniz', 'outros'].map((tipo) => (
          <div key={tipo} className="grid grid-cols-1 gap-3 rounded-md border p-3 md:grid-cols-3">
            ...
            />
            <NumField form={form} name={`${base}.pintura.${tipo}.consumoMensalL`} label="Consumo mensal (L)" />
          </div>
        ))}
      </SectionCard>

      <SectionCard title="44. Tratamento químico superficial / galvanotécnico">
        <TextField form={form} name={`${base}.tratamentoQuimico.ligas`} label="Ligas submetidas ao tratamento" />
        <CheckboxOptions
          form={form}
        ...
        />
        <NumField form={form} name={`${base}.tratamentoQuimico.volumeMensalKg`} label="Volume mensal de peças tratadas (Kg)" />
        <FormField
          control={form.control}
