...
export default function InvoicesPage() {
    ...
    React.useEffect(() => {
        ...
        if (isSelfRegistered) {
            ...
            const qApproved = query(clientsRef, where('approvedUserIds', 'array-contains', user.id));
            Promise.all([getDocs(qUserId), getDocs(qApproved)]).then(([snapU, snapA]) => {
                const ids = new Set<string>([...snapU.docs.map(d => d.id), ...snapA.docs.map(d => d.id)]);
                setClientIdsForUser(Array.from(ids));
            }).catch(() => setClientIdsForUser([]));
        } else {
            const clientsRef = collection(firestore, 'clients');
            const byUserId = getDocs(query(clientsRef, where('userId', '==', user.id)));
            const byApproved = getDocs(query(clientsRef, where('approvedUserIds', 'array-contains', user.id)));
            ...
            if (userDocs.length > 0) {
                ...
                Promise.all([byUserId, byApproved]).then(([snapU, snapA]) => {
                    const ids = new Set<string>([...snapU.docs.map(d => d.id), ...snapA.docs.map(d => d.id)]);
                    setClientIdsForUser(Array.from(ids));
                }).catch(() => setClientIdsForUser([]));
            }
