...
export function CommercialProposalsListView() {
  ...
  // Resolve os clientIds que o titular (Cliente Gest├úo / Aut├┤nomo) ou representante pode ver.
  React.useEffect(() => {
  ...
  const proposalsQuery = useMemoFirebase(() => {
    ...
    // Titular (gest├úo ou aut├┤nomo) ou representante: apenas propostas dos seus clientes.
  ...
  const filteredProposals = useMemo(() => {
    ...
    // Titular (gest├úo ou aut├┤nomo): apenas propostas dos pr├│prios clientIds.
    if (isClientePortalRole(user?.role)) {
      return proposals.filter((p) => userClients.includes(p.clientId));
    }

    // Representante: j├í filtrado em proposalsQuery via clientIdsForUser.
    if (isRepresentativeLikePortalRole(user?.role)) {
  ...
  const { activeProposals, finalizedProposals } = useMemo(() => {
    ...
    // Titular (gest├úo ou aut├┤nomo) e representante veem apenas propostas aprovadas (consulta/download).
    if (isEmpreendedorScopedPortalRole(user?.role)) {
  ...
  const handleExportProposalPdf = async (proposal: CommercialProposal) => {
    if (!firestore) {
      toast({
        variant: "destructive",
        title: "Erro",
        description: "Servi├ºo de banco de dados indispon├¡vel.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          "N├úo foi poss├¡vel gerar o arquivo. Verifique a conex├úo e tente novamente.",
      });
    } finally {
          ...
          >
            ...
            <CollapsibleContent>
              <div className="flex flex-wrap items-end gap-3 p-3 pt-0 border-t">
                ...
                <div>
                  <Label className="text-xs">N┬║ Proposta</Label>
                  <Input
                    placeholder="N├║mero"
                    className="h-8 w-28"
                    value={filterNumero}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Card className="bg-muted/20">
              <CardContent className="pt-4 pb-4">
                ...
                <p className="text-2xl font-semibold mt-1">
                  {isLoading ? "ÔÇö" : filteredActiveProposals.length}
                </p>
            ...
            <Card className="bg-muted/20">
              <CardContent className="pt-4 pb-4">
                ...
                <p className="text-2xl font-semibold mt-1">
                  {isLoading ? "ÔÇö" : filteredFinalizedProposals.length}
                </p>
                <p className="text-xs text-muted-foreground">
                            ...
                            <CardContent className="p-4 sm:p-5">
                              <div className="flex flex-col gap-4">
                                <div className="min-w-0 space-y-2">
                                  ...
                                  <p className="text-sm text-muted-foreground">
                                    Data:{" "}
                                    {new Date(proposal.proposalDate).toLocaleDateString("pt-BR")} ┬À
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="w-[95vw] max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Visualiza├º├úo dos dados cadastrados para a proposta.
            </DialogDescription>
          </DialogHeader>
          {viewingItem && (
            <div className="form-scroll-body max-h-[60vh] space-y-4">
              ...
              <div className="space-y-2">
                <Label className="text-sm font-medium">Itens de Servi├ºo</Label>
              ...
              {viewingItem.contractId && contractsMap.get(viewingItem.contractId) ? (
                <AttachmentPreviewSection
                  ...
                  emptyLabel="O contrato ainda n├úo tem PDF gerado. Use o bot├úo de documento na lista ou abra Contratos."
