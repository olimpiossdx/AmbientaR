...
import { AlertTriangle, CheckCircle2, Clock, Droplets, Trees } from "lucide-react";
...
interface EnvironmentalDashboardProps {
  /** Projetos / empreendimentos (coleção `projects`) — não são licenças emitidas. */
  initialPermits?: Project[] | null;
  /** Licenças ambientais (coleção `licenses`) — mesmo critério do menu Licenças. */
...
export default function EnvironmentalDashboard({
  ...
  const [isCondicionanteDialogOpen, setIsCondicionanteDialogOpen] =
    useState(false);
  const [dialogContent, setDialogContent] = useState<License[]>([]);
  const [dialogCondicionantes, setDialogCondicionantes] = useState<
    Condicionante[]
  >([]);
  const [dialogTitle, setDialogTitle] = useState("");

  /** Cliente/representante recebem dados via props (painel do titular). */
  ...
  const { data: permits, isLoading: isLoadingPermits } =
    useCollection<Project>(
      useMemoFirebase(() => {
        if (initialPermits !== undefined || !firestore || isExternalTitularView)
          return null;
        return collection(firestore, "projects");
      }, [firestore, initialPermits, isExternalTitularView]),
    );

  const { data: licensesFromDb, isLoading: isLoadingLicensesFromDb } =
    useCollection<License>(
      useMemoFirebase(() => {
        if (
          initialLicenses !== undefined ||
          !firestore ||
          isExternalTitularView
        )
          return null;
        return collection(firestore, "licenses");
      }, [firestore, initialLicenses, isExternalTitularView]),
    );

  const { data: condicionantes, isLoading: isLoadingCondicionantes } =
    useCollection<Condicionante>(
      useMemoFirebase(() => {
        if (initialCondicionantes !== undefined || !firestore) return null;
        if (isExternalTitularView) return null;
        return collection(firestore, "condicionantes");
      }, [firestore, initialCondicionantes, isExternalTitularView]),
    );

  const { data: outorgas, isLoading: isLoadingOutorgas } =
    useCollection<WaterPermit>(
      useMemoFirebase(() => {
        if (initialOutorgas !== undefined || !firestore) return null;
        if (isExternalTitularView) return null;
        return collection(firestore, "outorgas");
      }, [firestore, initialOutorgas, isExternalTitularView]),
    );

  const { data: intervencoes, isLoading: isLoadingIntervencoes } =
    useCollection<EnvironmentalIntervention>(
      useMemoFirebase(() => {
        if (initialIntervencoes !== undefined || !firestore) return null;
        if (isExternalTitularView) return null;
        return collection(firestore, "intervencoes");
      }, [firestore, initialIntervencoes, isExternalTitularView]),
    );

  const finalProjects = initialPermits !== undefined ? initialPermits : permits;
  const finalLicenses =
    initialLicenses !== undefined ? initialLicenses : licensesFromDb;
  const finalCondicionantes =
    initialCondicionantes !== undefined
      ? initialCondicionantes
      : condicionantes;
  const finalOutorgas =
    initialOutorgas !== undefined ? initialOutorgas : outorgas;
  const finalIntervencoes =
    initialIntervencoes !== undefined ? initialIntervencoes : intervencoes;

  const projectsQuery = useMemoFirebase(() => {
    if (!firestore) return null;
    if (isExternalTitularView) return null;
    return collection(firestore, "projects");
  }, [firestore, isExternalTitularView]);
  const { data: allProjects, isLoading: isLoadingProjects } =
    useCollection<Project>(projectsQuery);
  const projectsMap = useMemo(() => {
    const merged = new Map<string, string>();
    for (const p of allProjects ?? []) {
      merged.set(p.id, p.propertyName);
    }
  ...
  const empreendedoresMap = useMemo(() => {
    return new Map((allEmpreendedores ?? []).map((e) => [e.id, e.name]));
  }, [allEmpreendedores]);

  const { permitStats, complianceStats, outorgaStats, intervencaoStats } =
    useMemo(() => {
      ...
      const stats = {
        valid: 0,
        expired: [] as License[],
        expiringIn30: [] as License[],
        expiringIn60: [] as License[],
        expiringIn180: [] as License[],
        expiringIn360: [] as License[],
      };

      (finalLicenses || []).forEach((permit) => {
        if (!permit.expirationDate) {
          if (
            permit.status === "Válida" ||
            permit.status === "Em Renovação" ||
            permit.status === "Em Andamento"
          )
            stats.valid++;
          return;
        }
      ...
      const recentPermits = [...(finalLicenses || [])]
        .sort(
          (a, b) =>
      ...
      const activeOutorgas = (finalOutorgas || []).filter(
        (o) => o.status === "Válida",
      ).length;
    ...
    }, [finalLicenses, finalCondicionantes, finalOutorgas, finalIntervencoes]);
  ...
  const handleCardClick = (group: LicenseGroup, title: string) => {
    let licensesToShow: License[] = [];
    switch (group) {
      case "expiringIn30":
        licensesToShow = permitStats.expiringIn30;
        break;
      case "expiringIn60":
        licensesToShow = permitStats.expiringIn60;
        break;
      case "expiringIn180":
        licensesToShow = permitStats.expiringIn180;
        break;
      case "expiringIn360":
        licensesToShow = permitStats.expiringIn360;
        break;
      case "expired":
        licensesToShow = permitStats.expired;
        break;
    }
  ...
  const getStatusVariant = (
    status?: License["status"] | Condicionante["status"],
  ) => {
    switch (status) {
      case "Válida":
      case "Cumprida":
    ...
    <>
      <div className="space-y-6">
            ...
            <CardContent>
              ...
              {isLoading ? (
                ...
                <p className="text-xs text-muted-foreground">
                  de {permitStats.total} licenças totais
                </p>
              )}
            </CardContent>
          </Card>
          ...
          >
            ...
            <CardContent>
              ...
              {isLoading ? (
                ...
                <p className="text-xs text-muted-foreground">
                  {complianceStats.overdue} atrasadas
                </p>
              )}
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-2xl font-bold">{outorgaStats.active}</div>
              )}
              {isLoading ? (
                <Skeleton className="h-4 w-3/4 mt-1" />
              ) : (
                <p className="text-xs text-muted-foreground">
                  de {outorgaStats.total} outorgas
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          <Card
            className="border-red-500/50 bg-red-500/10 dark:bg-red-500/10 cursor-pointer hover:bg-red-500/20"
            onClick={() => handleCardClick("expiringIn30", "Vencem em 30 Dias")}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-8 w-1/2 bg-red-200" />
              ) : (
                <div className="text-2xl font-bold text-red-900 dark:text-red-200">
                  {permitStats.expiringIn30.length}
                </div>
              )}
          ...
          <Card
            className="border-orange-500/50 bg-orange-500/10 dark:bg-orange-500/10 cursor-pointer hover:bg-orange-500/20"
            onClick={() => handleCardClick("expiringIn60", "Vencem em 60 Dias")}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-8 w-1/2 bg-orange-200" />
              ) : (
                <div className="text-2xl font-bold text-orange-900 dark:text-orange-200">
                  {permitStats.expiringIn60.length}
                </div>
              )}
          ...
          <Card
            className="border-yellow-500/50 bg-yellow-500/10 dark:bg-yellow-500/10 cursor-pointer hover:bg-yellow-500/20"
            onClick={() =>
              handleCardClick("expiringIn180", "Vencem em 180 Dias")
            }
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-8 w-1/2 bg-yellow-200" />
              ) : (
                <div className="text-2xl font-bold text-yellow-900 dark:text-yellow-200">
                  {permitStats.expiringIn180.length}
                </div>
              )}
              <p className="text-xs text-yellow-700 dark:text-yellow-400">
                Requerem atenção em breve.
              </p>
            </CardContent>
          </Card>
          <Card
            className="border-blue-500/40 bg-blue-500/10 dark:bg-blue-500/10 cursor-pointer hover:bg-blue-500/20"
            onClick={() =>
              handleCardClick("expiringIn360", "Vencem em 360 Dias")
            }
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-2xl font-bold text-blue-900 dark:text-blue-200">
                  {permitStats.expiringIn360.length}
                </div>
              )}
              <p className="text-xs text-blue-700 dark:text-blue-400">
                No radar para o próximo ano.
              </p>
            </CardContent>
          </Card>
          <Card
            className="cursor-pointer hover:bg-muted/50"
            onClick={() => handleCardClick("expired", "Licenças Vencidas")}
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-2xl font-bold">
                  {permitStats.expired.length}
                </div>
              )}
              <p className="text-xs text-muted-foreground">
                Licenças que expiraram.
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Licença / processo</TableHead>
              ...
              <TableBody>
                ...
                {!isLoading &&
                  permitStats.recent.map((lic) => (
                    <TableRow key={lic.id}>
                      <TableCell>
                        <div className="font-medium">
                          {lic.permitNumber || lic.processNumber || lic.id}
                        </div>
                        <div className="text-sm text-muted-foreground hidden sm:block">
                          {projectsMap.get(lic.projectId) || "Empreendimento"}{" "}
                          · Proc. {lic.processNumber || "—"}
                        </div>
                        <Link
                          href={`/licenses/${lic.id}/edit`}
                          className="text-xs text-primary underline-offset-4 hover:underline sm:hidden"
                        >
                          Abrir cadastro
                        </Link>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex flex-col items-start gap-1 md:flex-row md:items-center md:gap-2">
                          <Badge
                            variant="outline"
                            className={cn(getStatusVariant(lic.status))}
                          >
                            {lic.status || "Não definido"}
                          </Badge>
                          <Link
                            href={`/licenses/${lic.id}/edit`}
                            className="text-xs text-primary underline-offset-4 hover:underline hidden sm:inline"
                          >
                            Editar
                          </Link>
                        </div>
                ...
                {!isLoading && permitStats.recent.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={3} className="h-24 text-center">
                      Nenhuma licença ambiental cadastrada.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Empreendimentos (cadastro)</CardTitle>
            <CardDescription>
              Projetos/empreendimentos cadastrados — não são licenças emitidas.{" "}
              <span className="font-medium text-foreground">
                Antes apareciam aqui como “licença” por engano.
              </span>
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="hidden md:table-cell">
                    Nº processo (cadastro)
                  </TableHead>
                  <TableHead>Status no cadastro</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading &&
                  ...
                  recentProjects.map((proj) => (
                    <TableRow key={proj.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={cn(getStatusVariant(proj.status))}
                        >
                          {proj.status || "—"}
                        </Badge>
                      </TableCell>
                    ...
                    <TableRow key={license.id}>
                      ...
                      <TableCell>
                        {license.permitNumber
                          ? `${license.permitNumber} (proc. ${license.processNumber ?? "—"})`
      ...
      <Dialog
        open={isCondicionanteDialogOpen}
        onOpenChange={setIsCondicionanteDialogOpen}
      >
                ...
                {dialogCondicionantes.length > 0 ? (
                  dialogCondicionantes.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        {projectsMap.get(item.referenceId ?? "") ?? "N/A"}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant={"outline"}
