...
import type { Condicionante, PermitStatus, Project, WaterPermit, EnvironmentalIntervention, License } from '@/lib/types';
import { useFirebase, errorEmitter, useCollection, useMemoFirebase } from '@/firebase';
import { FirestorePermissionError } from '@/firebase/errors';
import { collection, doc, addDoc, updateDoc } from 'firebase/firestore';
import { createNotificationForUser, getUserIdFromCondicionanteReference } from '@/lib/notifications';
...
export function ComplianceForm({ currentItem, referenceType, onSuccess }: ComplianceFormProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `condicionantes/${Date.now()}-${file.name}`);
  ...
  const referenceItems = React.useMemo(() => {
    if (referenceType === 'licenca') {
      ...
      return licenses?.map(l => ({
        id: l.id,
        name: [l.processNumber, l.permitNumber, projectsMap.get(l.projectId)?.propertyName].filter(Boolean).join(' — ') || l.processNumber || l.id || 'Licença',
      })) ?? [];
    }
    if (referenceType === 'outorga') {
      return outorgas?.map(o => ({
        id: o.id,
        name: [o.permitNumber, o.description].filter(Boolean).join(' - ') || o.permitNumber || o.id || 'Outorga',
      })) ?? [];
    }
    if (referenceType === 'intervencao') {
      return intervencoes?.map(i => ({
        id: i.id,
        name: [i.processNumber, i.description].filter(Boolean).join(' - ') || i.description || i.id || 'Intervenção',
      })) ?? [];
    }
    return [];
  }, [referenceType, licenses, projects, outorgas, intervencoes]);
  
  const isLoadingReference = isLoadingLicenses || isLoadingProjects || isLoadingOutorgas || isLoadingIntervencoes;
    ...
    if (currentItem) {
      ...
      updateDoc(docRef, dataToSave)
        .then(async () => {
          ...
          const refType = (values.referenceType || referenceType) as 'licenca' | 'outorga' | 'intervencao';
          const targetUserId = await getUserIdFromCondicionanteReference(firestore, refType, values.referenceId);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="h-full flex flex-col overflow-hidden">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value || undefined} disabled={isLoadingReference}>
                  <FormControl>
                    <SelectTrigger>
          ...
          <div className="space-y-2">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <FormLabel className="cursor-help">Anexar documento (opcional)</FormLabel>
                </TooltipTrigger>
                <TooltipContent><p>Carregar / Fazer download</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
            <Input
              type="file"
            ...
            />
            <FormDescription>
              Anexe comprovante ou documento relacionado à condicionante (PDF, JPG, PNG). Máx 10MB.
              {currentItem?.fileUrl && !uploadedFileUrl && (
                <span className="block mt-2 text-xs">
                  Arquivo atual: <a href={currentItem.fileUrl} target="_blank" className="underline" rel="noreferrer">Fazer download</a>
                </span>
              )}
              {uploadedFileUrl && (
                <span className="block mt-2 text-xs text-green-600">
                  Novo arquivo carregado: <a href={uploadedFileUrl} target="_blank" className="underline" rel="noreferrer">Fazer download</a>
                </span>
              )}
            </FormDescription>
          </div>
