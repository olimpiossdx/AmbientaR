...
import { MoreHorizontal, PlusCircle, ChevronDown, FolderOpen, Droplets, FileCheck2, Trees, Paperclip } from 'lucide-react';
import { useCollection, useFirebase, useMemoFirebase, errorEmitter } from '@/firebase';
import { collection, doc, deleteDoc, query, where, getDocs } from 'firebase/firestore';
import type { Condicionante, Project, WaterPermit, EnvironmentalIntervention, AppUser, Empreendedor, License } from '@/lib/types';
...
export default function CompliancePage() {
    ...
    if (user?.role === 'client' && firestore) {
        ...
        if (isSelfRegistered) {
            ...
            Promise.all([getDocs(qByUserId), getDocs(qByApproved)]).then(([snapUserId, snapApproved]) => {
                ...
                setEmpreendedorIdsForUser(ids.size > 0 ? Array.from(ids) : ['invalid-placeholder']);
        ...
        const userDocs = documentVariants(user.cpf, user.cnpjs);
        if (userDocs.length > 0) {
            const qByDoc = query(empreendedoresRef, where('cpfCnpj', 'in', userDocs));
            Promise.all([byUserId, byApproved, getDocs(qByDoc)]).then(([snapUserId, snapApproved, snapDoc]) => {
            ...
            Promise.all([byUserId, byApproved]).then(([snapUserId, snapApproved]) => {
                const ids = new Set<string>([...snapUserId.docs.map(d => d.id), ...snapApproved.docs.map(d => d.id)]);
                setEmpreendedorIdsForUser(ids.size > 0 ? Array.from(ids) : ['invalid-placeholder']);
            }).catch(() => setEmpreendedorIdsForUser(['invalid-placeholder']));
        }
  ...
  useEffect(() => {
    ...
    const promises = chunks.map(chunk =>
      getDocs(query(collection(firestore, 'licenses'), where('projectId', 'in', chunk)))
    );
  ...
  const projectIdChunks = useMemo(() => {
    if (!projectIds.length || projectIds.length <= CONDITIONANTES_CHUNK_SIZE) return [projectIds];
    const chunks: string[][] = [];
  ...
  const referenceIdChunks = useMemo(() => {
    if (!referenceIdsForClient.length || referenceIdsForClient.length <= CONDITIONANTES_CHUNK_SIZE) return [referenceIdsForClient];
    const chunks: string[][] = [];
    for (let i = 0; i < referenceIdsForClient.length; i += CONDITIONANTES_CHUNK_SIZE) {
      chunks.push(referenceIdsForClient.slice(i, i + CONDITIONANTES_CHUNK_SIZE));
    }
    return chunks;
  ...
  const condicionantesQuerySingle = useMemoFirebase(() => {
    ...
    if (user?.role === 'client') {
      ...
      if (referenceIdsForClient.length > 0 && referenceIdsForClient.length <= CONDITIONANTES_CHUNK_SIZE) {
        return query(collection(firestore, 'condicionantes'), where('referenceId', 'in', referenceIdsForClient));
      }
      return null;
    }
  ...
  const { data: condicionantesSingle, isLoading: isLoadingCondicionantesSingle, error: condicionantesError } = useCollection<Condicionante>(condicionantesQuerySingle);

  const condicionantesQueryChunk0 = useMemoFirebase(() => {
    if (!firestore || user?.role !== 'client' || referenceIdChunks.length < 1 || referenceIdChunks[0].length === 0) return null;
  ...
  const condicionantesQueryChunk1 = useMemoFirebase(() => {
    if (!firestore || user?.role !== 'client' || referenceIdChunks.length < 2 || referenceIdChunks[1].length === 0) return null;
  ...
  const condicionantesQueryChunk2 = useMemoFirebase(() => {
    if (!firestore || user?.role !== 'client' || referenceIdChunks.length < 3 || referenceIdChunks[2].length === 0) return null;
  ...
  const condicionantesQueryChunk3 = useMemoFirebase(() => {
    if (!firestore || user?.role !== 'client' || referenceIdChunks.length < 4 || referenceIdChunks[3].length === 0) return null;
  ...
  const condicionantesQueryChunk4 = useMemoFirebase(() => {
    if (!firestore || user?.role !== 'client' || referenceIdChunks.length < 5 || referenceIdChunks[4].length === 0) return null;
  ...
  const condicionantesMerged = useMemo(() => {
    ...
    return null;
  }, [user?.role, referenceIdChunks.length, condicionantesChunk0, condicionantesChunk1, condicionantesChunk2, condicionantesChunk3, condicionantesChunk4]);

  const condicionantes = condicionantesMerged ?? condicionantesSingle ?? null;
  const isLoadingCondicionantes = user?.role === 'client' && referenceIdChunks.length > 1
  ...
  const isLoading = isLoadingCondicionantes || isLoadingProjects || isLoadingLicenses || isLoadingOutorgas || isLoadingIntervencoes || (user?.role === 'client' && empreendedorIdsForUser === undefined) || (!user && !!firestore);
  ...
  const { licencaGroups, outorgaGroups, intervencaoGroups } = useMemo(() => {
    ...
    condicionantes.forEach(item => {
      const refType = String(item.referenceType ?? '').trim().toLowerCase();
    ...
    licencaGroups.forEach((arr, k) => licencaGroups.set(k, arr.sort(sortByCreated)));
    outorgaGroups.forEach((arr, k) => outorgaGroups.set(k, arr.sort(sortByCreated)));
    intervencaoGroups.forEach((arr, k) => intervencaoGroups.set(k, arr.sort(sortByCreated)));
                ...
                <TableCell>
                  {item.fileUrl ? (
                    <Tooltip>
                      ...
                      <TooltipContent><p>Fazer download</p></TooltipContent>
                    </Tooltip>
                  ) : (
    ...
    <div className="flex flex-col gap-6">
        {condicionantesError && (
          <div className="rounded-md bg-destructive/10 border border-destructive/30 p-4 text-sm text-destructive">
            Não foi possível carregar a lista de condicionantes. Verifique as regras de acesso no Firestore ou faça deploy das regras atualizadas.
          </div>
        )}
        <Card>
        <CardHeader>
                ...
                <Accordion type="single" collapsible className="w-full">
                    {Array.from(licencaGroups.entries()).map(([refId, items]) => {
                        ...
                        const subtitle = license
                          ? [license.permitNumber, projectsMap.get(license.projectId)?.propertyName].filter(Boolean).join(' · ')
