...
function NewClientPageContent() {
    ...
    const skipCpfStep = !!(initialFromAnalise?.cpfCnpj && initialFromAnalise.cpfCnpj.replace(/\D/g, '').length >= MIN_CPF_LENGTH);
    ...
    const handleCpfContinue = async () => {
      ...
      if (firestore && digits.length >= MIN_CPF_LENGTH) {
        ...
        if (!snap.empty) {
          ...
          setExistingClient({ id: doc.id, ...doc.data() } as Client);
        }
      }
      setStep('form');
    };
    ...
    const initialClient = step === 'form' && cpfCnpjInitial
      ? { ...initialFromAnalise, cpfCnpj: cpfCnpjInitial } as Partial<Client>
      : (initialFromAnalise && Object.values(initialFromAnalise).some(Boolean) ? initialFromAnalise : null);
            ...
            {showCpfStep ? (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    A primeira etapa do cadastro é informar o CPF ou CNPJ do cliente. Em seguida você preenche os demais dados.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Button onClick={handleCpfContinue}>
