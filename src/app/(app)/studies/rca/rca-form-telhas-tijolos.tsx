...
interface RcaFormTelhasTijolosProps {
    form: any;
    clients: Client[];
...
export function RcaFormTelhasTijolos({ form, clients, isLoadingClients, projects, isLoadingProjects }: RcaFormTelhasTijolosProps) {
                ...
                <AccordionContent className="space-y-6 p-1">
                    {/* Simplified for brevity - You would add all fields from the Termo de Referência here. */}
                     <div className="space-y-4 p-4 border rounded-md">
                        <h3 className="font-semibold">12. REGIME DE OPERAÇÃO DO EMPREENDIMENTO</h3>
                        <FormField control={form.control} name="regimeOperacao.turnos" render={({ field }: any) => (<FormItem><FormLabel>Turnos</FormLabel><FormControl><Input placeholder="1º, 2º, 3º..." {...field} /></FormControl></FormItem>)} />
                        {/* ... outros campos de regime de operação ... */}
                    </div>
                     <div className="space-y-4 p-4 border rounded-md">
                     ...
                     <div className="space-y-4 p-4 border rounded-md">
                        ...
                        {equipamentosCalorFields.map((item, index) => (
                            <div key={item.id} className="p-2 border rounded-md space-y-2 relative">
                                ...
                                <FormField control={form.control} name={`equipamentosCalor.${index}.quantidade`} render={({ field }: any) => (<FormItem><FormLabel>Quantidade</FormLabel><FormControl><Input type="number" {...field} /></FormControl></FormItem>)} />
                                <FormField control={form.control} name={`equipamentosCalor.${index}.capacidadeNominal`} render={({ field }: any) => (<FormItem><FormLabel>Capacidade Nominal</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                            </div>
                        ))}
