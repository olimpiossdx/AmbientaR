...
import type { Empreendedor as Client, Project, OwnerCondition, Datum, Fuso, RegularizacaoSituacao, ManagementCategory, Jurisdiction, Biome } from '@/lib/types';
import { Textarea } from '@/components/ui/textarea';
import { PlusCircle, Trash2 } from 'lucide-react';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';

interface RcaFormMoveisProps {
    form: any;
    clients: Client[];
...
export function RcaFormMoveis({ form, clients, isLoadingClients, projects, isLoadingProjects }: RcaFormMoveisProps) {
                     ...
                     <div className="space-y-4 p-4 border rounded-md">
                        ...
                        {materiasPrimasFields.map((item, index) => (
                            <div key={item.id} className="p-2 border rounded-md space-y-2 relative">
                                ...
                                <FormField control={form.control} name={`materiasPrimas.${index}.nome`} render={({ field }) => (<FormItem><FormLabel>Nome Técnico/Comercial</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                     ...
                     <div className="space-y-4 p-4 border rounded-md">
                        ...
                        {equipamentosCalorFields.map((item, index) => (
                            <div key={item.id} className="p-2 border rounded-md space-y-2 relative">
                                ...
                                <FormField control={form.control} name={`equipamentosCalor.${index}.nome`} render={({ field }) => (<FormItem><FormLabel>Nome/Marca/Ano</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                         ...
                         {equipamentosProducaoFields.map((item, index) => (
                            <div key={item.id} className="p-2 border rounded-md space-y-2 relative">
                                ...
                                <FormField control={form.control} name={`equipamentosProducao.${index}.nome`} render={({ field }) => (<FormItem><FormLabel>Nome/Marca</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                        ...
                        {residuosSolidosFields.map((item, index) => (
                             <div key={item.id} className="grid grid-cols-2 md:grid-cols-3 gap-4 border p-2 rounded-md relative">
                                <Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={() => removeResiduoSolido(index)}><Trash2 className="h-4 w-4" /></Button>
                                 <FormField control={form.control} name={`residuosSolidos.${index}.nome`} render={({ field }) => (<FormItem><FormLabel>Nome do Resíduo</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                 <FormField control={form.control} name={`residuosSolidos.${index}.equipamento`} render={({ field }) => (<FormItem><FormLabel>Operação Geradora</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                 <FormField control={form.control} name={`residuosSolidos.${index}.classe`} render={({ field }) => (<FormItem><FormLabel>Classe</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                 <FormField control={form.control} name={`residuosSolidos.${index}.taxa`} render={({ field }) => (<FormItem><FormLabel>Taxa Mensal Máx.</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                 <FormField control={form.control} name={`residuosSolidos.${index}.acondicionamento`} render={({ field }) => (<FormItem><FormLabel>Acondicionamento</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                                 <FormField control={form.control} name={`residuosSolidos.${index}.local`} render={({ field }) => (<FormItem><FormLabel>Local de Acond.</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>)} />
                            </div>
                        ))}
                    </div>
                </AccordionContent>
