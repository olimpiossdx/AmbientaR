...
import {
  ...
  type RcaListagemBFormTipo} from './rca-listagem-b-registry';
import {
  getRcaListagemBDefaultValues,
  rcaListagemBFormSchema,
  type RcaListagemBFormValues} from './rca-listagem-b-schema';
import { RcaListagemBFormularioTipoCard } from './rca-form-listagem-b-tipo-card';
...
import {
  prefillRcaListagemBFromProject,
  serializeRcaListagemBForFirestore} from './rca-project-prefill';
...
function mapCurrentItemToFormValues(currentItem: RCA): RcaListagemBFormValues {
  const extended = currentItem as RCA & Partial<RcaListagemBFormValues>;
  const formularioTipo = normalizarFormularioTipoRcaListagemB(
    extended.formularioTipo ?? inferirFormularioRcaListagemB(extended.subActivity),
  );
  return getRcaListagemBDefaultValues({
    ...getRcaListagemBInitialValues(currentItem),
    ...extended,
    listagemCode: 'B',
    activity: extended.activity || RCA_LISTAGEM_B_ACTIVITY,
    subActivity:
...
export function RcaFormListagemB({ currentItem, onSuccess }: RcaFormListagemBProps) {
  ...
  const form = useForm<any>({
    resolver: zodResolver(rcaListagemBFormSchema),
    defaultValues: currentItem
      ? mapCurrentItemToFormValues(currentItem)
      : getRcaListagemBDefaultValues({ activity: RCA_LISTAGEM_B_ACTIVITY })});

  const formularioTipo =
    form.watch('formularioTipo') ?? RCA_LISTAGEM_B_FORM_TIPO_PADRAO;
  ...
  const setFormularioTipo = React.useCallback(
    (tipo: RcaListagemBFormTipo) => {
      form.setValue('formularioTipo', tipo, { shouldDirty: true });
      form.setValue('subActivity', subatividadeParaFormularioRcaListagemB(tipo), {
        shouldDirty: true});
    },
    [form],
  );
  ...
  React.useEffect(() => {
    ...
    const patch = prefillRcaListagemBFromProject(project, empreendedor);
  ...
  async function handleSave(status: 'Rascunho' | 'Aprovado') {
    ...
    const values = form.getValues() as RcaListagemBFormValues;
    if (!firestore) {
    ...
    const dataToSave = serializeRcaListagemBForFirestore(values, status);
    ...
    try {
      if (currentItem?.id) {
        ...
        toast({
          title: 'RCA criado',
          description: `RCA Listagem B para ${(values.empreendimento as { nome?: string })?.nome ?? 'empreendimento'} criado.`});
      }
      ...
      >
        <div className="flex-1 space-y-4 overflow-y-auto pr-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">RCA – indústria metalúrgica e afins</Badge>
          ...
          <RcaListagemBFormularioTipoCard
            form={form}
          ...
          {formularioTipo === 'materiais_ceramicos' && (
            <RcaFormMateriaisCeramicos {...formProps} />
          )}
          {formularioTipo === 'siderurgia' && <RcaFormSiderurgia {...formProps} />}
          {formularioTipo === 'ligas_ferrosas' && <RcaFormLigasFerrosas {...formProps} />}
          {formularioTipo === 'fundidos_ferro_aco' && (
            <RcaFormFundidosFerroAco {...formProps} />
          )}
          {formularioTipo === 'fundidos_nao_ferrosos' && (
            <RcaFormFundidosNaoFerrosos {...formProps} />
          )}
          {formularioTipo === 'moveis' && <RcaFormMoveis {...formProps} />}
        </div>
