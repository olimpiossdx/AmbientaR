...
import {
  ...
  type RcaListagemDFormTipo} from './rca-listagem-d-registry';
import {
  getRcaListagemDDefaultValues,
  rcaListagemDFormSchema,
  type RcaListagemDFormValues} from './rca-listagem-d-schema';
import { RcaListagemDFormularioTipoCard } from './rca-form-listagem-d-tipo-card';
...
import {
  prefillRcaListagemDFromProject,
  serializeRcaListagemDForFirestore} from './rca-project-prefill';
...
interface RcaFormListagemDProps {
  currentItem?: RCA | null;
  onSuccess?: () => void;
}

function mapCurrentItemToFormValues(currentItem: RCA): RcaListagemDFormValues {
  const extended = currentItem as RCA & Partial<RcaListagemDFormValues>;
  const formularioTipo = normalizarFormularioTipoRcaListagemD(
    extended.formularioTipo ?? inferirFormularioRcaListagemD(extended.subActivity),
  );
  return getRcaListagemDDefaultValues({
    ...getRcaListagemDInitialValues(currentItem),
    ...extended,
    listagemCode: 'D',
    activity: extended.activity || RCA_LISTAGEM_D_ACTIVITY,
    subActivity:
...
export function RcaFormListagemD({ currentItem, onSuccess }: RcaFormListagemDProps) {
  ...
  const form = useForm<any>({
    resolver: zodResolver(rcaListagemDFormSchema),
    defaultValues: currentItem
      ? mapCurrentItemToFormValues(currentItem)
      : getRcaListagemDDefaultValues({ activity: RCA_LISTAGEM_D_ACTIVITY })});

  const formularioTipo =
    form.watch('formularioTipo') ?? RCA_LISTAGEM_D_FORM_TIPO_PADRAO;
  ...
  const setFormularioTipo = React.useCallback(
    (tipo: RcaListagemDFormTipo) => {
      form.setValue('formularioTipo', tipo, { shouldDirty: true });
      form.setValue('subActivity', subatividadeParaFormularioRcaListagemD(tipo), {
        shouldDirty: true});
    },
    [form],
  );
  ...
  React.useEffect(() => {
    ...
    const patch = prefillRcaListagemDFromProject(project, empreendedor);
  ...
  async function handleSave(status: 'Rascunho' | 'Aprovado') {
    ...
    const values = form.getValues() as RcaListagemDFormValues;
    if (!firestore) {
    ...
    const dataToSave = serializeRcaListagemDForFirestore(values, status);
    ...
    try {
      if (currentItem?.id) {
        ...
        toast({
          title: 'RCA criado',
          description: `RCA Listagem D para ${(values.empreendimento as { nome?: string })?.nome ?? 'empreendimento'} criado.`});
      }
      ...
      >
        <div className="flex-1 space-y-4 overflow-y-auto pr-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">RCA – indústria alimentícia</Badge>
          ...
          <RcaListagemDFormularioTipoCard
            form={form}
          ...
          {formularioTipo === 'aguardente' && <RcaFormAguardente {...formProps} />}
          {formularioTipo === 'laticinios' && <RcaFormLaticinios {...formProps} />}
          {formularioTipo === 'abatedouros' && <RcaFormAbatedouros {...formProps} />}
          {formularioTipo === 'racao_animal' && <RcaFormRacaoAnimal {...formProps} />}
          {formularioTipo === 'subprodutos_animal' && (
            <RcaFormSubprodutosAnimal {...formProps} />
          )}
          {formularioTipo === 'oleos_gorduras' && <RcaFormOleosGorduras {...formProps} />}
        </div>
