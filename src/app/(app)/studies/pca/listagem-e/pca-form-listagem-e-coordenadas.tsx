...
export function TrechoCoordenadasBlock({
  ...
  return (
    <div className="space-y-4 rounded-md border p-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="space-y-2 rounded-md border p-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <PcaTextField form={form} name={`${basePath}.latLong.lat.grau`} label="Grau" />
              <PcaTextField form={form} name={`${basePath}.latLong.lat.min`} label="Min" />
              <PcaTextField form={form} name={`${basePath}.latLong.lat.seg`} label="Seg" />
            </div>
          </div>
          <div className="space-y-2 rounded-md border p-3">
            <p className="text-sm font-medium">Longitude</p>
            <div className="grid grid-cols-3 gap-2">
              <PcaTextField form={form} name={`${basePath}.latLong.long.grau`} label="Grau" />
              <PcaTextField form={form} name={`${basePath}.latLong.long.min`} label="Min" />
              <PcaTextField form={form} name={`${basePath}.latLong.long.seg`} label="Seg" />
            </div>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <PcaTextField form={form} name={`${basePath}.utm.x`} label="X (6 dígitos)" />
          <PcaTextField form={form} name={`${basePath}.utm.y`} label="Y (7 dígitos)" />
          <FormField
            control={form.control}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <PcaTextField form={form} name={`${basePath}.local`} label="Local (fazenda, sítio etc.)" />
        <PcaTextField form={form} name={`${basePath}.municipio`} label="Município(s)" />
        <PcaTextField
          form={form}
        ...
        />
        <PcaTextField form={form} name={`${basePath}.baciaHidrografica`} label="Bacia hidrográfica" />
        <PcaTextField form={form} name={`${basePath}.subBaciaHidrografica`} label="Sub-bacia hidrográfica" />
        <PcaTextField form={form} name={`${basePath}.upgrh`} label="UPGRH" />
        <PcaTextField form={form} name={`${basePath}.cursoDaguaProximo`} label="Curso d'água mais próximo" />
      </div>
    </div>
  );
}
