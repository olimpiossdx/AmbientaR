...
/** Bloco técnico PCA Listagem A — estrutura alinhada ao TR / cadastro empreendimento (listagemA.*). */
export const pcaListagemATecnicoSchema = z.any().optional();

export const pcaListagemAFormSchema = z.object({
  ...
  listagemA: pcaListagemATecnicoSchema,
}).passthrough();

export type PcaListagemAFormValues = z.infer<typeof pcaListagemAFormSchema>;

export function getPcaListagemADefaultValues(
  partial?: Partial<PcaListagemAFormValues> | null,
): PcaListagemAFormValues {
  return {
    status: 'Rascunho',
    listagemCode: 'A',
    activity: partial?.activity || PCA_LISTAGEM_A_ACTIVITY,
    subActivity: partial?.subActivity ?? '',
    formularioTipo: partial?.formularioTipo ?? 'geral',
    formSource: 'react',
    termoReferencia: {
