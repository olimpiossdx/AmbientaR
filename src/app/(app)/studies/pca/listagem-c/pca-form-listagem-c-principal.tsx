...
import { PcaFormListagemCEspecifico } from './pca-form-listagem-c-especifico';
import { PcaFormListagemCSecao6 } from './pca-form-listagem-c-secao6';
import { PcaFormListagemCModulo4Complemento } from './pca-form-listagem-c-modulo4-complemento';
import { CoordinateInput } from '@/components/coordinates';
import type { Datum } from '@/lib/types';
interface PcaFormListagemCComumProps {
  form: any;
...
export function PcaFormListagemCPrincipal({
  form,
  hideEspecifico,
  slotEspecifico,
  slotSecao6,
  ...
  const { fields: outrasAtividadesFields, append: appendOutrasAtividades, remove: removeOutrasAtividades } =
    useFieldArray({ control: form.control, name: 'listagemC.outrasAtividades' as never });
  const { fields: nucleoPopulacionalFields, append: appendNucleoPopulacional, remove: removeNucleoPopulacional } =
    useFieldArray({ control: form.control, name: 'listagemC.legislacaoMunicipal.nucleosPopulacionais' as never });
  const { fields: recursosHidricosFields, append: appendRecursosHidricos, remove: removeRecursosHidricos } =
    useFieldArray({ control: form.control, name: 'listagemC.recursosHidricos.intervencoes' as never });
  return (
    <div className="space-y-6">
      ...
      />
      {slotSecao6 ?? <PcaFormListagemCSecao6 form={form} />}
...
<div className="space-y-4 rounded-md border p-4">
      ...
      {slotModulo4Complemento ?? <PcaFormListagemCModulo4Complemento form={form} />}

      {hideEspecifico ? slotEspecifico : <PcaFormListagemCEspecifico form={form} />}
