          ...
          <TooltipTrigger asChild>
            <Button variant="ghost" size="icon" className="h-9 w-9 shrink-0" asChild>
              <Link
                ...
                aria-label="Elaborar relat├│rio"
              >
                <FileText className="h-4 w-4 text-primary" />
              </Link>
            </Button>
          </TooltipTrigger>
          <TooltipContent>Elaborar relat├│rio a partir deste projeto</TooltipContent>
        </Tooltip>
      )}
          ...
          items.map((study) => (
            <TableRow key={study.id}>
              <TableCell className="font-medium">
                {empreendedorMap.get(study.empreendedorId) || "ÔÇö"}
              </TableCell>
              ...
              <TableCell className="hidden lg:table-cell">
                ...
                >
                  {study.status === "completed" ? "Conclu├¡do" : "Rascunho"}
                </Badge>
              </TableCell>
              <TableCell className="text-right">
                <StudyRowActions
...
export function StudiesFaunaHubView() {
  ...
  const handleComplete = async (study: FaunaStudy) => {
    ...
    try {
      ...
      toast({
        title: "Estudo conclu├¡do",
        description:
          "O documento ficar├í dispon├¡vel em Documentos Ambientais ÔåÆ Fauna para o cliente.",
      });
    } catch (serverError) {
      handleFirestoreFormError(serverError, {
        toast,
    ...
    <>
      <div className="flex flex-col h-full">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6 space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Em elabora├º├úo</CardTitle>
              <CardDescription>
                Rascunhos da equipe t├®cnica. Ao concluir, o documento passa a
                aparecer em Documentos Ambientais ÔåÆ Fauna.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TooltipProvider>
                <StudyTable
                  ...
                  showComplete
                  emptyMessage="Nenhum estudo em elabora├º├úo."
                />
              </TooltipProvider>
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle>Conclu├¡dos pela consultoria</CardTitle>
              <CardDescription>
                Estudos finalizados, vis├¡veis no portal do cliente em Fauna.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <TooltipProvider>
                <StudyTable
                  ...
                  onDelete={openDeleteConfirm}
                  canDeleteStudy={canDeleteStudy}
                  emptyMessage="Nenhum estudo conclu├¡do ainda."
                />
              </TooltipProvider>
            </CardContent>
          </Card>
      ...
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta a├º├úo n├úo pode ser desfeita. O documento ser├í removido
              permanentemente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
