...
export function OutorgaForm({ currentItem, onSuccess }: OutorgaFormProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `outorgas/${Date.now()}-${file.name}`);
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <FormLabel className="cursor-help">Anexar Documento da Outorga</FormLabel>
                    </TooltipTrigger>
                ...
                <FormDescription>
                  Anexe a outorga ou documento relacionado (PDF, JPG, PNG). Máx 10MB.
