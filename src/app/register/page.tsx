...
import { Loader2, Leaf, ArrowLeft, ArrowRight, Check, Crown, Star, Zap, Rocket, Gift, MessageSquareMore, X } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleCancelRegistration = () => {
    if (window.confirm('Deseja cancelar o cadastro? Os dados preenchidos serão descartados.')) {
      form.reset();
      router.push('/login');
    }
  };
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      await setDoc(doc(firestore, 'users', uid), {
        ...
        contractAcceptedAt: mode === 'representative' ? null : serverTimestamp(),
        createdAt: serverTimestamp(),
        lastLogin: serverTimestamp(),
        isOnline: true,
      });

      // Apenas clientes (titulares) geram cadastro automático em Clientes/Empreendedores.
      if (mode === 'client') {
        try {
          const clientData = {
            name: values.name,
            cpfCnpj: values.cpf.trim(),
          ...
          toast({
            title: 'Cadastro concluído com aviso',
            description:
              'Sua conta foi criada. Se os dados não aparecerem em Clientes ou Empreendedores, entre em contato com o suporte.',
            variant: 'destructive',
          });
        }
      }
    ...
    <Card className="shadow-lg bg-card/80 backdrop-blur-sm border">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">A primeira etapa do cadastro é informar seu CPF.</p>
        </div>
        <FormField
    ...
    <Card className="shadow-lg bg-card/80 backdrop-blur-sm border">
      ...
      <CardContent>
        ...
        <div className="flex gap-3 mt-6 flex-wrap">
          <Button type="button" variant="outline" onClick={handleCancelRegistration}>
      ...
      <div className="w-full max-w-2xl animate-fade-in-up">
        <div className="mb-6 flex flex-col items-center">
          ...
          <p className="text-center text-muted-foreground text-sm">
            {mode === 'representative' ? 'Cadastro de Representante' : 'Cadastro de Cliente'}
          </p>
